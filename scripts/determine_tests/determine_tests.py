...
def main():
    ...
    if len(sys.argv) >= 2 and sys.argv[1] == "1":
        ...
        if len(sys.argv) >= 3 and sys.argv[2] == "pr":
            ...
            subprocess.run([
                "git", "remote", "add", "upstream", "https://github.com/khulnasoft/aikit"
            ])
        ...
        directories = (
            ...
            + [x[0] for x in os.walk("aikit_tests/test_aikit")]
            + ["aikit_tests"]
        )
        ...
        for test_backend in new_tests[old_num_tests:num_tests]:
            ...
            if len(sys.argv) < 3:
                ...
                command = (
                    f'docker run -v "$(pwd)":/aikit khulnasoft/aikit:latest /bin/bash -c "coverage run --source=aikit,'  # noqa
                    f"aikit_tests -m pytest {test_name} --backend {backend} --disable-warnings > coverage_output;coverage "  # noqa
