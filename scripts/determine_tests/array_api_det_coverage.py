...
def main():
    ...
    test_names = []
    func_folder = "aikit_tests/array_api_testing/array_api_methods_to_test"
            ...
            for framework in framework_tests_to_run:
                ...
                for s in contents:
                    ...
                    if ("#" not in s) or (
                        ...
                        submod = f"aikit_tests/array_api_testing/test_array_api/array_api_tests/test_{fname.replace('.txt', '.py')}"  # noqa
    ...
    for backend in BACKENDS:
        k_flag_file = f"aikit_tests/array_api_testing/.array_api_tests_k_flag_{backend}"
    ...
    directories = (
        [x[0] for x in os.walk("aikit")]
        + [x[0] for x in os.walk("aikit_tests/array_api_testing")]
        + ["aikit_tests"]
    )
