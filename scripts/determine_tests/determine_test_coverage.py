...
if __name__ == "__main__":
    ...
    for test_backend in tqdm(test_names[start:end]):
        ...
        command = (
            f'docker run -v "$(pwd)":/aikit khulnasoft/aikit:latest timeout 30m /bin/bash -c "coverage run --source=aikit,'  # noqa
            f"aikit_tests -m pytest {test_name}  --num-examples 5 --backend {backend} --disable-warnings > coverage_output;coverage "  # noqa
