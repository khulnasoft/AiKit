...
def _move_to_device(args=None, kwargs=None, device="cpu"):
    ...
    kwargs_idxs = aikit.nested_argwhere(kwargs, aikit.is_array)

    def func(x):
        return aikit.to_device(x, device, out=x)

    if args is not None:
        args = aikit.map_nest_at_indices(args, args_idxs, func)
    if kwargs is not None:
        kwargs = aikit.map_nest_at_indices(kwargs, kwargs_idxs, func)
    return args, kwargs
...
def eager_benchmark(
    ...
    obj
        ...
        In case of a function from aikit's functional API, this parameter would receive
        a string which is the function name, along with functional_api set to True.
    functional_api
        Should only be set to ``True`` if the obj being passed is a part of aikit's
    ...
    backends
        ...
        that aikit supports (Default value = ``None``).
    devices
        A list of target devices that aikit supports with the backends. The devices that
    ...
    >>> fn = "conv1d"
    >>> args = (
    ...     aikit.array([[[0.0], [3.0], [0.0]]], device="cpu"),
    ...     aikit.array([[[0.0]], [[1.0]], [[0.0]]], device="cpu"),
    ...     (1,),
    ...     "SAME",
    ... )
    ...
    >>> aikit.set_backend("torch")
    >>> def fn(*args, **kwargs):
    ...     return aikit.conv1d(*args, **kwargs) + 1
    >>> args = (
    ...     aikit.array([[[0.0], [3.0], [0.0]]], device="cpu"),
    ...     aikit.array([[[0.0]], [[1.0]], [[0.0]]], device="cpu"),
    ...     (1,),
    ...
    >>> from benchmark import eager_benchmark
    >>> aikit.set_backend("torch")
    >>> module = aikit.GELU(approximate=False)
    >>> args = (aikit.random_uniform(shape=(4, 32)),)
    >>> eager_benchmark(
    ...     module,
    ...     label="GELU",
    ...     backends=["jax", "numpy", "tensorflow", "torch"],
    ...
    ... )
    """
    backends = aikit.default(backends, [])
    devices = aikit.default(devices, [])
    output_path = aikit.default(output_path, "./report.csv")
...
def visualize_speed_up(
    ...
    >>> visualize_speed_up(
    ...     file_path="./aikit/report.csv",
    ...     output_path="./aikit/save_fig.png",
    ...     backends=["torch", "jax"],
    ...     devices=["cpu", "gpu:0"],
    ... )
    ...
    ... )
    """
    file_path = aikit.default(file_path, "./report.csv")
    output_path = aikit.default(output_path, "./saved_fig.png")
