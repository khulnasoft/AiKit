...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 8 and sys.argv[8] != "null":
        ...
        run_id = f"https://github.com/khulnasoft/aikit/actions/runs/{workflow_id}"
    ...
    if device == "gpu":
        os.system("docker pull khulnasoft/aikit:latest-gpu")
    ...
    with open("tests_to_run", "r") as f:
        for line in f:
            ...
            collection = db["frontend_tests"] if is_frontend_test else db["aikit_tests"]
            ...
            if version_flag == "true":
                ...
                os.system(
                    'docker run --name test-container -v "$(pwd)":/aikit/aikit '
                    f"-e REDIS_URL={redis_url} -e REDIS_PASSWD={redis_pass} "
                    "-itd khulnasoft/multiversion:latest /bin/bash -c"
                    f'python multiversion_framework_directory.py {" ".join(backends)};'
                )
                os.system(
                    "docker exec test-container cd aikit; python3 -m pytest --tb=short "
                ...
                device_str = ""
                device_access_str = ""
                image = "khulnasoft/aikit:latest"

                # gpu tests
                if device == "gpu":
                    image = "khulnasoft/aikit:latest-gpu"
                    device_str = " --device=gpu:0"
                    device_access_str = " --gpus all"

                os.system(
                    f"docker run{device_access_str} --name test-container -v "
                    '"$(pwd)":/aikit -v "$(pwd)"/.hypothesis:/.hypothesis -e '
            ...
            if not failed and function_name:
                ...
                os.system(command)
                os.system(
                    "docker cp test-container:/aikit/report.json"
            ...
            # for frontend tests and aikit tests respectively
            test_info = {
                "_id": function_name,
                "test_path": test_path,
                "submodule": submodule,
                ...
                if is_frontend_test:
                    test_info = {
                        **test_info,
                        ...
                        "aikit_nodes": report_content["aikit_nodes"],
                    }
