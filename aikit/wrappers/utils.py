...
wrapers_dir = os.path.join(folder_path, "aikit/wrappers")


def download_cython_wrapper(func_name: str):
    """Get the wrapper for the given function name."""
    if func_name + ".so" not in wrappers["aikit"]["functional"]:
...
def wrapper_exists(func_name: str):
    ...
    return func_name + ".so" in wrappers["aikit"]["functional"]
...
def load_one_wrapper(func_name: str):
    ...
    if os.path.isfile(os.path.join(dir_path, module_name + ".so")):
        aikit.wrappers.__dict__[module_name] = importlib.import_module(module_name)
        aikit.wrappers.__dict__[module_name + "_wrapper"] = getattr(
            aikit.wrappers.__dict__[module_name], module_name + "_wrapper"
        )
        aikit.wrappers.__all__.append(module_name + "_wrapper")
        return True
    else:
        return False
