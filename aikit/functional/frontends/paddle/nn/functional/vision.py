...
from aikit.utils.assertions import check_equal
...
def affine_grid(theta, out_shape, align_corners=True):
    if len(out_shape) == 4:
        ...
        if align_corners:
            ...
            height_values = aikit.expand_dims(aikit.linspace(-1, 1, H), axis=-1)
            base_grid[:, :, :, 1] = aikit.array(
            ...
            height_values = aikit.expand_dims(
                aikit.linspace(-1, 1, H) * (H - 1) / H, axis=-1
            )
            base_grid[:, :, :, 1] = aikit.array(
                [[[height_values[i]] * W for i in range(H)]]
        ...
        if align_corners:
            base_grid[:, :, :, :, 0] = aikit.linspace(-1, 1, W)
            base_grid[:, :, :, :, 1] = aikit.expand_dims(aikit.linspace(-1, 1, H), axis=-1)
            height_values = aikit.linspace(-1, 1, H)
            base_grid[:, :, :, :, 1] = aikit.array(
            ...
            width_values = aikit.linspace(-1, 1, D)
        else:
            base_grid[:, :, :, :, 0] = aikit.linspace(-1, 1, W) * (W - 1) / W
            base_grid[:, :, :, :, 1] = aikit.expand_dims(
                aikit.linspace(-1, 1, H) * (H - 1) / H, axis=-1
            )
            height_values = aikit.linspace(-1, 1, H) * (H - 1) / H
            base_grid[:, :, :, :, 1] = aikit.array(
                [[[[height_values[i]] * W for i in range(H)]] * D]
            )
            base_grid[:, :, :, :, 2] = aikit.expand_dims(
            ...
            width_values = aikit.linspace(-1, 1, D) * (D - 1) / D

        base_grid[:, :, :, :, 2] = aikit.array(
            [[aikit.array([[width_values[i]] * W] * H) for i in range(D)]]
        )
...
def channel_shuffle(x, groups, data_format="NCHW", name=None):
    ...
    if data_format == "NCHW":
        b, c, h, w = aikit.shape(x)
        x = aikit.reshape(x, (b, groups, c // groups, h, w))
        x = aikit.permute_dims(x, (0, 2, 1, 3, 4))
        x = aikit.reshape(x, (b, c, h, w))
    else:
        b, h, w, c = aikit.shape(x)
        x = aikit.reshape(x, (b, h, w, groups, c // groups))
        x = aikit.permute_dims(x, (0, 1, 2, 4, 3))
        x = aikit.reshape(x, (b, h, w, c))
    return x


@to_aikit_arrays_and_back
def pixel_shuffle(x, upscale_factor, data_format="NCHW"):
    input_shape = aikit.shape(x)
    check_equal(
        len(input_shape),
        4,
    ...
    if data_format == "NCHW":
        input_reshaped = aikit.reshape(x, (b, oc, upscale_factor, upscale_factor, h, w))
    else:
        input_reshaped = aikit.reshape(x, (b, h, w, upscale_factor, upscale_factor, oc))
    ...
    return aikit.reshape(
        aikit.permute_dims(input_reshaped, (0, 1, 4, 2, 5, 3)), (b, oh, ow, oc)
    )
...
def pixel_unshuffle(x, downscale_factor, data_format="NCHW"):
    ...
    if data_format == "NCHW":
        ...
        x = aikit.reshape(x, (b, c, oh, downscale_factor, ow, downscale_factor))
        x = aikit.permute_dims(x, (0, 1, 3, 5, 2, 4))
        x = aikit.reshape(x, (b, oc, oh, ow))
    else:
        b, h, w, c = aikit.shape(x)
        oc = c * downscale_factor**2
        oh = h // downscale_factor
        ...
        x = aikit.reshape(x, (b, downscale_factor, oh, downscale_factor, ow, c))
        x = aikit.permute_dims(x, (0, 1, 3, 5, 2, 4))
        x = aikit.reshape(x, (b, oh, ow, oc))
    return x
