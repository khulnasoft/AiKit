...
def adaptive_avg_pool1d(x, output_size, name=None):
    return aikit.adaptive_avg_pool1d(x, output_size)


@to_aikit_arrays_and_back
@with_supported_dtypes({"2.6.0 and below": ("float32", "float64")}, "paddle")
def adaptive_avg_pool2d(x, output_size, data_format="NCHW", name=None):
    return aikit.adaptive_avg_pool2d(x, output_size, data_format=data_format)


@to_aikit_arrays_and_back
@with_supported_dtypes({"2.5.0 and below": ("float32", "float64")}, "paddle")
def adaptive_avg_pool3d(x, output_size, data_format="NCHW", name=None):
    return aikit.adaptive_avg_pool3d(x, output_size)


@to_aikit_arrays_and_back
@with_supported_dtypes({"2.6.0 and below": ("float32", "float64")}, "paddle")
def adaptive_max_pool2d(x, output_size, return_mask=None, name=None):
    return aikit.adaptive_max_pool2d(x, output_size)
...
def avg_pool1d(
    ...
    if all(
        pad == aikit.ceil((kernel - 1) / 2) for kernel, pad in zip(kernel_size, padding)
    ):
        padding = "SAME"
    else:
        padding = "VALID"
