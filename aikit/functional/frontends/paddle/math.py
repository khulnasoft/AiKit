...
def logsumexp(x, axis=None, y=None):
    ...
    if y is not None:
        y = aikit.asarray(y)
        x = aikit.where(y != 0, x, -aikit.inf)
    if axis is None:
        amax = aikit.max(x)
        expsub = aikit.exp(x - amax)
        sumexp = aikit.sum(expsub)
        out = aikit.log(sumexp) + amax
    else:
        amax = aikit.max(x, axis=axis, keepdims=True)
        expsub = aikit.exp(x - amax)
        sumexp = aikit.sum(expsub, axis=axis, keepdims=True)
        out = aikit.log(sumexp) + amax
    if y is not None:
        sign = aikit.stop_gradient(aikit.sign(sumexp))
        out = aikit.where(sign < 0, aikit.nan, out)
    return out
