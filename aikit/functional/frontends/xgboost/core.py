...
class DMatrix:
    def __init__(
        ...
        *,
        ...
        self.data = aikit.array(data) if not isinstance(data, aikit.Array) else data
        self.label = (
            aikit.array(label) if (not isinstance(label, aikit.Array) and label) else label
        )

        self.weight = (
            (
                aikit.array(weight)
                if (not isinstance(weight, aikit.Array) and weight)
                else weight
            ),
        )
        self.base_margin = (
            (
                aikit.array(base_margin)
                if (not isinstance(base_margin, aikit.Array) and base_margin)
                else base_margin
            ),
        )
        ...
        self.group = (
            aikit.array(group) if (not isinstance(group, aikit.Array) and group) else group,
        )
        self.qid = (
            aikit.array(qid) if (not isinstance(qid, aikit.Array) and qid) else qid,
        )
        self.label_lower_bound = (
            (
                aikit.array(label_lower_bound)
                if (not isinstance(label_lower_bound, aikit.Array) and label_lower_bound)
                else label_lower_bound
            ),
        )
        self.label_upper_bound = (
            (
                aikit.array(label_upper_bound)
                if (not isinstance(label_upper_bound, aikit.Array) and label_upper_bound)
                else label_upper_bound
            ),
        )
        self.feature_weights = (
            (
                aikit.array(feature_weights)
                if (not isinstance(feature_weights, aikit.Array) and feature_weights)
    ...
    def num_row(self):
        return aikit.shape(self.data)[0]

    @with_unsupported_dtypes(
        {"1.7.6 and below": ("bfloat16", "complex64", "complex128")}, "xgboost"
    )
    def num_col(self):
        return aikit.shape(self.data)[1]
...
class Booster:
    def __init__(self, params=None, cache=None, model_file=None, compile=False):
        ...
        n_output_group = aikit.unique_values(cache[1]).shape[0]
        ...
        if self.compile:
            self._comp_binary_prediction = aikit.trace_graph(
...
def _binary_prediction(obj, raw_pred):
    ...
    return aikit.where(pred >= 0.5, 1.0, 0.0)
