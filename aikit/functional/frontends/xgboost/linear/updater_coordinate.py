import aikit
from aikit.functional.frontends.xgboost.linear.coordinate_common import (
    get_bias_gradient,
...
def coordinate_updater(gpair, data, lr, weight, n_feat, n_iter, reg_alpha, reg_lambda):
    ...
    hess = aikit.expand_dims(gpair[:, 1], axis=1)

    # don't update where hessian is less than zero
    mask = aikit.where(hess < 0.0, 0.0, 1.0)
    sum_hess = aikit.sum(aikit.square(data) * hess * mask, axis=0, keepdims=True)
    sum_grad = aikit.sum(data * grad * mask, axis=0, keepdims=True)
    ...
    return aikit.vstack([feature_weights, bias_weights])
