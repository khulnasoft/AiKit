...
class ClassificationCriterion(Criterion):
    def __init__(self, n_outputs: int, n_classes: aikit.Array):
        self.start = 0
        self.pos = 0
        self.end = 0
        ...
        self.n_classes = aikit.empty(n_outputs, dtype=aikit.int16)
        max_n_classes = 0
        ...
        if isinstance(max_n_classes, aikit.Array):
            max_n_classes = aikit.to_scalar(max_n_classes)
        self.max_n_classes = max_n_classes

        self.sum_total = aikit.zeros((n_outputs, max_n_classes), dtype=aikit.float64)
        self.sum_left = aikit.zeros((n_outputs, max_n_classes), dtype=aikit.float64)
        self.sum_right = aikit.zeros((n_outputs, max_n_classes), dtype=aikit.float64)
    ...
    def init(
        ...
        for k in range(self.n_outputs):
            n_cls = aikit.to_scalar(self.n_classes[k])
    ...
    def init_sum_missing(self):
        self.sum_missing = aikit.zeros(
            (self.n_outputs, self.max_n_classes), dtype=aikit.float64
        )
    ...
    def update(self, new_pos):
        ...
        for k in range(self.n_outputs):
            for c in range(aikit.to_scalar(self.n_classes[k])):
