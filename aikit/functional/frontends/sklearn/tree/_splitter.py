...
class Splitter:
    ...
    def init(
        ...
        *args,
        ...
        n_samples = X.shape[0]
        self.samples = aikit.empty(n_samples, dtype=aikit.int32)
        ...
        self.features = aikit.arange(n_features, dtype=aikit.int32)
        self.n_features = n_features
        self.feature_values = aikit.empty(n_samples, dtype=aikit.float32)
        self.constant_features = aikit.empty(n_features, dtype=aikit.int32)
        self.y = y
...
class DensePartitioner:
    ...
    def sort_samples_and_feature_values(self, current_feature):
        ...
        if (
            ...
            while i <= current_end:
                if aikit.isnan(X[samples[current_end], current_feature]):
                    n_missing += 1
                    current_end -= 1
                    continue
                if aikit.isnan(X[samples[i], current_feature]):
    ...
    def partition_samples_final(
        ...
        if best_n_missing != 0:
            while p < partition_end:
                if aikit.isnan(X[samples[end], best_feature]):
                    end -= 1
                    continue
                current_value = X[samples[p], best_feature]
                if aikit.isnan(current_value):
...
def _init_split(split_record, start_pos):
    split_record.impurity_left = aikit.inf
    split_record.impurity_right = aikit.inf
    split_record.pos = start_pos
    split_record.feature = 0
    split_record.threshold = 0.0
    split_record.improvement = -aikit.inf
...
def node_split_best(
    ...
    best_proxy_improvement = -aikit.inf
    ...
    while f_i > n_total_constants and (
        ...
        f_j = aikit.randint(n_drawn_constants, f_i - n_found_constants)
            ...
            while p < end_non_missing:
                ...
                if current_proxy_improvement > best_proxy_improvement:
                    ...
                    if (
                        ...
                        or current_split.threshold == aikit.inf
                        or current_split.threshold == -aikit.inf
                    ):
        ...
        if has_missing:
            ...
            if not ((n_left < min_samples_leaf) or (n_right < min_samples_leaf)):
                ...
                if not (
                    ...
                    if current_proxy_improvement > best_proxy_improvement:
                        best_proxy_improvement = current_proxy_improvement
                        current_split.threshold = aikit.inf
                        current_split.missing_go_to_left = missing_go_to_left
                        current_split.n_missing = n_missing
                        current_split.pos = p
...
def sort(feature_values, samples, n):
    if n == 0:
        return
    idx = aikit.argsort(feature_values)
