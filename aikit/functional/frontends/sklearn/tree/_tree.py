...
EPSILON = aikit.finfo(aikit.double).eps
INFINITY = aikit.inf
INTPTR_MAX = aikit.iinfo(aikit.int32).max
TREE_LEAF = -1
TREE_UNDEFINED = -2
...
class Tree:
    def __init__(self, n_features, n_classes, n_outputs):
        ...
        self.n_classes = aikit.zeros(n_outputs, dtype=aikit.int32)

        self.max_n_classes = aikit.max(n_classes)
    ...
    def predict(self, X):
        ...
        X_threshold = aikit.where(X_applied > X.shape[0], X.shape[0] - 1, X_applied)
        out = aikit.gather(self.value, X_threshold, axis=0)
        if self.n_outputs == 1:
