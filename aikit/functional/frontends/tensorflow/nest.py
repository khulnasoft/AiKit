# local
import aikit.functional.frontends.tensorflow.ragged as ragged_tf
...
def _flatten_composite_array(x, expand_composites=False):
    if isinstance(x, ragged_tf.RaggedTensor):
        ...
        return new_struc
    elif aikit.is_aikit_sparse_array(x) or aikit.is_native_sparse_array(x):
        return aikit.native_sparse_array_to_indices_values_and_shape(x)
...
def _is_composite_array(x):
    if isinstance(x, ragged_tf.RaggedTensor):
        return True
    if aikit.is_aikit_sparse_array(x) or aikit.is_native_sparse_array(x):
        return True
    return False
