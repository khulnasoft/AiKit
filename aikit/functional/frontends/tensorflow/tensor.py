...
from aikit.functional.frontends.tensorflow.func_wrapper import _to_aikit_array
...
class EagerTensor:
    ...
    def __repr__(self):
        return (
            repr(self.aikit_array).replace(
                "aikit.array", "aikit.frontends.tensorflow.EagerTensor"
            )[:-1]
            + ", shape="
            + str(self.shape)
            + ", dtype="
    ...
    def dtype(self):
        return tf_frontend.DType(
            tf_frontend.tensorflow_type_to_enum[self.aikit_array.dtype]
        )

    @property
    def shape(self):
        return TensorShape(self.aikit_array.shape.shape)
    ...
    def __bool__(self, name="bool"):
        temp = aikit.squeeze(self.aikit_array, axis=None)
        if temp.shape != ():
            raise ValueError(
    ...
    def __getitem__(self, slice_spec, var=None, name="getitem"):
        aikit_args = aikit.nested_map(_to_aikit_array, [self, slice_spec])
...
class TensorShape:
    ...
    def num_elements(self):
        return aikit.to_scalar(aikit.prod(self._dims))
