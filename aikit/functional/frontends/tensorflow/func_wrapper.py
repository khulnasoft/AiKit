...
# local
import aikit
import aikit.functional.frontends.tensorflow as frontend
...
def _aikit_array_to_tensorflow(x):
...
def _tf_frontend_array_to_aikit(x):
...
def _to_aikit_array(x):
    return _tf_frontend_array_to_aikit(_native_to_aikit_array(x))
...
def inputs_to_aikit_arrays(fn: Callable) -> Callable:
    @functools.wraps(fn)
    def _inputs_to_aikit_arrays_tf(*args, **kwargs):
        """Convert all `TensorFlow.Tensor` instances in both the positional and
        keyword arguments into `aikit.Array` instances, and then call the
        ...
        aikit_args = aikit.nested_map(
            _to_aikit_array, args, include_derived=True, shallow=False
        )
        aikit_kwargs = aikit.nested_map(
            _to_aikit_array, kwargs, include_derived=True, shallow=False
        )
        if has_out:
            aikit_kwargs["out"] = out
        return fn(*aikit_args, **aikit_kwargs)

    _inputs_to_aikit_arrays_tf.inputs_to_aikit_arrays_tf = True
    return _inputs_to_aikit_arrays_tf
...
def outputs_to_frontend_arrays(fn: Callable) -> Callable:
    ...
    def _outputs_to_frontend_arrays_tf(*args, **kwargs):
        ...
        instances in the function return into `aikit.Array` instances.
        ...
        Returns
        -------
            The return of the function, with aikit arrays as tensorflow.Tensor arrays.
        """
        ...
        return aikit.nested_map(
            _aikit_array_to_tensorflow, ret, include_derived={"tuple": True}
        )
...
def to_aikit_dtype(dtype):
    if not dtype or isinstance(dtype, str):
        return dtype
    if isinstance(dtype, np_frontend.dtype):
        return dtype.aikit_dtype
    return frontend.as_dtype(dtype).aikit_dtype
