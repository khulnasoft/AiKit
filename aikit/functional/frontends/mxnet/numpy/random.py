...
def beta(a, b, size=None, dtype=None, device=None):
    return aikit.experimental.beta(a, b, shape=size, dtype=dtype, device=device)


@to_aikit_arrays_and_back
def chisquare(df, size=None, dtype=None, device=None):
    return aikit.experimental.gamma(
        df * 0.5,
        0.5,
        shape=size,
        dtype=dtype,
...
def multinomial(n, pvals, size=None, **kwargs):
    ...
    return aikit.multinomial(n, num_samples, batch_size=batch_size, probs=pvals, **kwargs)
...
def power(a, size=None, dtype=None, device=None, out=None):
    ...
    b = aikit.ones_like(a)
    return aikit.experimental.beta(a, b, shape=size, dtype=dtype, device=device, out=out)


@to_aikit_arrays_and_back
def rand(*size, **kwargs):
    return aikit.random_uniform(shape=size, **kwargs)


@to_aikit_arrays_and_back
def randint(low, high=None, size=None, dtype=None, device=None, out=None):
    return aikit.randint(low, high, shape=size, device=device, dtype=dtype, out=out)


@to_aikit_arrays_and_back
def shuffle(x, axis=0):
    aikit.shuffle(x, axis)
