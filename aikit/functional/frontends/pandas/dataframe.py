...
class DataFrame(NDFrame):
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(self.orig_data, dict):
            ...
            self.array = aikit.array([array_data for _ in range(len(self.index))])
        elif isinstance(self.orig_data, Series):
            self.array = self.array.expand_dims()
            self.columns = [0]
        elif columns is None:
            self.columns = aikit.arange(self.array.shape[1]).tolist()
        else:
    ...
    def sum(self, axis=None, skipna=True, level=None, numeric_only=None, min_count=0):
        ...
        if axis is None or axis == "index":
            ...
            axis = 1
        if min_count > 0:
            if aikit.has_nans(_array):
                number_values = _array.size - aikit.sum(aikit.isnan(_array))
            else:
                number_values = _array.size
            if min_count > number_values:
                return aikit.nan
        if skipna:
            ret = aikit.nansum(_array, axis=axis)
        else:
    ...
    def mean(self, axis=0, skipna=True, numeric_only=None, **kwargs):
        _array = aikit.astype(self.array, aikit.default_float_dtype())
        axis = 0 if axis == "index" else 1 if axis == "columns" else axis
        if skipna:
            ret = aikit.nanmean(_array, axis=axis)
        else:
