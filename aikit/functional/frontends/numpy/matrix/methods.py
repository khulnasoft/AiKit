...
# local
from aikit.functional.frontends.numpy import (
    argmax,
    any,
    ndarray,
)
...
class matrix:
    ...
    def _init_data(self, data, dtype, copy):
        if isinstance(data, str):
            ...
            if aikit.is_array(data) and dtype is None:
                dtype = data.dtype
            data = aikit.array(data, dtype=dtype, copy=copy)
            self._data = data
    ...
    def _process_str_data(self, data, dtype):
        ...
        if dtype is None:
            if is_complex:
                dtype = aikit.complex128
            else:
                dtype = aikit.float64 if is_float else aikit.int64
        self._data = aikit.array(data, dtype=dtype)
    ...
    @property
    def A1(self):
        return aikit.reshape(self._data, (self.size,))

    # flake8: noqa: E743, E741
    @property
    def I(self):
        if aikit.is_int_dtype(self._data):
            return aikit.inv(self._data.astype(aikit.float64))
        return aikit.inv(self._data)
    ...
    @property
    def data(self):
        return memoryview(aikit.to_numpy(self._data).tobytes())
    ...
    def dtype(self, dtype):
        self._data = aikit.astype(self._data, dtype)
        self._dtype = self._data.dtype
    ...
    def __repr__(self):
        return "aikit.matrix(" + str(self._data.to_list()) + ")"
