import aikit
from aikit.functional.frontends.jax.func_wrapper import to_aikit_arrays_and_back
from aikit.func_wrapper import with_supported_dtypes


# --- Helpers --- #
# --------------- #


def _batch_promotion(*args, default_dtype="float64"):
    # Promote all types
    promote_types = set()

    for arg in args:
        if args is None:
            continue
        if isinstance(arg, (float, int)):
            continue
        promote_types.add(aikit.dtype(arg))

    if "float64" in promote_types:
        return "float64"

    if "float32" in promote_types:
        return "float32"

    if "float16" in promote_types:
        return "float32" if "bfloat16" in promote_types else "float16"

    if "bfloat16" in promote_types:
        return "bfloat16"

    if "int64" in promote_types or "uint64" in promote_types:
        return "float64"

    ints = ["int8", "int16", "int32"]
    if "uint32" in promote_types and any(d in promote_types for d in ints):
        return "float64"

    return default_dtype


def _canonicalize_axis(axis, ndim):
    if not -ndim <= axis < ndim:
        raise aikit.utils.exceptions.AikitException(
            f"axis {axis} is out of bounds for array of dimension {ndim}"
        )
    if axis < 0:
        axis = axis + ndim
    return axis


def _len(x):
    shape = aikit.shape(x)
    return 0 if len(shape) == 0 else shape[0]


def _mean(x, axis=None, keepdims=False, where=None):
    # Mean with support for where
    if where is None:
        return aikit.mean(x, axis=axis, keepdims=keepdims)

    filtered_x = aikit.where(where, aikit.array(x), aikit.zeros_like(x))
    counter_x = aikit.where(where, aikit.ones_like(x), aikit.zeros_like(x))

    sums = aikit.sum(filtered_x, axis=axis, keepdims=keepdims)
    counts = aikit.sum(counter_x, axis=axis, keepdims=keepdims)

    return aikit.divide(sums, counts)


def _reduction_dims(a, axis):
    ndims = len(aikit.shape(a))
    if axis is None:
        return (tuple(range(ndims)),) * 2
    if not isinstance(axis, (tuple, list)):
        axis = (axis,)
    canon_axis = tuple(_canonicalize_axis(ax, ndims) for ax in axis)
    aikit.utils.assertions.check_equal(
        len(canon_axis),
        len(set(canon_axis)),
        message=f"duplicate value in 'axis': {axis}",
        as_array=False,
    )

    # TODO: deal with named axis

    canon_pos_axis = tuple(x for x in canon_axis if isinstance(x, int))

    if len(canon_pos_axis) != len(canon_axis):
        return canon_pos_axis, canon_axis
    else:
        return canon_axis, canon_axis


def _type_conversion(x):
    # Does type conversion, floats maps to float,
    # complex maps to complex,
    # 64bit dtype to float64, everything else to float32
    x = aikit.asarray(x)
    dtype = aikit.as_aikit_dtype(x.dtype)
    if not ("float" in dtype or "complex" in dtype):
        dtype = "float64" if "64" in dtype[-2:] else "float32"
    return aikit.astype(x, dtype)


def _type_conversion_64(x):
    # Does type conversion, floats maps to float,
    # complex maps to complex, everything else to float64
    x = aikit.asarray(x)
    dtype = aikit.as_aikit_dtype(x.dtype)
    if not ("float" in dtype or "complex" in dtype):
        dtype = "float64"
    return aikit.astype(x, dtype)


# --- Main --- #
# ------------ #


@to_aikit_arrays_and_back
def celu(x, alpha=1.0):
    return aikit.celu(x, alpha=alpha)


@to_aikit_arrays_and_back
def elu(x, alpha=1.0):
    ret = aikit.where(x > 0, x, alpha * aikit.expm1(x))
    dtype = _batch_promotion(x, alpha, default_dtype="float64")
    return aikit.asarray(ret, dtype=dtype)


@to_aikit_arrays_and_back
def gelu(x, approximate=True):
    return aikit.gelu(x, approximate=approximate, complex_mode="jax")


@to_aikit_arrays_and_back
def glu(x, axis=-1):
    size = x.shape[axis]
    aikit.utils.assertions.check_equal(
        size % 2, 0, message="axis size must be divisible by 2", as_array=False
    )
    x1, x2 = aikit.split(x, num_or_size_splits=2, axis=axis)
    return aikit.multiply(x1, aikit.sigmoid(x2))


@to_aikit_arrays_and_back
def hard_sigmoid(x):
    dtype = _batch_promotion(x, default_dtype="float64")
    return aikit.divide(aikit.minimum(aikit.maximum(aikit.add(x, 3), 0), 6), 6).astype(dtype)


@to_aikit_arrays_and_back
def hard_silu(x):
    dtype = _batch_promotion(x, default_dtype="float64")
    sig = aikit.divide(aikit.minimum(aikit.maximum(aikit.add(x, 3), 0), 6), 6)
    return aikit.multiply(x, sig).astype(dtype)


@to_aikit_arrays_and_back
def hard_swish(x):
    res = (x * aikit.minimum(aikit.maximum(x + 3, 0.0), 6.0)) / 6
    return aikit.asarray(res, dtype=x.dtype)


@to_aikit_arrays_and_back
def hard_tanh(x):
    n1 = -1
    if "uint" in str(x.dtype):
        dtype = x.dtype
        # tensorflow can't use -1 for uint
        n1 = aikit.asarray((1 << aikit.dtype_bits(dtype)) - 1, dtype=dtype)

    return aikit.where(x > 1, 1, aikit.where(x < n1, n1, x)).astype(x.dtype)


@to_aikit_arrays_and_back
def leaky_relu(x, negative_slope=0.01):
    x = _type_conversion_64(x)
    return aikit.leaky_relu(x, alpha=negative_slope, complex_mode="jax")


@to_aikit_arrays_and_back
def log_sigmoid(x):
    x = _type_conversion(x)
    return aikit.logsigmoid(x, complex_mode="jax").astype(x.dtype)


@to_aikit_arrays_and_back
def log_softmax(x, axis=-1):
    return aikit.log_softmax(x, axis=axis)


@to_aikit_arrays_and_back
def logsumexp(a, axis=None, b=None, keepdims=False, return_sign=False):
    a = aikit.asarray(a)
    if b is not None:
        dtype = _batch_promotion(a, b, default_dtype="float32")
        a = aikit.astype(a, dtype)
        b = aikit.asarray(b, dtype=dtype)
        a = aikit.where(b != 0, a, -aikit.inf)
        a = aikit.astype(a, dtype)
    out_dtype = _batch_promotion(a, b, default_dtype="float32")
    pos_dims, dims = _reduction_dims(a, axis)

    amax = aikit.max(a, axis=pos_dims, keepdims=keepdims)
    notinf = aikit.asarray(not aikit.isinf(amax))
    amax = aikit.stop_gradient(aikit.where(notinf, amax, aikit.zeros_like(amax)))
    amax_with_dims = amax if keepdims else aikit.expand_dims(amax, axis=pos_dims)

    # fast path for non-negative result
    if b is None:
        out = aikit.add(
            aikit.log(
                aikit.sum(
                    aikit.exp(aikit.subtract(a, amax_with_dims)),
                    axis=dims,
                    keepdims=keepdims,
                )
            ),
            amax,
        )
        sign = aikit.where(aikit.isnan(out), out, 1.0)
        sign = aikit.where(aikit.isinf(-out), 0.0, sign).astype(out.dtype)
    else:
        expsub = aikit.exp(aikit.subtract(a, amax_with_dims))
        if b is not None:
            expsub = aikit.multiply(expsub, b)
        sumexp = aikit.sum(expsub, axis=dims, keepdims=keepdims)
        sign = aikit.stop_gradient(aikit.sign(sumexp))
        out = aikit.add(aikit.log(aikit.abs(sumexp)), amax)
    if return_sign:
        return out, sign

    if b is not None:
        out = aikit.where(sign < 0, aikit.array(aikit.nan, dtype=out.dtype), out)

    return out.astype(out_dtype)


@to_aikit_arrays_and_back
def normalize(x, axis=-1, mean=None, variance=None, epsilon=1e-5, where=None):
    default = "float64" if mean is not None and variance is not None else "float32"

    x_typed = _type_conversion(x)
    if mean is None:
        mean = _mean(x_typed, axis=axis, keepdims=True, where=where)
    if variance is None:
        variance = _mean(
            aikit.square(x).astype(x_typed.dtype), axis=axis, keepdims=True, where=where
        ) - aikit.square(mean)

    res = (x - mean) / aikit.sqrt(variance + aikit.asarray(epsilon, dtype=x_typed.dtype))

    out_type = _batch_promotion(x, mean, variance, default_dtype=default)

    return aikit.asarray(res, dtype=out_type)


@to_aikit_arrays_and_back
def one_hot(x, num_classes, *, dtype=None, axis=-1):
    dtype = aikit.float64 if dtype is None else aikit.as_aikit_dtype(dtype)
    return aikit.one_hot(x, num_classes, axis=axis, dtype=dtype)


@to_aikit_arrays_and_back
def relu(x):
    return aikit.relu(x, complex_mode="jax")


@to_aikit_arrays_and_back
def relu6(x):
    res = aikit.relu6(x, complex_mode="jax")
    return _type_conversion_64(res)


@to_aikit_arrays_and_back
def selu(x):
    x = _type_conversion_64(x)
    return aikit.selu(x)


@to_aikit_arrays_and_back
def sigmoid(x):
    x = _type_conversion(x)
    ret = aikit.sigmoid(x, complex_mode="jax")
    return aikit.astype(ret, x.dtype)


@with_supported_dtypes(
    {"0.4.21 and below": ("complex", "float")},
    "jax",
)
@to_aikit_arrays_and_back
def silu(x):
    x = _type_conversion(x)
    return aikit.multiply(x, aikit.sigmoid(x))


@to_aikit_arrays_and_back
def soft_sign(x):
    dtype = _type_conversion(x).dtype
    ret = x / (aikit.abs(x) + 1)
    return ret.astype(dtype)


@to_aikit_arrays_and_back
def softmax(x, axis=-1, where=None, initial=None):
    return aikit.softmax(x, axis=axis)


@to_aikit_arrays_and_back
def softplus(x):
    x = _type_conversion(x)
    return aikit.softplus(x, complex_mode="jax").astype(x.dtype)


@to_aikit_arrays_and_back
def swish(x):
    ret = x / (1 + aikit.exp(-x))
    return aikit.asarray(ret, dtype=x.dtype)
