...
def broadcast_arrays(*arrays: JaxArray) -> List[JaxArray]:
    try:
        ...
        raise aikit.utils.exceptions.AikitBroadcastShapeError(e)
...
def finfo(type: Union[jnp.dtype, str, JaxArray, np.ndarray], /) -> Finfo:
    ...
    return Finfo(jnp.finfo(aikit.as_native_dtype(type)))
...
def iinfo(type: Union[jnp.dtype, str, JaxArray, np.ndarray], /) -> np.iinfo:
    if isinstance(type, np.ndarray):
        type = type.dtype.name
    return jnp.iinfo(aikit.as_native_dtype(type))


def result_type(*arrays_and_dtypes: Union[JaxArray, jnp.dtype]) -> aikit.Dtype:
...
def dtype(x: Union[JaxArray, np.ndarray], *, as_native: bool = False) -> aikit.Dtype:
    if as_native:
