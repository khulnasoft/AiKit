...
def divide(
    ...
    *,
    ...
    if aikit.is_float_dtype(x1.dtype) or aikit.is_complex_dtype(x1.dtype):
        ...
        ret = tf.cast(ret, dtype=aikit.default_float_dtype(as_native=True))
    return ret
...
def isfinite(
    ...
    *,
    ...
    if aikit.is_int_dtype(x):
        return tf.ones_like(x, tf.bool)
    elif aikit.is_complex_dtype(x):
...
def isinf(
    ...
    *,
    ...
    if not aikit.is_complex_dtype(x):
        if aikit.is_int_dtype(x):
...
def logaddexp2(
    ...
    *,
    ...
    x1, x2 = promote_types_of_inputs(x1, x2)
    if not aikit.is_float_dtype(x1):
        x1 = tf.cast(x1, aikit.default_float_dtype(as_native=True))
        x2 = tf.cast(x2, aikit.default_float_dtype(as_native=True))
    amax = aikit.maximum(x1, x2)
    delta = x1 - x2
    return aikit.where(
        aikit.isnan(delta),
        x1 + x2,
        amax + aikit.log1p(aikit.exp2(-aikit.abs(delta))) / aikit.log(2.0).astype(amax.dtype),
    )
...
def pow(
    ...
    *,
    ...
    if aikit.is_complex_dtype(x1) and aikit.any(aikit.isinf(x2)):
        ret = tf.experimental.numpy.power(x1, x2)
        return tf.where(
            aikit.isinf(x2), aikit.nan + aikit.nan * 1j if x2 < 0 else -0 * 1j, ret
        )
    if aikit.is_complex_dtype(x2) and aikit.any(x1 == 0):
        ret = tf.experimental.numpy.power(x1, x2)
        return tf.where(x1 == 0, aikit.nan + aikit.nan * 1j, ret)
    x1, x2 = aikit.promote_types_of_inputs(x1, x2)
