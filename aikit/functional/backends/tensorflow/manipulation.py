...
from aikit.functional.aikit.manipulation import _calculate_out_shape
    ...
    if axis is not None:
        try:
            ...
            if "(zero-based) was expected to be" in error.message:
                ...
                for i in xs:
                    highest_dtype = aikit.promote_types(highest_dtype, i.dtype)
                highest_dtype = aikit.as_native_dtype(highest_dtype)
                return tf.concat(
                    [
...
def stack(
    ...
    *,
    ...
    try:
        ...
        raise aikit.utils.exceptions.AikitIndexError(e)
