    ...
    *,
    ...
    if _is_data_not_indices_values_and_shape(
        ...
        aikit.utils.assertions.check_true(
            aikit.is_native_sparse_array(data), message="not a sparse array"
        )
        return data
    ...
    if format == "coo":
        ...
        for i in range(values.shape[0]):
            coordinate = aikit.gather(coo_indices, aikit.array([[i]]))
            coordinate = aikit.reshape(coordinate, (coo_indices.shape[0],))
