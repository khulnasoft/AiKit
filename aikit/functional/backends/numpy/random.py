"""Collection of Numpy random functions, wrapped to fit Aikit syntax and
signature."""
...
def random_uniform(
    *,
    ...
    shape: Optional[Union[aikit.NativeShape, Sequence[int], np.ndarray]] = None,
    dtype: np.dtype,
...
def randint(
    ...
    *,
    shape: Optional[Union[aikit.NativeShape, Sequence[int]]] = None,
    device: Optional[str] = None,
    dtype: Optional[Union[np.dtype, aikit.Dtype]] = None,
    seed: Optional[int] = None,
    out: Optional[np.ndarray] = None,
) -> np.ndarray:
    if not dtype:
