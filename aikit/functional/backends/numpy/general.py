...
# local
import aikit
from aikit.functional.backends.numpy.helpers import _scalar_output_to_0d_array
...
def inplace_decrement(
    x: Union[aikit.Array, np.ndarray], val: Union[aikit.Array, np.ndarray]
) -> aikit.Array:
...
def shape(
    ...
    *,
    ...
    if as_array:
        return aikit.array(np.shape(x), dtype=aikit.default_int_dtype())
    else:
