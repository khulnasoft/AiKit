...
# local
import aikit
from aikit.func_wrapper import with_unsupported_dtypes, _update_torch_views
...
def to_list(x: torch.Tensor, /) -> list:
    if isinstance(x, np.ndarray):
        ...
        if x.dtype is torch.bfloat16:
            default_dtype = aikit.default_float_dtype(as_native=True)
    ...
    *,
    ...
    updates = torch.tensor(
        ...
        dtype=(
            ...
            if aikit.exists(out)
            else aikit.default_dtype(item=updates, as_native=True)
        ),
    )
    ...
    if aikit.exists(shape) and target_given:
        aikit.utils.assertions.check_equal(
            aikit.Shape(out.shape), aikit.Shape(shape), as_array=False
        )
...
def vmap_v_1p13p1_and_below(
    ...
    def _vmap(*args):
        def new_fun(*args):
            return aikit.to_native(func(*args))
