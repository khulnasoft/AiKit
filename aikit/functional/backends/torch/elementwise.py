...
def ceil(x: torch.Tensor, /, *, out: Optional[torch.Tensor] = None) -> torch.Tensor:
    ...
    if "int" in str(x.dtype):
        if aikit.exists(out):
            return aikit.inplace_update(out, x)
        return x
...
def divide(
    ...
    *,
    ...
    if aikit.is_float_dtype(x1.dtype) or aikit.is_complex_dtype(x1.dtype):
        ret = aikit.astype(ret, x1.dtype, copy=False)
    else:
        ret = aikit.astype(ret, aikit.default_float_dtype(as_native=True), copy=False)
    return ret
...
def floor_divide(
    ...
    *,
    ...
    x1, x2 = aikit.promote_types_of_inputs(x1, x2)
    if aikit.exists(out):
        if not aikit.is_float_dtype(out):
...
def pow(
    ...
    *,
    ...
    if aikit.is_complex_dtype(x1) and aikit.any(aikit.isinf(x2)):
        ...
        return torch.where(
            aikit.isinf(x2), torch.nan + torch.nan * 1j if x2 < 0 else -0 * 1j, ret
        )
    x1, x2 = aikit.promote_types_of_inputs(x1, x2)
    if aikit.any(x1 == 0):
        if aikit.is_complex_dtype(x2):
            x2 = torch.broadcast_to(x2, x1.shape)
            ret = torch.pow(x1, x2)
            return torch.where(x1 == 0, torch.nan + torch.nan * 1j, ret)
        elif (
            aikit.any(x2 < 0)
            and aikit.is_int_dtype(x2)
...
def logaddexp2(
    ...
    *,
    ...
    if not aikit.is_float_dtype(x1):
        x1 = x1.type(aikit.default_float_dtype(as_native=True))
        x2 = x2.type(aikit.default_float_dtype(as_native=True))
...
def trunc_divide(
    ...
    *,
    ...
    if aikit.is_float_dtype(x1.dtype):
        ret = ret.to(x1.dtype)
    else:
        ret = ret.to(aikit.default_float_dtype(as_native=True))
    return ret
