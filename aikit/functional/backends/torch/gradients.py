"""Collection of PyTorch gradient functions, wrapped to fit Aikit syntax and
signature."""
...
def variable(x, /):
    if aikit.is_int_dtype(x.dtype):
        x = aikit.astype(x, aikit.default_float_dtype()).to_native()
    if not x.is_leaf:
    ...
    def callback_fn(xs):
        ...
        def autograd_fn(x):
            ...
            grad = (
                grad
                if grad is not None
                else aikit.to_native(aikit.zeros_like(aikit.to_aikit(x)))
            )
...
def jac(func: Callable):
    ...
    def callback_fn(x_in):
        return aikit.to_aikit(
            torch.func.jacfwd(grad_fn)(aikit.to_native(x_in, nested=True)),
            nested=True,
            include_derived=True,
        )
