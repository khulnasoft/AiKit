...
def searchsorted(
    ...
    *,
    ...
    assert aikit.is_int_dtype(ret_dtype), TypeError(
        "only Integer data types are supported for ret_dtype."
    )
    if sorter is not None:
        sorter_dtype = aikit.as_native_dtype(sorter.dtype)
        assert aikit.is_int_dtype(sorter_dtype), TypeError(
    ...
    ret_dtype = aikit.as_native_dtype(ret_dtype)
    func_out = out
    if aikit.exists(out) and out.dtype != ret_dtype:
        func_out = None
