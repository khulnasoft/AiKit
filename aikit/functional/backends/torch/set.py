...
def unique_all(
    ...
    *,
    ...
    aikit_torch = aikit.current_backend()
    values = aikit_torch.gather(values, sort_idx, axis=axis)
    counts = aikit_torch.gather(counts, sort_idx)
    indices = aikit_torch.gather(indices, sort_idx)
    inv_sort_idx = aikit_torch.invert_permutation(sort_idx)
