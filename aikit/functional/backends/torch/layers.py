...
from aikit.functional.aikit.layers import _get_embed_dim, _handle_padding, _deconv_length
...
def multi_head_attention(
    ...
    *,
    ...
    ret = torch.nn.functional.multi_head_attention_forward(
        ...
        use_separate_proj_weight=not aikit.exists(in_proj_weights),
...
multi_head_attention.partial_mixed_handler = (
    lambda *args, scale=None, out_proj_weights=None, is_causal=False, attention_mask=None, return_attention_weights=False, in_proj_weights=None, q_proj_weights=None, k_proj_weights=None, v_proj_weights=None, **kwargs: not aikit.exists(  # noqa: E501
        scale
    )
    and aikit.exists(out_proj_weights)
    and (not is_causal or aikit.exists(attention_mask))
    and (not is_causal or not return_attention_weights)
    and (
        aikit.exists(in_proj_weights)
        or all(aikit.exists(x) for x in [q_proj_weights, k_proj_weights, v_proj_weights])
    )
    and len(
        set(
            _get_embed_dim(
    ...
    if x_dilations_idxs:
        for i in x_dilations_idxs:
            ...
            h = torch.eye(
                new_height,
                dtype=x.dtype,
                device=aikit.as_native_dev(aikit.default_device()),
            )[:: x_dilations[i]]
...
def depthwise_conv2d(
    ...
    *,
    ...
    if data_format == "NHWC":
        x = x.permute(0, 3, 1, 2)
    filters = aikit.squeeze(filters, axis=3).to_native() if filters.ndim == 4 else filters
