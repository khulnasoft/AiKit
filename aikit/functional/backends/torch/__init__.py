...
# Registering aikit.Array as trackable submodule
if hasattr(torch, "_dynamo"):
    torch._dynamo.config.traceable_tensor_subclasses = (aikit.Array,)
...
valid_int_dtypes = {
    "2.1.2 and below": (aikit.int8, aikit.int16, aikit.int32, aikit.int64, aikit.uint8)
}
valid_float_dtypes = {
    "2.1.2 and below": (aikit.bfloat16, aikit.float16, aikit.float32, aikit.float64)
}
valid_uint_dtypes = {"2.1.2 and below": (aikit.uint8,)}
valid_complex_dtypes = {"2.1.2 and below": (aikit.complex64, aikit.complex128)}
...
invalid_numeric_dtypes = {"2.1.2 and below": (aikit.uint16, aikit.uint32, aikit.uint64)}
invalid_int_dtypes = {"2.1.2 and below": (aikit.uint16, aikit.uint32, aikit.uint64)}
invalid_float_dtypes = {"2.1.2 and below": ()}
invalid_uint_dtypes = {"2.1.2 and below": (aikit.uint16, aikit.uint32, aikit.uint64)}
...
def closest_valid_dtype(type=None, /, as_native=False):
    if type is None:
        ...
        type = aikit.as_aikit_dtype(
            {"uint16": aikit.uint8, "uint32": aikit.uint8, "uint64": aikit.uint8}[type]
        )
...
aikit.func_wrapper.globals_getter_func = globals_getter_func
# local sub-modules
