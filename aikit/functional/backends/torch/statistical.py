...
# local
import aikit
from aikit.functional.aikit.statistical import _get_promoted_type_of_operands
...
def _infer_dtype(dtype: torch.dtype) -> torch.dtype:
    ...
    if default_dtype in aikit.valid_dtypes:
        if aikit.dtype_bits(dtype) < aikit.dtype_bits(default_dtype):
            return aikit.as_native_dtype(default_dtype)
...
@with_unsupported_dtypes(
    {"2.1.1 and below": ("int8", "int16", "int32", "int64", "float16")},
    backend_version,
)
def std(
    x: torch.Tensor,
    ...
    *,
    ...
    if size - correction <= 0:
        ...
        ret = aikit.full(ret.shape, float("nan"), dtype=ret.dtype)
        return ret
    ret = torch.mul(
...
# Function does support uint8, but allowing support for unsigned will cause
# the function to break the upcasting rule defined in the Array API Standard
@with_unsupported_dtypes({"2.1.1 and below": ("uint8",)}, backend_version)
def sum(
    x: torch.Tensor,
    /,
    *,
    ...
    dtype = aikit.as_native_dtype(dtype)
    if dtype is None and not aikit.is_bool_dtype(x):
        dtype = x.dtype
    if axis == ():
        return x.type(dtype)
...
def einsum(
    ...
    *operands: torch.Tensor,
    ...
    return aikit.astype(torch.einsum(equation, *operands), dtype, copy=False)
