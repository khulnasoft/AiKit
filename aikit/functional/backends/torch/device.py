...
) -> Union[aikit.Device, torch.device]:
    dv = x.device
    if as_native:
...
def as_native_dev(
    device: Optional[Union[aikit.Device, torch.device]] = None,
    /,
) -> Optional[torch.device]:
    if not isinstance(device, str):
        return device
    if hasattr(torch.backends, "mps") and torch.backends.mps.is_available():
        return torch.device(aikit.Device(device).replace("gpu", "mps"))
    return torch.device(aikit.Device(device).replace("gpu", "cuda"))


def clear_cached_mem_on_dev(device: Union[aikit.Device, torch.device], /) -> None:
