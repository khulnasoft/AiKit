...
def inplace_decrement(
    x: Union[aikit.Array, paddle.Tensor],
    val: Union[aikit.Array, paddle.Tensor],
) -> aikit.Array:
...
def scatter_flat(
    ...
    *,
    ...
    if aikit.exists(size) and aikit.exists(out):
        aikit.utils.assertions.check_equal(out.ndim, 1, as_array=False)
        aikit.utils.assertions.check_equal(out.shape[0], size, as_array=False)
...
def scatter_nd(
    ...
    *,
    ...
    updates = paddle.to_tensor(
        updates,
        dtype=(
            aikit.promote_types(out.dtype, updates.dtype)
            if aikit.exists(out)
    ...
    if indices.ndim > 1 and reduction != "sum":
        ...
        indices, unique_idxs = aikit.unique_all(
            aikit.flip(indices, axis=[0]), axis=0, by_value=True
        )[:2]
        ...
        if len(unique_idxs) < num_indices:
            ...
            updates = aikit.gather(aikit.flip(updates, axis=[0]), unique_idxs, axis=0).data
            expected_shape = (
                list(indices.shape[:-1]) + list(out.shape[indices.shape[-1] :])
                if aikit.exists(out)
                else list(indices.shape[:-1]) + list(shape[indices.shape[-1] :])
    ...
    # implementation
    target_given = aikit.exists(out)
    if target_given:
        target = out.data
    else:
        shape = list(shape) if aikit.exists(shape) else out.shape
    ...
    if reduction == "min":
        updates = aikit.minimum(aikit.gather_nd(target, indices), updates).data
    elif reduction == "max":
        updates = aikit.maximum(aikit.gather_nd(target, indices), updates).data
    elif reduction == "sum":
        updates = aikit.add(aikit.gather_nd(target, indices), updates).data
    if indices.ndim <= 1:
        indices = aikit.expand_dims(indices, axis=0).data
        updates = aikit.expand_dims(updates, axis=0).data
    updates_ = _broadcast_to(aikit.gather_nd(target, indices), expected_shape).data
...
def shape(
    ...
    if as_array:
        return aikit.array(x.shape, dtype=aikit.default_int_dtype())
    else:
