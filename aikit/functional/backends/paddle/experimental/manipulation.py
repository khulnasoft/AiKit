...
from aikit.functional.aikit.experimental.manipulation import (
    _check_paddle_pad,
    _to_paddle_padding,
)
...
def vstack(
    ...
    *,
    ...
    with aikit.ArrayMode(False):
        ...
        return aikit.concat(arrays, axis=0)
...
def hstack(
    ...
    *,
    ...
    with aikit.ArrayMode(False):
        if arrays[0].ndim >= 2:
            return aikit.concat(arrays, axis=1)
        else:
            return aikit.concat(arrays, axis=0)
...
def flatten(
    ...
    *,
    ...
    if order == "F":
        with aikit.ArrayMode(False):
            x = aikit.permute_dims(x, list(reversed(range(x.ndim))))
            ret = _flatten(x, start_dim, end_dim)
            return aikit.permute_dims(ret, list(reversed(range(ret.ndim))))
...
def atleast_1d(
    *arys: paddle.Tensor, copy: Optional[bool] = None
    ...
    res = []
    for ary in arys:
        ary = aikit.array(ary, copy=copy).data
        if ary.ndim < 1:
            with aikit.ArrayMode(False):
                res.append(aikit.expand_dims(ary, axis=0))
        else:
            res.append(ary)
    if len(res) == 1:
        return res[0]
...
def dstack(
    ...
    *,
    ...
    with aikit.ArrayMode(False):
        arrays = aikit.atleast_2d(*arrays)
        if not isinstance(arrays, list):
            arrays = [arrays]
        if arrays[0].ndim < 3:
            return aikit.stack(arrays, axis=-1)
        else:
            return aikit.concat(arrays, axis=2)
...
def atleast_2d(
    *arys: paddle.Tensor, copy: Optional[bool] = None
    ...
    for ary in arys:
        ...
        if ary.ndim < 2:
            with aikit.ArrayMode(False):
                res.append(aikit.expand_dims(ary, axis=list(range(2 - ary.ndim))))
        else:
            res.append(ary)
    if len(res) == 1:
        return res[0]
...
def take_along_axis(
    ...
    *,
    ...
    if mode == "clip":
        ...
        with aikit.ArrayMode(False):
            indices = aikit.clip(indices, 0, max_index)
        ...
        with aikit.ArrayMode(False):
            ...
            arr_shape[axis] = 1
            fill_arr = aikit.full(arr_shape, fill_value, dtype=arr.dtype)
            arr = aikit.concat([arr, fill_arr], axis=axis)
            indices = aikit.where(indices < 0, arr.shape[axis] + indices, indices)
...
def concat_from_sequence(
    ...
    *,
    ...
    with aikit.ArrayMode(False):
        if new_axis == 0:
            return aikit.concat(input_sequence, axis=axis)
        elif new_axis == 1:
            return aikit.stack(input_sequence, axis=axis)
...
def unique_consecutive(
    ...
    *,
    ...
    output = paddle.concat(
        [
            aikit.current_backend().unique_all(sub_array, axis=axis)[0]
...
def _take_with_axis(
    ...
    ret = aikit.array(x)[tuple(slicer)]
