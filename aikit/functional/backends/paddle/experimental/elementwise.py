...
from aikit.functional.backends.paddle.elementwise import _elementwise_helper
...
def sinc(x: paddle.Tensor, /, *, out: Optional[paddle.Tensor] = None) -> paddle.Tensor:
    y = aikit.pi * paddle.where(x == 0, paddle.to_tensor(1.0e-20, dtype=x.dtype), x)
...
def ldexp(
    ...
    *,
    ...
    with aikit.ArrayMode(False):
        if aikit.any(aikit.less(x2, 0)):
            pos_exp = aikit.greater_equal(x2, 0).astype(x2.dtype) * x2
            neg_exp = aikit.less(x2, 0).astype(x2.dtype) * x2
            ret = aikit.multiply(aikit.pow(2, pos_exp), x1)
            ret = aikit.divide(ret, aikit.pow(2, -neg_exp))
        else:
            ret = aikit.multiply(aikit.pow(2, x2), x1)
        return aikit.astype(ret, out_dtype, copy=False)
...
def nextafter(
    ...
    *,
    ...
    with aikit.ArrayMode(False):
        eps = aikit.finfo(x1.dtype).eps
        return aikit.where(
            aikit.equal(x1, x2),
            x2,
            aikit.where(aikit.greater(x2, x1), aikit.add(x1, eps), aikit.subtract(x1, eps)),
        )
...
def zeta(
    ...
    *,
    ...
    with aikit.ArrayMode(False):
        s, a = aikit.promote_types_of_inputs(x, q)
        ...
        k = paddle.unsqueeze(aikit.arange(N, dtype=q.dtype), tuple(range(q.ndim)))
        S = paddle.sum((a_ + k) ** -s_, -1)
        Q = aikit.divide((q + N) ** (1 - x), x - 1)
        T0 = (q + N) ** -x
        m = paddle.unsqueeze(aikit.arange(2 * M, dtype=s.dtype), tuple(range(s.ndim)))
        s_over_a = (s_ + m) / (a_ + N)
        s_over_a = aikit.where(
            s_over_a == 0, paddle.ones_like(s_over_a) * 1e-20, s_over_a
        )
        T1 = paddle.cumprod(s_over_a, -1)[..., ::2]
        # t=np.array(T1)
        T1 = paddle.clip(T1, max=aikit.finfo(T1.dtype).max)
...
def xlogy(
    ...
    with aikit.ArrayMode(False):
        x_ok = aikit.not_equal(x, 0.0)
        safe_x = aikit.where(x_ok, x, 1.0)
        safe_y = aikit.where(x_ok, y, 1.0)
        return aikit.where(
            x_ok, aikit.multiply(safe_x, aikit.log(safe_y)), aikit.zeros_like(x)
        ).cast(ret_dtype)
...
def count_nonzero(
    ...
    *,
    ...
    with aikit.ArrayMode(False):
        return aikit.sum(aikit.not_equal(a, 0), axis=axis, keepdims=keepdims, dtype=dtype)
