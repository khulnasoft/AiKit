...
def cummax(
    ...
    *,
    ...
    if not (exclusive or reverse):
        ...
        x, indices = __find_cummax(aikit.flip(x, axis=(axis,)), axis=axis)
        x, indices = aikit.swapaxes(x, axis, -1), aikit.swapaxes(indices, axis, -1)
        x = aikit.concat((aikit.zeros_like(x[..., -1:]), x[..., :-1]), axis=-1)
        indices = aikit.concat(
            (aikit.zeros_like(indices[..., -1:]), indices[..., :-1]), axis=-1
        )
        x, indices = aikit.swapaxes(x, axis, -1), aikit.swapaxes(indices, axis, -1)
        return aikit.flip(x, axis=(axis,)), aikit.flip(indices, axis=(axis,))
...
def __find_cummax(
    ...
    if (
        ...
        and len(x[0].shape) >= 1
        and (isinstance(x[0], (paddle.Tensor, aikit.Array)))
    ):
        if axis >= 1:
    ...
    if isinstance(x, paddle.Tensor):
        ...
        return aikit.array(values, dtype=x_dtype), aikit.array(indices, dtype="int64")
