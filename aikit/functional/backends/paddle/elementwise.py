...
def positive(
    ...
    if not isinstance(x, paddle.Tensor):
        x = paddle.to_tensor(
            x, dtype=aikit.default_dtype(item=x, as_native=True)
        ).squeeze()
    return x.clone()
...
def trapz(
    ...
    *,
    ...
    with aikit.ArrayMode(False):
        if y.shape[axis] < 2:
            return aikit.zeros_like(aikit.squeeze(y, axis=axis))
        ret = aikit.sum(
            aikit.divide(
                aikit.multiply(
                    d,
                    aikit.add(
                        aikit.get_item(y, tuple(slice1)), aikit.get_item(y, tuple(slice2))
                    ),
                ),
                2.0,
            ),
...
def abs(
    ...
    *,
    ...
    if not isinstance(x, paddle.Tensor):
        x = paddle.to_tensor(x, dtype=aikit.default_dtype(item=x)).squeeze()
    return paddle.abs(x)
...
def logaddexp2(
    ...
    *,
    ...
    with aikit.ArrayMode(False):
        return aikit.log2(aikit.exp2(x1) + aikit.exp2(x2))
    ...
    *,
    ...
    with aikit.ArrayMode(False):
        if aikit.is_int_dtype(x):
            if posinf is None:
                posinf = aikit.iinfo(x).max
            if neginf is None:
                neginf = aikit.iinfo(x).min
        elif aikit.is_float_dtype(x) or aikit.is_complex_dtype(x):
            if posinf is None:
                posinf = aikit.finfo(x).max
            if neginf is None:
                neginf = aikit.finfo(x).min
        ret = aikit.where(aikit.isnan(x), paddle.to_tensor(nan, dtype=x.dtype), x)
        ret = aikit.where(
            aikit.logical_and(aikit.isinf(ret), ret > 0),
