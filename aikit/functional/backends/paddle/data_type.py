...
def finfo(type: Union[paddle.dtype, str, paddle.Tensor], /) -> Finfo:
    ...
    if aikit.as_native_dtype(type) == paddle.bfloat16:
...
def result_type(*arrays_and_dtypes: Union[paddle.Tensor, paddle.dtype]) -> aikit.Dtype:
    return aikit.promote_types_of_inputs(*arrays_and_dtypes)[0].dtype
...
def as_aikit_dtype(dtype_in: Union[paddle.dtype, str, bool, int, float], /) -> aikit.Dtype:
    if dtype_in is int:
    ...
    return aikit.Dtype(aikit_dtype_dict[dtype_in])
...
def dtype(x: paddle.Tensor, *, as_native: bool = False) -> aikit.Dtype:
    if as_native:
...
def is_native_dtype(dtype_in: Union[paddle.dtype, str], /) -> bool:
    if not aikit.is_hashable_dtype(dtype_in):
        return False
    if dtype_in in aikit_dtype_dict:
        return True
    else:
        return False
