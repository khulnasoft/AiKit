...
def arange(
    ...
    step: float = 1,
    *,
    dtype: Optional[Union[aikit.Dtype, paddle.dtype]] = None,
    ...
    *,
    ...
    if isinstance(obj, paddle.Tensor):
        ...
        if ret.ndim != 0:  # for versions <2.5.0
            ...
            return ret
    obj = aikit.nested_map(_remove_np_bfloat16, obj, shallow=False)
...
def full(
    ...
    *,
    ...
    if dtype is None:
        dtype = aikit.default_dtype(item=fill_value)
    ...
    if isinstance(fill_value, complex):
        ...
        dtype_ = None if aikit.as_native_dtype(dtype) == paddle.int8 else dtype
        ret = paddle.full(shape=shape, fill_value=fill_value, dtype=dtype_)
    if ret.dtype != aikit.as_native_dtype(dtype):
...
def copy_array(
    x: paddle.Tensor,
    *,
    to_aikit_array: Optional[bool] = True,
    out: Optional[paddle.Tensor] = None,
) -> paddle.Tensor:
    if 0 in x.shape:
...
def frombuffer(
    ...
    dtype_bytes = int(aikit.Dtype(dtype).dtype_bits / 8)
    if str(dtype) == "bool":
        dtype_bytes = 1
    dtype_str = str(dtype)
    struct_format = {
