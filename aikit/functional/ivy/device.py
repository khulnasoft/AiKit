...
class DefaultDevice:
    ...
    def __init__(
        self,
        device: Union[aikit.Device, aikit.NativeDevice],
        /,
    ) -> None:
        ...
        ----------
        device
            The device string - as an aikit device or nativedevice class

        Examples
        --------
        A "tpu" as device:

        >>> x = aikit.DefaultDevice("tpu")
        """
        self._dev = device
    ...
    def __enter__(self):
        ...
        >>> with aikit.DefaultDevice("cpu") as device:
        >>>     # with block calls device.__enter__()
        >>>     print(device._dev)
        "cpu"
        """
        aikit.set_default_device(self._dev)
    ...
    def __exit__(
        ...
        >>> with aikit.DefaultDevice("gpu") as device:
        >>>     pass
...
def handle_soft_device_variable(*args, fn, **kwargs):
    return aikit.current_backend().handle_soft_device_variable(*args, fn=fn, **kwargs)


# Helpers #


def _get_nvml_gpu_handle(device: Union[aikit.Device, aikit.NativeDevice], /) -> int:
    global dev_handles
...
def _shift_native_arrays_on_default_device(*args, **kwargs):
    with aikit.ArrayMode(False):
        default_device = aikit.default_device()
        args, kwargs = aikit.nested_map(
            lambda x: (
                aikit.to_device(x, default_device)
                if (aikit.is_native_array(x) and aikit.dev(x) != default_device)
                else x
            ),
            [args, kwargs],
        )
    return args, kwargs, aikit.as_native_dev(default_device)
...
@handle_exceptions
def get_all_aikit_arrays_on_dev(
    device: Union[aikit.Device, aikit.NativeDevice],
    /,
) -> aikit.Container:
    """Get all aikit arrays which are currently alive on the specified device.
    ...
    >>> x = aikit.array([1,0,2])
    >>> y = aikit.dev(x)
    >>> z = aikit.get_all_aikit_arrays_on_dev(y)
    >>> print(z)
    {139740789224448:aikit.array([1,0,2])},
    """
    device = aikit.as_aikit_dev(device)
    all_arrays = []
    for obj in gc.get_objects():
        if (
            obj is aikit.data_classes.array.array.Array
            and aikit.is_aikit_array(obj)
            and aikit.dev(obj) == device
        ):
            all_arrays.append(obj)

    return aikit.Container(dict(zip([str(id(a)) for a in all_arrays], all_arrays)))


@handle_exceptions
def num_aikit_arrays_on_dev(device: Union[aikit.Device, aikit.NativeDevice], /) -> int:
    ...
    Examples
    --------
    >>> x1 = aikit.array([-1, 0, 5.2])
    >>> x2 = aikit.array([-1, 0, 5.2, 4, 5])
    >>> y = aikit.num_aikit_arrays_on_dev(aikit.default_device())
    >>> print(y)
    2

    >>> x1 = aikit.native_array([-1, 0, 5.2])
    >>> y = aikit.num_aikit_arrays_on_dev(aikit.default_device())
    >>> print(y)
    0

    >>> x = aikit.Container(x1=aikit.array([-1]),
    ...                   x2=aikit.native_array([-1]))
    >>> y = aikit.num_aikit_arrays_on_dev(aikit.default_device())
    >>> print(y)
    1
    """
    return len(aikit.get_all_aikit_arrays_on_dev(device))


@handle_exceptions
@handle_nestable
def print_all_aikit_arrays_on_dev(
    *,
    device: Optional[Union[aikit.Device, aikit.NativeDevice]] = None,
    attr_only: bool = True,
) -> None:
    """Print the shape and dtype for all aikit arrays which are currently alive
    ...
    Examples
    --------
    >>> x = aikit.array([[1,0,2], [3,2,1]])
    >>> y = aikit.dev(x)
    >>> aikit.print_all_aikit_arrays_on_dev(y)
    ((3,), 'int32')
    ((3,), 'int32')
    ...
    >>> x = aikit.array([[1,0,2], [3,2,1]])
    >>> y = aikit.dev(x)
    >>> aikit.print_all_aikit_arrays_on_dev(y, attr_only = False)
    [1,0,2]
    [3,2,1]
    """
    arrs = aikit.get_all_aikit_arrays_on_dev(device).values()
    if attr_only:
...
aikit.soft_device_mode = soft_device_mode_stack[-1] if soft_device_mode_stack else False


@handle_exceptions
def set_soft_device_mode(mode: bool) -> None:
    """Set the mode of whether to move input arrays to `aikit.default_device()`
    ...
    aikit.__setattr__("soft_device_mode", mode, True)


@handle_exceptions
def unset_soft_device_mode() -> None:
    """Reset the mode of moving input arrays to `aikit.default_device()` before
...
def dev(
...
) -> Union[aikit.Device, aikit.NativeDevice]:
    ...
    >>> x = aikit.native_array([[2, 5, 4], [3, 1, 5]])
    >>> y = aikit.dev(x, as_native=True)
    >>> print(y)
    cpu
    """
    return aikit.current_backend(x).dev(x, as_native=as_native)
...
@handle_exceptions
def as_aikit_dev(device: Union[aikit.Device, str], /) -> aikit.Device:
    ...
    Examples
    --------
    >>> y = aikit.as_aikit_dev('cpu')
    >>> print(y)
    cpu
    """
    return aikit.current_backend().as_aikit_dev(device)


@handle_exceptions
def as_native_dev(device: Union[aikit.Device, aikit.NativeDevice], /) -> aikit.NativeDevice:
    ...
    >>> aikit.as_native_dev("cpu")
    'cpu'

    >>> aikit.set_backend("tensorflow")
    >>> aikit.as_native_dev("tpu:3")
    '/TPU:3'
    ...
    >>> aikit.as_native_dev(device)
    device(type='cuda')
    """
    return aikit.current_backend().as_native_dev(device)
...
@handle_exceptions
def clear_cached_mem_on_dev(device: Union[aikit.Device, aikit.NativeDevice], /) -> None:
    ...
    >>> device = torch.device("cuda")
    >>> aikit.clear_cached_mem_on_dev(device)
    """
    aikit.current_backend().clear_cached_mem_on_dev(device)


@handle_exceptions
def total_mem_on_dev(device: Union[aikit.Device, aikit.NativeDevice], /) -> float:
    ...
    Examples
    --------
    >>> x = aikit.total_mem_on_dev("cpu")
    >>> print(x)
    53.66700032

    >>> x = aikit.total_mem_on_dev("gpu:0")
    >>> print(x)
    8.589934592
    """
    if "gpu" in device:
...
def used_mem_on_dev(
    ...
    *,
    ...
    Examples
    --------
    >>> x = aikit.used_mem_on_dev("cpu", process_specific = False)
    >>> print(x)
    6.219563008

    >>> x = aikit.used_mem_on_dev("cpu", process_specific = True)
    >>> print(x)
    0.902400346

    >>> y = aikit.used_mem_on_dev("gpu:0", process_specific = False)
    >>> print(y)
    0.525205504
    """
    aikit.clear_cached_mem_on_dev(device)
...
def percent_used_mem_on_dev(
    ...
    *,
    ...
    Examples
    --------
    >>> x = aikit.percent_used_mem_on_dev("cpu", process_specific = False)
    >>> print(x)
    94.036902561555

    >>> x = aikit.percent_used_mem_on_dev("cpu", process_specific = True)
    >>> print(x)
    0.7024003467681645

    >>> x = aikit.as_native_dev("gpu:0")
    >>> y = aikit.percent_used_mem_on_dev(x, process_specific = False)
    >>> print(y)
    0.7095597456708771
    """
    aikit.clear_cached_mem_on_dev(device)
...
def dev_util(
    ...
    >>> aikit.dev_util('cpu')
    13.4
    >>> aikit.dev_util('gpu:0')
    7.8
    >>> aikit.dev_util('cpu')
    93.4
    >>> aikit.dev_util('gpu:2')
    57.4
    >>> aikit.dev_util('cpu')
    84.2
    """
...
def gpu_is_available() -> bool:
    ...
    Examples
    --------
    >>> print(aikit.gpu_is_available())
    False
    """
    return aikit.current_backend().gpu_is_available()
...
def num_cpu_cores(*, logical: bool = True) -> int:
    ...
    Examples
    --------
    >>> print(aikit.num_cpu_cores(logical=False))
    2
    """
    if logical:
...
def num_gpus() -> int:
    ...
    Examples
    --------
    >>> print(aikit.num_gpus())
    1
    """
    return aikit.current_backend().num_gpus()
...
def tpu_is_available() -> bool:
    ...
    --------
    >>> aikit.set_backend("torch")
    >>> print(aikit.tpu_is_available())
    True
    """
    return aikit.current_backend().tpu_is_available()
...
def default_device(
    ...
    /,
    *,
    item: Optional[Union[list, tuple, dict, aikit.Array, aikit.NativeArray]] = None,
    ...
    Examples
    --------
    >>> aikit.default_device()
    device(type='cpu')

    >>> aikit.default_device("gpu:0")
    'gpu:0'

    >>> aikit.default_device(item=[], as_native=False)
    'cpu'

    >>> aikit.default_device(item=(), as_native=True)
    device(type='cpu')

    >>> aikit.default_device(item={"a": 1}, as_native=True)
    device(type='cpu')

    >>> x = aikit.array([1., 2., 3.])
    >>> x = aikit.to_device(x, 'gpu:0')
    >>> aikit.default_device(item=x, as_native=True)
    device(type='gpu', id=0)
    """
    if aikit.exists(device):
        if as_native is True:
            return aikit.as_native_dev(device)
        elif as_native is False:
            return aikit.as_aikit_dev(device)
        return device
    as_native = aikit.default(as_native, False)
    if aikit.exists(item):
        if isinstance(item, (list, tuple, dict)) and len(item) == 0:
            pass
        elif aikit.is_array(item):
            return aikit.dev(item, as_native=as_native)
    ...
    if as_native:
        return aikit.as_native_dev(ret)
    return aikit.as_aikit_dev(ret)


@handle_exceptions
def set_default_device(device: Union[aikit.Device, aikit.NativeDevice], /) -> None:
    ...
    >>> aikit.set_backend("torch")
    >>> aikit.set_default_device("gpu:0")
    >>> aikit.default_device(as_native=True)
    device(type='cuda', index=0)

    >>> import torch
    >>> aikit.set_backend("torch")
    >>> device = torch.device("cuda")
    >>> aikit.set_default_device(device)
    >>> aikit.default_device(as_native=True)
    device(type='cuda')
    """
    global default_device_stack
...
def to_device(
    ...
    *,
    ...
    cpu
    """
    return aikit.current_backend(x).to_device(x, device, stream=stream, out=out)
...
def split_factor(
    ...
    Examples
    --------
    >>> x = aikit.split_factor()
    >>> print(x)
    0.0

    >>> y = aikit.split_factor("gpu:0")
    >>> print(y)
    0.0
    """
...
def set_split_factor(
    factor: float, /, *, device: Optional[Union[aikit.Device, aikit.NativeDevice]] = None
) -> None:
    ...
    Examples
    --------
    >>> print(aikit.default_device())
    cpu

    >>> aikit.set_split_factor(0.5)
    >>> print(aikit.split_factors)
    {'cpu': 0.5}

    >>> import torch
    >>> aikit.set_backend("torch")
    >>> device = torch.device("cuda")
    >>> aikit.set_split_factor(0.3, device=device)
    >>> print(aikit.split_factors)
    {device(type='cuda'): 0.3}

    >>> aikit.set_split_factor(0.4, device="tpu")
    >>> print(aikit.split_factors)
    {'tpu': 0.4}
    ...
    >>> aikit.set_backend("torch")
    >>> device = torch.device("cuda")
    >>> aikit.set_split_factor(0.2)
    >>> aikit.set_split_factor(0.3, device='gpu')
    >>> print(aikit.split_factors)
    {'cpu': 0.2, 'gpu': 0.3}
    """
    aikit.utils.assertions.check_less(0, factor, allow_equal=True, as_array=False)
    global split_factors
    device = aikit.default(device, default_device())
...
def split_func_call(
    func: Callable,
    inputs: Union[aikit.Array, aikit.NativeArray],
    mode: str,
    /,
    *,
    ...
    if not aikit.exists(max_chunk_size) and not aikit.exists(chunk_size):
    ...
    chunk_size = aikit.default(
        chunk_size,
        default_val=lambda: 1
        + int(
            round((max_chunk_size - 1) * aikit.split_factor(aikit.default_device(device)))
        ),
        with_callable=True,
    )
    ...
    is_mean = mode == "mean"
    is_sum = mode == "sum"
    post_fn = aikit.stop_gradient if stop_gradients else lambda x: x
    ...
    ret = [
        aikit.concat([r[i] for r in rets], axis=output_axes[i])
...
def _get_devices(fn: Callable, complement: bool = True) -> Tuple:
    valid_devices = aikit.valid_devices
    invalid_devices = aikit.invalid_devices
    all_devices = aikit.all_devices

    supported = set(aikit.valid_devices)
    ...
    for key, merge_fn, base in basic:
        if hasattr(fn, key):
            ...
            if "einops" in fn.__name__ and isinstance(v, dict):
                v = v.get(aikit.current_backend_str(), base)
            aikit.utils.assertions.check_isinstance(v, tuple)
...
def function_supported_devices(
    ...
    --------
    >>> import aikit
    >>> print(aikit.function_supported_devices(aikit.ones))
    ('cpu', 'gpu')
    """
    ...
    if hasattr(fn, "partial_mixed_handler"):
        ...
        if recurse:
            supported_devices = aikit.functional.data_type._nested_get(
...
def function_unsupported_devices(
    ...
    Examples
    --------
    >>> print(aikit.function_unsupported_devices(aikit.ones))
    ('tpu',)
    """
    aikit.utils.assertions.check_true(
        _is_valid_devices_attributes(fn),
    ...
    if hasattr(fn, "partial_mixed_handler"):
        ...
        if recurse:
            unsupported_devices = aikit.functional.data_type._nested_get(
