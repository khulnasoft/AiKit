...
def hinge_embedding_loss(
    ...
    *,
    ...
    --------
    >>> input_tensor = aikit.array([1, 2, 3, 4], dtype=aikit.float64)
    >>> target_tensor = aikit.array([1, 1, 1, 1], dtype=aikit.float64)
    >>> loss = aikit.hinge_embedding_loss(input_tensor, target_tensor, reduction="none")
    >>> loss
    aikit.array([1., 2., 3., 4.])

    >>> input_tensor = aikit.array([21, 22], dtype=aikit.float32)
    >>> target_tensor = aikit.array([-1, 1], dtype=aikit.float32)
    >>> loss = aikit.hinge_embedding_loss(input_tensor,target_tensor,
    ...                                 margin=2.0, reduction="sum")
    >>> loss
    aikit.array(22.)
    """
    return aikit.current_backend().hinge_embedding_loss(
        input,
        target,
        margin=margin,
        reduction=reduction,
