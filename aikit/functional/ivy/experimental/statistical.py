...
def histogram(
    ...
    /,
    *,
    bins: Optional[Union[int, aikit.Array, aikit.NativeArray]] = None,
    ...
    >>> x = aikit.array([0, 1, 2])
    >>> y = aikit.array([0., 0.5, 1., 1.5, 2.])
    >>> z = aikit.histogram(x, bins=y)
    >>> print(z)
    aikit.array([1., 0., 1., 1.])
    ...
    >>> range = (0., 5.)
    >>> dtype = aikit.int32
    >>> y = aikit.histogram(x, bins=bins, range=range, dtype=dtype)
    >>> print(y)
    aikit.array([2, 1, 1, 1])
    ...
    >>> dtype = aikit.float32
    >>> weights = aikit.array([[1., 1., 1.], [1., 1., 1.]])
    >>> z = aikit.histogram(
    ...                     x,
    ...                     bins=y,
    ...                     axis=axis,
    ...
    >>> y = aikit.array([0., 1., 2., 3., 4., 5.])
    >>> dtype = aikit.int32
    >>> z = aikit.histogram(x, bins=y, dtype=dtype)
    >>> print(z)
    {
    ...
    """
    return aikit.current_backend(a).histogram(
        a,
        bins=bins,
        axis=axis,
        extend_lower_interval=extend_lower_interval,
...
def median(
    ...
    *,
    ...
    >>> aikit.median(a)
    3.5
    >>> aikit.median(a, axis=0)
    aikit.array([6.5, 4.5, 2.5])
    """
    return aikit.current_backend().median(input, axis=axis, keepdims=keepdims, out=out)
...
def nanmean(
    ...
    *,
    ...
    Examples
    --------
    >>> a = aikit.array([[1, aikit.nan], [3, 4]])
    >>> aikit.nanmean(a)
    2.6666666666666665
    >>> aikit.nanmean(a, axis=0)
    aikit.array([2.,  4.])
    """
    return aikit.current_backend(a).nanmean(
...
def nanmin(
    ...
    *,
    ...
    >>> aikit.nanmin(a)
    1.0
    >>> aikit.nanmin(a, axis=1)
    [1. 3.]
    >>> aikit.nanmin(a, axis=0, keepdims=True)
    [[1. 2.]]
    """
    return aikit.current_backend(x).nanmin(
        x,
        axis=axis,
        keepdims=keepdims,
        out=out,
...
def nanprod(
    ...
    *,
    ...
    >>> aikit.nanprod(a)
    12.0
    >>> aikit.nanprod(a, axis=0)
    [3. 4.]
    >>> aikit.nanprod(a, axis=0, keepdims=True)
    [[3. 4.]]
    """
    return aikit.current_backend(a).nanprod(
        a,
        axis=axis,
        keepdims=keepdims,
        dtype=dtype,
...
def quantile(
    ...
    *,
    ...
    >>> aikit.quantile(a, q, axis=0)
    aikit.array([6.5, 4.5, 2.5])

    >>> aikit.quantile(a, q, axis=1)
    aikit.array([7.,  2.])

    >>> aikit.quantile(a, q, axis=1, keepdims=True)
    aikit.array([[7.],[2.]])

    >>> a = aikit.array([1., 2., 3., 4.])
    >>> q = aikit.array([0.3, 0.7])
    >>> aikit.quantile(a, q, interpolation='lower')
    aikit.array([1., 3.])
    """
    return aikit.current_backend(a).quantile(
...
def corrcoef(
    ...
    /,
    *,
    y: Optional[aikit.Array] = None,
    rowvar: bool = True,
    out: Optional[aikit.Array] = None,
) -> aikit.Array:
    return aikit.current_backend(x).corrcoef(x, y=y, rowvar=rowvar, out=out)
...
def nanmedian(
    ...
    *,
    ...
    """aikit.Array instance method variant of aikit.nanmedian. This method simply
    wraps the function, and so the docstring for aikit.nanmedian also applies to
    ...
    >>> x = aikit.array([[12.0, 10.0, 34.0], [45.0, 23.0, aikit.nan]])
    >>> aikit.nanmedian(x)
        aikit.array(23.)
    ...
    """
    return aikit.current_backend().nanmedian(
...
def bincount(
    ...
    /,
    *,
    weights: Optional[aikit.Array] = None,
    minlength: int = 0,
    out: Optional[aikit.Array] = None,
) -> aikit.Array:
    ...
    >>> a.bincount(a, axis=0)
        array([6.5, 2. , 2.5])
    """
    return aikit.current_backend(x).bincount(
...
def igamma(
    ...
    /,
    *,
    x: Optional[Union[aikit.Array, aikit.NativeArray]] = None,
    ...
    """
    return aikit.current_backend().igamma(a, x=x, out=out)
...
def cov(
    ...
    x2: Union[aikit.Array, aikit.NativeArray] = None,
    /,
    *,
    rowVar: bool = True,
    bias: bool = False,
    ddof: Optional[int] = None,
    fweights: Optional[aikit.Array] = None,
    aweights: Optional[aikit.Array] = None,
    dtype: Optional[Union[aikit.Dtype, aikit.NativeDtype]] = None,
) -> aikit.Array:
    ...
    With :class:`aikit.Array` input and rowVar flag set to False (True by default):
    >>> x = aikit.array([[1,2,3],
    ...                [4,5,6]])
    ...
    With :class:`aikit.Array` input and bias flag set to True (False by default):
    >>> x = aikit.array([[1,2,3],
    ...                [4,5,6]])
    >>> y = x[0].cov(x[1], bias=True)
    >>> print(y)
    aikit.array([[0.66666667, 0.66666667],
           [0.66666667, 0.66666667]])

    With :class:`aikit.Array` input with both fweights and aweights given:
    >>> x = aikit.array([[1,2,3],
    ...                [4,5,6]])
    >>> fw = aikit.array([1,2,3])
    >>> aw = aikit.array([ 1.2, 2.3, 3.4 ])
    >>> y = x[0].cov(x[1], fweights=fw, aweights=aw)
    >>> print(y)
    aikit.array([[0.48447205, 0.48447205],
           [0.48447205, 0.48447205]])
    """
    return aikit.current_backend(x1).cov(
        x1,
        x2,
        rowVar=rowVar,
        bias=bias,
...
def cummax(
    ...
    *,
    ...
    >>> x = aikit.array([-86, -19, 41, 88, -5, 80, 32, 87, -90, -12])
    >>> y = aikit.cummax(x, exclusive=False, reverse=False)
    >>> print(y)
    (aikit.array([-86, -19,  41,  88,  88,  88,  88,  88,  88,  88]),
    aikit.array([0, 1, 2, 3, 3, 3, 3, 3, 3, 3]))

    >>> x = aikit.array([ 14,  15,  49, -24, -39])
    >>> y = aikit.cummax(x, axis=0, exclusive=False, reverse=False)
    >>> print(y)
    (aikit.array([14, 15, 49, 49, 49]), aikit.array([0, 1, 2, 2, 2]))

    >>> x = aikit.array([[ 63,  43, -16,  -4],[ 21,  82,  59,  33]])
    >>> aikit.cummax(x, axis=0, reverse=False, dtype='int64', out=x)
    >>> print(x)
    aikit.array([[0, 0, 0, 0],
           [0, 1, 1, 1]])

    >>> x = aikit.array([[-36,  83, -81],
    ...                [ 23,  29,  63],
    ...                [-83,  85,   2],
    ...                [ 31,  25, -86],
    ...                [-10, -52,   0],
    ...                [ 22,  38,  55],
    ...                [ 33,  54, -16]])
    >>> y = aikit.cummax(x, axis=1, exclusive=True, reverse=False)
    >>> print(y)
    (aikit.array([[ 0,  0, 83],
           [ 0, 23, 29],
           [ 0,  0, 85],
    ...
    >>> x = aikit.array([73, 15, 47])
    >>> y = aikit.cummax(x, axis=0, reverse=True, exclusive=True)
    >>> print(y)
    (aikit.array([47, 47,  0]), aikit.array([0, 0, 0]))

    >>> x = aikit.array([-47, -14, -67, 15, -23, -45])
    >>> y = aikit.cummax(x, axis=0, reverse=True, exclusive=False)
    >>> print(y)
    (aikit.array([ 15,  15,  15,  15, -23, -45]), aikit.array([2, 2, 2, 2, 1, 0]))
    """
    return aikit.current_backend(x).cummax(
...
def cummin(
    ...
    *,
    ...
    ...                [1, 3, 0]])
    >>> y = aikit.zeros((2,3))
    >>> aikit.cummin(x, axis=0, reverse=True, out=y)
    >>> print(y)
    aikit.array([[1., 3., 0.],
           [1., 3., 0.]])
    ...
    ...                [3, 6, 5],
    ...                [1, 3, 10]])
    >>> aikit.cummin(x,axis=1,reverse=True, dtype='int64', out=x)
    >>> print(x)
    aikit.array([[ 2,  4,  5],
           [ 3,  5,  5],
           [ 1,  3, 10]])
    ...
    ...                   b = aikit.zeros((2, 3)),
    ...                   c = aikit.zeros((3,3)))
    >>> aikit.cummin(x,axis=1,reverse=True, out=y)
    >>> print(y)
    {
        a: aikit.array([[1., 3., 4.]]),
        b: aikit.array([[3., 5., 8.],
    ...
    >>> x = aikit.Container(a=aikit.array([[0],[5]]),
    ...                   b=aikit.array([[6, 8, 7],
    ...                                [4, 2, 3]]),
    ...                   c=aikit.array([[1, 2],
    ...                                [3, 4],
    ...                                [6, 4]]))
    >>> aikit.cummin(x,axis=0,out=x)
    >>> print(x)
    {
        a: aikit.array([[0],
                      [0]]),
    ...
    """
    return aikit.current_backend(x).cummin(
        x, axis=axis, exclusive=exclusive, reverse=reverse, dtype=dtype, out=out
    )
