...
def invert_permutation(
    ...
    Examples
    --------
    >>> a = aikit.asarray([0, 3, 1, 2])
    >>> aikit.invert_permutation(a)
    aikit.array([0, 2, 3, 1])
    """
    return aikit.current_backend().invert_permutation(x)
...
@handle_device
def lexsort(
    keys: Union[aikit.Array, aikit.NativeArray],
    /,
    *,
    axis: int = -1,
    ...
    >>> aikit.lexsort([b, a]) # Sort by a, then by b
    array([2, 0, 4, 6, 5, 3, 1])
    """
    return aikit.current_backend().lexsort(keys, axis=axis, out=out)
