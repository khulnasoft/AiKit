...
def _verify_coo_components(indices=None, values=None, dense_shape=None):
    ...
    aikit.utils.assertions.check_equal(
        len(aikit.shape(indices)), 2, message="indices must be 2D", as_array=False
    )
    aikit.utils.assertions.check_equal(
        len(aikit.shape(values)), 1, message="values must be 1D", as_array=False
    )
    aikit.utils.assertions.check_equal(
        len(aikit.to_aikit_shape(dense_shape)),
        aikit.shape(indices)[0],
        message="shape and indices shape do not match",
        as_array=False,
    )
    # number of values must match number of coordinates
    aikit.utils.assertions.check_equal(
        aikit.shape(values)[0],
        aikit.shape(indices)[1],
        message="values and indices do not match",
        as_array=False,
    )
    for i in range(aikit.shape(indices)[0]):
        aikit.utils.assertions.check_less(
            indices[i],
            aikit.to_aikit_shape(dense_shape)[i],
...
def _verify_common_row_format_components(
    ...
    aikit.utils.assertions.check_equal(
        len(aikit.shape(crow_indices)),
        1,
        message="crow_indices must be 1D.",
        as_array=False,
    )
    aikit.utils.assertions.check_equal(
        len(aikit.shape(col_indices)),
        1,
...
def _verify_csr_components(
    ...
    aikit.utils.assertions.check_equal(
        aikit.shape(crow_indices)[0] - 1, dense_shape[0], as_array=False
    )
...
def _verify_bsr_components(
    ...
    aikit.utils.assertions.check_equal(
        len(aikit.shape(values)), 3, message="values must be 3D.", as_array=False
    )
    nrowblocks, ncolblocks = aikit.shape(values)[-2:]
    aikit.utils.assertions.check_equal(
    ...
    aikit.utils.assertions.check_equal(
        aikit.shape(crow_indices)[0] - 1, dense_shape[0] // nrowblocks, as_array=False
    )
    aikit.utils.assertions.check_less(
        col_indices,
        dense_shape[1] // ncolblocks,
...
def _verify_common_column_format_components(
    ...
    aikit.utils.assertions.check_equal(
        len(aikit.shape(ccol_indices)),
        1,
        message="ccol_indices must be 1D",
        as_array=False,
    )
    aikit.utils.assertions.check_equal(
        len(aikit.shape(row_indices)), 1, message="row_indices must be 1D", as_array=False
    )
...
def _verify_csc_components(
    ...
    aikit.utils.assertions.check_equal(
        aikit.shape(ccol_indices)[0] - 1, dense_shape[1], as_array=False
    )
    aikit.utils.assertions.check_less(
        row_indices,
        dense_shape[0],
...
def _verify_bsc_components(
    ...
    # number of intervals must be equal to y in shape (x, y)
    aikit.utils.assertions.check_equal(
        aikit.shape(ccol_indices)[0] - 1, dense_shape[1] // ncolblocks, as_array=False
    )
    aikit.utils.assertions.check_less(
        row_indices,
        dense_shape[0] // nrowblocks,
...
def _is_valid_format(
    ...
    if format.endswith("o"):
        # format is coo
        return (
            aikit.exists(coo_indices)
            and aikit.exists(values)
            and aikit.exists(dense_shape)
    ...
    if format.endswith("r"):
        ...
        return (
            aikit.exists(crow_indices)
            and aikit.exists(col_indices)
            and aikit.exists(values)
            and aikit.exists(dense_shape)
    ...
    return (
        aikit.exists(ccol_indices)
        and aikit.exists(row_indices)
        and aikit.exists(values)
        and aikit.exists(dense_shape)
        and coo_indices is None
        and crow_indices is None
...
class SparseArray(aikit.Array):
    ...
    def _init_data(self, data):
        if aikit.is_aikit_sparse_array(data):
            ...
            aikit.utils.assertions.check_true(
                aikit.is_native_sparse_array(data), message="not a native sparse array"
            )
            self._data = data
            self._native_sparse_array_to_indices_values_and_shape()

    def _native_sparse_array_to_indices_values_and_shape(self):
        indices, values, shape = aikit.native_sparse_array_to_indices_values_and_shape(
            self._data
        )

        if "coo_indices" in indices:
            self._coo_indices = aikit.array(indices["coo_indices"], dtype="int64")
            ...
            self._crow_indices = aikit.array(indices["crow_indices"], dtype="int64")
            self._col_indices = aikit.array(indices["col_indices"], dtype="int64")
            self._coo_indices = None
            self._ccol_indices = None
            self._row_indices = None

        else:
            self._ccol_indices = aikit.array(indices["ccol_indices"], dtype="int64")
            self._row_indices = aikit.array(indices["row_indices"], dtype="int64")
            self._coo_indices = None
            self._crow_indices = None
            self._col_indices = None

        self._values = aikit.array(values)
        self._dense_shape = aikit.Shape(shape)
        self._format = self._data.format.lower()

    def _init_coo_components(self, coo_indices, values, shape, format):
        coo_indices = aikit.array(coo_indices, dtype="int64")
        values = aikit.array(values)
        shape = aikit.Shape(shape)
        self._data = aikit.native_sparse_array(
    ...
    def _init_compressed_row_components(
        ...
        crow_indices = aikit.array(crow_indices, dtype="int64")
        col_indices = aikit.array(col_indices, dtype="int64")
        values = aikit.array(values)
        shape = aikit.Shape(shape)
        self._data = aikit.native_sparse_array(
            crow_indices=crow_indices,
    ...
    def _init_compressed_column_components(
        ...
        ccol_indices = aikit.array(ccol_indices, dtype="int64")
        row_indices = aikit.array(row_indices, dtype="int64")
        values = aikit.array(values)
        shape = aikit.Shape(shape)
        self._data = aikit.native_sparse_array(
            ccol_indices=ccol_indices,
    ...
    def __repr__(self):
        if self._dev_str is None:
            ...
            self._pre_repr = "aikit.sparse_array"
    ...
    def coo_indices(self, indices):
        indices = aikit.array(indices, dtype="int64")
    ...
    def dense_shape(self, dense_shape):
        dense_shape = aikit.Shape(dense_shape)
        _verify_coo_components(
    ...
    def _coo_to_dense_coordinates(self):
        ...
        for i in range(self._values.shape[0]):
            coordinate = aikit.gather(self._coo_indices, aikit.array([[i]]))
            coordinate = aikit.reshape(coordinate, (self._coo_indices.shape[0],))
    ...
    def to_dense_array(self, *, native=False):
        ...
        # make dense array
        ret = aikit.scatter_nd(
            aikit.array(all_coordinates),
            aikit.flatten(self._values),
            aikit.array(self._dense_shape),
        )
...
def is_aikit_sparse_array(x):
    return isinstance(x, aikit.SparseArray)


@handle_exceptions
@inputs_to_native_arrays
def is_native_sparse_array(x):
    return aikit.current_backend().is_native_sparse_array(x)
...
def native_sparse_array(
    ...
    *,
    ...
    return aikit.current_backend().native_sparse_array(
        data,
...
def native_sparse_array_to_indices_values_and_shape(x):
    return aikit.current_backend().native_sparse_array_to_indices_values_and_shape(x)
