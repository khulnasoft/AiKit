...
@handle_exceptions
def index_nest(
    nest: Union[List, Tuple, Dict, aikit.Array, aikit.NativeArray, aikit.Container],
...
def set_nest_at_index(
    nest: Union[aikit.Array, aikit.NativeArray, aikit.Container, Dict, List, Tuple],
    index: Sequence[Union[str, int]],
    value: Any,
    /,
    shallow: bool = True,
    _result: Union[aikit.Array, aikit.NativeArray, aikit.Container, Dict, List, Tuple] = None,
) -> Union[aikit.Array, aikit.NativeArray, aikit.Container, Dict, List, Tuple]:
...
def insert_into_nest_at_index(nest: Iterable, index: Tuple, value) -> None:
    ...
    [[1, 2], [3, 99, 4]]
    """
    if isinstance(nest, (dict, aikit.Container)):
        if len(index) == 1:
            key = index[0]
...
def map_nest_at_index(
    nest: Union[aikit.Array, aikit.NativeArray, aikit.Container, Dict, List],
    index: Sequence[Union[str, int]],
    fn: Callable[[Any], Any],
    /,
    shallow: bool = True,
    _result: Union[aikit.Array, aikit.NativeArray, aikit.Container, Dict, List] = None,
) -> Union[aikit.Array, aikit.NativeArray, aikit.Container, Dict, List, Tuple]:
...
def multi_index_nest(
    nest: Union[List, Dict, Tuple, aikit.Array, aikit.NativeArray, aikit.Container],
...
def set_nest_at_indices(
    nest: Union[List, Tuple, Dict, aikit.Array, aikit.NativeArray],
    ...
    >>> indices = [[0, 4], [1, 3]]
    >>> values = [111, 'x']
    >>> aikit.set_nest_at_indices(nest, indices, values)
    >>> print(nest)
    ...
    >>> nest = aikit.array([[1., 2., 3.],[4., 5., 6.]])
    >>> indices = ((0, 1),(1, 2))
    >>> values = (11., 22.)
    >>> aikit.set_nest_at_indices(nest, indices, values)
    >>> print(nest)
    aikit.array([[1., 11., 3.], [4., 5., 22.]])
    """
    is_tuple = isinstance(nest, tuple)
    nest_type = type(nest) if is_tuple else lambda x: x
    if shallow:
...
def map_nest_at_indices(
    ...
    >>> aikit.map_nest_at_indices(nest, indices, function)
    >>> print(nest)
    ...
    >>> nest = aikit.array([[-9., 8., -17.],[11., -3., 5.]])
    >>> indices = ((0, 1),(1, 1),(1, 2))
...
def nested_argwhere(
    ...
    >>> fun = aikit.abs
    >>> nested_indices = aikit.nested_argwhere(nest, fn=fun)
    ...
    >>> nest = ([-5, 9, 2], [0.3, 4.])
    >>> fun = aikit.abs
    >>> nested_indices = aikit.nested_argwhere(nest, fn=fun, stop_after_n_found=4)
    >>> print(nested_indices)
    [[0, 0], [0, 1], [0, 2], [1, 0]]
...
def all_nested_indices(
    nest: Union[List, Tuple, Dict, aikit.Array, aikit.NativeArray, aikit.Container] = None,
    /,
    ...
    >>> x = (189, (863, 672), (264, 384))
    >>> y = aikit.all_nested_indices(x, include_nests=True)
    >>> print(y)
    ...
    >>> x = aikit.array([[True, False], [False, False]])
    >>> y = aikit.all_nested_indices(x)
    >>> print(y)
    [[]]
    ...
    >>> x = aikit.Container(a=aikit.array([412, 948, 482]), b=aikit.array([168, 674, 341]))
    >>> y = aikit.all_nested_indices(x)
    >>> print(y)
    [['a'], ['b']]
    """
...
def map(
    ...
    >>> results = aikit.map(fn = special_square,
    ...                   constant = None,
    ...                   unique = {'x' : [1,2,3]},
    ...                   mean = False)
    >>> print(results)
    ...
    >>> results = aikit.map(fn = special_pow,
    ...                   constant = {'y':[0,1]},
    ...                   unique = {'x':[1,2,3]},
    ...                   mean = False)
    >>> print(results)
    ...
    >>> results = aikit.map(fn = linear_model,
    ...                   constant = {'w':10., 'b':1.},
    ...                   unique = {'x':[0.,1.,2.]},
    ...                   mean = False)
    >>> print(results)
    ...
    >>> results = aikit.map(fn = linear_model,
    ...    constant = {'w':aikit.array([1.,0.,1.]), 'b':aikit.array([0.,10.,100.])},
    ...    unique = {'x':[aikit.array([0.,1.,0.]), aikit.array([1.,1.,1.])]},
    ...    mean = False)
    >>> print(results)
    [aikit.array([0., 10., 100.]),
    aikit.array([1., 10., 101.])]
    ...
    ...    mean = True)
    >>> print(results)
    aikit.array([  0.5,  10. , 100. ])
    """
    c = aikit.default(constant, {})
    u = aikit.default(unique, {})
    rets = [
        r
        for r in _map(
...
def nested_map(
    ...
    shallow: bool = True,
) -> Union[aikit.Array, aikit.NativeArray, Iterable, Dict]:
    ...
    >>> x = aikit.Container(
    ...   a=aikit.array([[1, 2, 3], [9, 8, 7]]) , b=aikit.array([[4, 5, 6], [12, 13, 14]])
    ... )
    >>> function = lambda a : a  + 1
    >>> aikit.nested_map(function, x)
    {
    ...
    >>> function = lambda a :  a * 2
    >>> aikit.nested_map(function, nest, to_ignore=list)
    ([1, 2, 1, 2], [3, 4, 3, 4], [5, 6, 5, 6], {'a': 2, 'b': 4, 'c': 6})

    >>> nest = ([23, 25, 1337], [63, 98, 6])
    >>> function = lambda a :  a + 1
    >>> aikit.nested_map(function, nest, to_mutable=True)
    [[24, 25, 1338], [64, 99, 7]]
    """
    to_ignore = aikit.default(to_ignore, ())
...
@handle_exceptions
def copy_nest(
    nest: Union[aikit.Array, aikit.NativeArray, Iterable],
    /,
    ...
    >>> nest = aikit.array([[1.,2.,3.],[7.,8.,9.]])
    >>> copied_nest = aikit.copy_nest(nest)
    >>> print(copied_nest)
    aikit.array([[1., 2., 3.],
            [7., 8., 9.]])
    ...
    >>> copied_nest = aikit.copy_nest(nest, include_derived = True)
    >>> print(copied_nest)
    [[1, 2, 3, 4, 5], [23, 24, 25, 26, 27]]

    >>> nest = ([23, 25, 1337], [63, 98, 6])
    >>> copied_nest = aikit.copy_nest(nest, to_mutable = True)
    >>> print(copied_nest)
    [[23, 25, 1337], [63, 98, 6]]
...
def nested_multi_map(
    ...
    to_aikit
        convert the output to aikit_arrays. Default is ``True``
    ...
    if nest0 is not None:
        ...
        for index, val in enumerate(nest0):
            ...
            if ret is not None:
                if to_aikit and isinstance(nest, (aikit.Array, aikit.NativeArray)):
                    ret = aikit.array(aikit.to_list(ret))
                (
        ...
        if index_chains is not None:
            ...
            if (found and not to_apply) or (not found and to_apply):
                if prune_unapplied:
                    return return_nest
                if aikit.is_array(value0):
                    if not to_aikit:
                        value0 = aikit.array(value0)
                (
                ...
                return (
                    ...
                    if isinstance(nest, tuple)
                        ...
                        aikit.Container(return_nest)
                        if aikit.is_aikit_container(nest)
                        else return_nest
                    )
                )
        ret = func(values, this_index_chain)
        if to_aikit and ret is not None:
            if isinstance(nest, (aikit.Array, aikit.NativeArray)):
                return ret
            else:
                return aikit.array(ret)
        else:
            return ret
    ...
    return (
        ...
        else aikit.Container(return_nest) if aikit.is_aikit_container(nest0) else return_nest
    )


@handle_exceptions
def duplicate_array_index_chains(nest: Union[aikit.Array, aikit.NativeArray, Iterable]):
    ...
    """
    all_index_chains = aikit.nested_argwhere(nest, lambda _: True)
    duplicates = []
    duplicate_index_chains = {}
    for index_chain in all_index_chains:
        val = aikit.index_nest(nest, index_chain)
...
def prune_empty(nest):
    ...
    if isinstance(nest, dict):
        ...
        for i in range(len(nest) - 1, -1, -1):
            if nest[i] is None:
                del nest[i]
    if not valid and not aikit.is_array(nest) and not isinstance(nest, (int, float, str)):
        return None
    return nest
