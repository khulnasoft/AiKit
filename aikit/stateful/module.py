...
from aikit.stateful.helpers import ModuleHelpers
from aikit.stateful.converters import ModuleConverters
...
class Module(ModuleHelpers, ModuleConverters, ModuleMeta):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        aikit.utils.assertions.check_elem_in_list(build_mode, valid_build_modes)
        ...
        self._buffers = Container(aikit.default(buffers, {}))
        ...
        self._device = aikit.default(device, aikit.default_device())
        self._dtype = aikit.default(dtype, aikit.default_dtype())
    ...
    def build(
        ...
        **kwargs,
        ...
        """
        self._device = aikit.default(device, self._device)
        self._dtype = aikit.default(dtype, self._dtype)
        self._dynamic_backend = aikit.default(dynamic_backend, self._dynamic_backend)
        ...
        # remain un-generated
        built = aikit.default(self._build(*args, **kwargs), True)
        ...
        if aikit.exists(self._v_from_constructor):
        ...
        if not built and from_call:
            ...
            if not aikit.exists(self._v_from_constructor):
                created_n_found = Container(
                    dict(
    ...
    def trace_graph(
        ...
        **trace_kwargs,
        ...
        """Trace the `aikit.Module`'s `_unified_aikit_graph` or `_call` method to
        the target backend.
        ...
        args = aikit.default(args, ())
        kwargs = aikit.default(kwargs, {})

        # shallow copy the kwargs dict
        kwargs = copy.copy(kwargs)
        kwargs["v"] = self.v

        fn_to_trace = aikit.default(self._module_graph, self._call)

        self._module_graph = aikit.trace_graph(
    ...
    def train(self, mode: bool = True):
        ...
        for module in self.v:
            ...
            if isinstance(module, aikit.Module):
    ...
    def to_device(self, device):
        ...
        for obj in self.state_dict.values():
            ...
            elif aikit.is_array(obj) or aikit.is_aikit_container(obj):
                aikit.to_device(obj, device, out=obj)
        return self
    ...
    def show_graph(
        ...
        if not aikit.exists(self._module_graph):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if self._lazy_traced:
            ...
            if self._target:
                aikit.set_backend(self._target)
...
class _HaikuIvyModule(Module):
    ...
    def _build(self, params_hk, *args, **kwargs):
        ...
        args, kwargs = aikit.args_to_native(*args, **kwargs)
        # noinspection PyUnresolvedReferences
        params_dict = self._hk_flat_map_to_dict(params_hk)
        self._hk_params = aikit.Container(params_dict, dynamic_backend=False)
        param_iterator = self._hk_params.cont_to_iterator()
        _, param0 = next(param_iterator, ["_", 0])
        if hasattr(param0, "device"):
            self._device = aikit.as_aikit_dev(param0.device())
        else:
            self._device = aikit.as_aikit_dev("cpu")

    def _forward(self, *a, **kw):
        a, kw = aikit.args_to_native(*a, **kw)
...
class _FlaxIvyModule(Module):
    ...
    def _build(self, params_fx, *args, **kwargs):
        ...
        self._fx_params = aikit.Container(params_dict, dynamic_backend=False)
        param_iterator = self._fx_params.cont_to_iterator()
        _, param0 = next(param_iterator, ["_", 0])
        self._device = aikit.as_aikit_dev(aikit.dev(param0))
...
class _KerasIvyModule(Module):
    def __init__(self, *args, native_module, device, devices, **kwargs):
        ...
        aikit.Module.__init__(self, *args, device=device, devices=devices, **kwargs)

    def _create_variables(self, device=None, dtype=None):
        return self._native_params

    def _build(self, *args, **kwargs):
        self._native_params = aikit.Container(
            OrderedDict(
...
class _TorchIvyModule(Module):
    ...
    def _inplace_update_v(self, new_v):
        aikit.Container.cont_multi_map(
    ...
    def _replace_update_v(self, new_v, native=None):
        ...
        native = aikit.default(native, self._native_module)
