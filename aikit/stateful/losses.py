...
class LogPoissonLoss(Module):
    ...
    def _forward(
        ...
        return aikit.log_poisson_loss(
            true,
            pred,
            compute_full_loss=aikit.default(compute_full_loss, self._compute_full_loss),
            axis=aikit.default(axis, self._axis),
            reduction=aikit.default(reduction, self._reduction),
        )
...
class CrossEntropyLoss(Module):
    ...
    def _forward(self, true, pred, *, axis=None, epsilon=None, reduction=None):
        ...
        return aikit.cross_entropy(
            ...
            pred,
            axis=aikit.default(axis, self._axis),
            epsilon=aikit.default(epsilon, self._epsilon),
            reduction=aikit.default(reduction, self._reduction),
        )
    ...
    def _forward(
        ...
        *,
        ...
        return aikit.binary_cross_entropy(
            true,
            pred,
            from_logits=aikit.default(from_logits, self._from_logits),
            epsilon=aikit.default(epsilon, self._epsilon),
            reduction=aikit.default(reduction, self._reduction),
