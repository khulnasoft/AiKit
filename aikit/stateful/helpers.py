...
from aikit.func_wrapper import _get_first_array
...
class ModuleHelpers:
    def _find_variables(
        ...
        *,
        ...
        """
        _visited = aikit.default(_visited, {})
        vs = Container()
    ...
    def _find_buffers(self):
        for obj in self.__dict__.keys():
            if isinstance(getattr(self, obj), aikit.Module):
    ...
    def _call(self, *args, v=None, buffers=None, **kwargs):
        ...
        if not self._built:
            ...
            self.build(
                ...
                **kwargs,
                from_call=True,
                dtype=first_arr.dtype if aikit.exists(first_arr) else aikit.default_dtype(),
            )
    ...
    def _compute_module_dict(self):
        ...
        for key, value in self.__dict__.items():
            if isinstance(value, aikit.Module):
