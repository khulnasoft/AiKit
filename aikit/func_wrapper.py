...
casting_modes_dict = {
    "uint": lambda: aikit.valid_uint_dtypes,
    "int": lambda: sorted(
        set(aikit.valid_int_dtypes).difference(set(aikit.valid_uint_dtypes))
    ),
    "float": lambda: aikit.valid_float_dtypes,
    "complex": lambda: aikit.valid_complex_dtypes,
}


def caster(dtype, intersect):
    if hasattr(dtype, "dtype"):
        dtype = aikit.as_aikit_dtype(dtype.dtype)
    else:
        dtype = aikit.as_aikit_dtype(dtype)
    if str(dtype) in intersect:
        # based on upcasting or downcasting do something
        if aikit.cast_dtypes():
            ...
            if ret_dtype:
                return ret_dtype
        elif aikit.crosscast_dtypes:
            # check cross_casting
            ret_dtype = cross_caster(intersect)
            if ret_dtype:
                return ret_dtype
        elif aikit.upcast_dtypes:
            # check upcasting
            ret_dtype = upcaster(dtype, intersect)
            if ret_dtype:
                return ret_dtype
        elif aikit.downcast_dtypes:
            # check downcasting
            ret_dtype = downcaster(dtype, intersect)
            if ret_dtype:
                return ret_dtype
...
def cross_caster(intersect):
    ...
    # sorted lists
    dtype = ""
    valid_float = sorted(aikit.valid_float_dtypes)
    valid_int = sorted(aikit.valid_int_dtypes)
...
def try_array_function_override(func, overloaded_args, types, args, kwargs):
    ...
    for overloaded_arg in overloaded_args:
        # Note that we're only calling __aikit_array_function__ on the *first*
        ...
        # which each __aikit_array_function__ implementation might reasonably need to
    ...
    raise TypeError(
        ...
        f" __aikit_array_function__: {list(map(type, overloaded_args))}"
    )


def _get_first_array(*args, **kwargs):
    # ToDo: make this more efficient, with function aikit.nested_nth_index_where
    def array_fn(x):
        return (
            aikit.is_array(x)
            if not hasattr(x, "_aikit_array")
            else aikit.is_array(x.aikit_array)
        )

    array_fn = array_fn if "array_fn" not in kwargs else kwargs["array_fn"]
    arr = None
    if args:
        arr_idxs = aikit.nested_argwhere(args, array_fn, stop_after_n_found=1)
        if arr_idxs:
            arr = aikit.index_nest(args, arr_idxs[0])
        else:
            arr_idxs = aikit.nested_argwhere(kwargs, array_fn, stop_after_n_found=1)
            if arr_idxs:
                arr = aikit.index_nest(kwargs, arr_idxs[0])
    elif kwargs:
        arr_idxs = aikit.nested_argwhere(kwargs, array_fn, stop_after_n_found=1)
        if arr_idxs:
            arr = aikit.index_nest(kwargs, arr_idxs[0])
...
def _build_view(original, view, fn, args, kwargs, index=None):
    if aikit.exists(original._base):
        base = original._base
    ...
    if aikit.exists(original._torch_base):
        view._torch_base = (
            original
            if aikit.exists(original._torch_manipulation)
...
def _get_preferred_device(args, kwargs):
    ...
    if "device" in kwargs and kwargs["device"] is not None:
        return device
    if not aikit.soft_device_mode:
        arr_arg = _get_first_array(*args, **kwargs)
        return aikit.default_device(item=arr_arg, as_native=True)
    return aikit.default_device(as_native=True)
...
def handle_array_function(fn):
    ...
    def _handle_array_function(*args, **kwargs):
        ...
        for arg in args + tuple(kwargs.values()):
            if aikit.exists(arg):
                if not isinstance(arg, aikit.Container) and hasattr(
                    arg, "__aikit_array_function__"
                ):
                    if type(arg) not in overloaded_types:
                        overloaded_types.append(type(arg))
                        if (
                            arg.__aikit_array_function__
                            is not aikit.Array.__aikit_array_function__
                            and not isinstance(arg, (aikit.Array, aikit.NativeArray))
                        ):
                ...
                elif isinstance(arg, aikit.Container):
                    arg = aikit.Container.cont_flatten_key_chains(arg)
                    indices = aikit.nested_argwhere(
                        arg, lambda x: hasattr(x, "__aikit_array_function__")
                    )
                    for a in indices:
                        if type(getattr(arg, a[0])) not in overloaded_types:
                            ...
                            if getattr(
                                arg, a[0]
                            ).__aikit_array_function__ is not aikit.Array.__aikit_array_function__ and not isinstance(  # noqa: E501
                                getattr(arg, a[0]), (aikit.Array, aikit.NativeArray)
                            ):
                                index = len(overloaded_args)
                                for i, old_arg in enumerate(overloaded_args):
                                    if issubclass(
        ...
        success, value = try_array_function_override(
            aikit.__dict__[fn.__name__], overloaded_args, overloaded_types, args, kwargs
        )
        if success:
            return value
            ...
            if (
                ...
                if i < num_args:
                    ...
                    if _check_in_nested_sequence(arg, value=Ellipsis, _type=slice):
                        continue
                    if not aikit.is_array(arg):
                        args[i] = aikit.array(arg, device=device)
                elif parameters in kwargs:
                    kwarg = kwargs[parameter]
                    if not aikit.is_array(kwarg):
                        kwargs[parameter] = aikit.array(kwarg, device=device)
...
def inputs_to_native_arrays(fn: Callable) -> Callable:
    ...
    def _inputs_to_native_arrays(*args, **kwargs):
        """Convert all `aikit.Array` instances in both the positional and keyword
        arguments into `aikit.NativeArray` instances, and then calls the function
        ...
        if "out" in kwargs:
            ...
            del kwargs["out"]
            has_out = True
        # convert all arrays in the inputs to aikit.NativeArray instances
        new_args, new_kwargs = aikit.args_to_native(*args, **kwargs)
...
def inputs_to_aikit_arrays(fn: Callable) -> Callable:
    @functools.wraps(fn)
    def _inputs_to_aikit_arrays(*args, **kwargs):
        """Convert all `aikit.NativeArray` instances in both the positional and
        ...
        aikit_args, aikit_kwargs = aikit.args_to_aikit(
            *args, **kwargs, include_derived={"tuple": True}
        )
        if has_out:
            aikit_kwargs["out"] = out
        return fn(*aikit_args, **aikit_kwargs)

    _inputs_to_aikit_arrays.inputs_to_aikit_arrays = True
    return _inputs_to_aikit_arrays


def inputs_to_native_shapes(fn: Callable) -> Callable:
    @functools.wraps(fn)
    def _inputs_to_native_shapes(*args, **kwargs):
        args, kwargs = aikit.nested_map(
            lambda x: (x.shape if isinstance(x, aikit.Shape) and aikit.array_mode else x),
            [args, kwargs],
        )
        return fn(*args, **kwargs)
...
def outputs_to_aikit_shapes(fn: Callable) -> Callable:
    @functools.wraps(fn)
    def _outputs_to_aikit_shapes(*args, **kwargs):
        args, kwargs = aikit.nested_map(
            lambda x: (x.shape if isinstance(x, aikit.Shape) and aikit.array_mode else x),
            [args, kwargs],
        )
    ...
    _outputs_to_aikit_shapes.outputs_to_aikit_shapes = True
    return _outputs_to_aikit_shapes


def to_native_shapes_and_back(fn: Callable) -> Callable:
    """Make `fn` receive `aikit.NativeShape` and return `aikit.Shape`.

    Wrap `fn` so that input shapes are all converted to
    `aikit.NativeShape` instances and return shapes are all converted to
    `aikit.Shape` instances.
    """
    return outputs_to_aikit_shapes(inputs_to_native_shapes(fn))


def outputs_to_aikit_arrays(fn: Callable) -> Callable:
    @functools.wraps(fn)
    def _outputs_to_aikit_arrays(*args, **kwargs):
        """Call the function, and then converts all `aikit.NativeArray` instances
        in the function return into `aikit.Array` instances.
        ...
        Returns
        -------
            The return of the function, with native arrays as aikit arrays.
        """
        # call unmodified function
        ret = fn(*args, **kwargs)
        # convert all arrays in the return to `aikit.Array` instances
        return (
            aikit.to_aikit(ret, nested=True, include_derived={"tuple": True})
            if aikit.array_mode
            else ret
        )

    _outputs_to_aikit_arrays.outputs_to_aikit_arrays = True
    return _outputs_to_aikit_arrays


def output_to_native_arrays(fn: Callable) -> Callable:
    """Call the function, and then converts all `aikit.Array` instances in the
    function return into `aikit.NativeArray` instances.
    ...
    Returns
    -------
        The return of the function, with aikit arrays as native arrays.
    """

    @functools.wraps(fn)
    def _output_to_native_arrays(*args, **kwargs):
        ret = fn(*args, **kwargs)
        return aikit.to_native(ret, nested=True, include_derived={"tuple": True})
...
def to_aikit_arrays_and_back(fn: Callable) -> Callable:
    """Make `fn` receive `aikit.Array` and return `aikit.NativeArray`.

    Wrap `fn` so that input arrays are all converted to `aikit.Array`
    instances and return arrays are all converted to `aikit.NativeArray`
    instances.
    """
    return output_to_native_arrays(inputs_to_aikit_arrays(fn))


def to_native_arrays_and_back(fn: Callable) -> Callable:
    """Make `fn` receive `aikit.NativeArray` and return `aikit.Array`.
    ...
    """
    return outputs_to_aikit_arrays(inputs_to_native_arrays(fn))


def frontend_outputs_to_aikit_arrays(fn: Callable) -> Callable:
    """Wrap `fn` and convert all frontend arrays in its return to aikit arrays.
    ...
    of aikit functions, its outputs need to be converted to aikit arrays.
    """
    ...
    def _outputs_to_aikit_arrays(*args, **kwargs):
        ...
        return aikit.nested_map(
            lambda x: x.aikit_array if hasattr(x, "aikit_array") else x,
            ret,
            shallow=False,
        )
...
def handle_view(fn: Callable) -> Callable:
    ...
    def _handle_view(*args, **kwargs):
        ret = fn(*args, **kwargs)
        if ("copy" in kwargs and kwargs["copy"]) or not aikit.is_aikit_array(args[0]):
            return ret
        original = args[0]
...
def handle_view_indexing(fn: Callable) -> Callable:
    ...
    def _handle_view_indexing(*args, **kwargs):
        ...
        #  Need general way to convert special method to function found in aikit.__dict__
...
def _convert_numpy_arrays_to_backend_specific(*args):
    if isinstance(args, np.ndarray):
        np_arr_idxs = aikit.nested_argwhere(args, lambda x: isinstance(x, np.ndarray))
        np_arr_val = aikit.multi_index_nest(args, np_arr_idxs)
        backend_arr_vals = [aikit.array(x).to_native() for x in np_arr_val]
        aikit.set_nest_at_indices(args, np_arr_idxs, backend_arr_vals)
    return args
...
def infer_dtype(fn: Callable) -> Callable:
    ...
    def _infer_dtype(*args, dtype=None, **kwargs):
        ...
        arr = None if aikit.exists(dtype) else _get_first_array(*args, **kwargs)
        # infer the correct data type
        dtype = aikit.default_dtype(dtype=dtype, item=arr, as_native=True)
...
def handle_device(fn: Callable) -> Callable:
    ...
    def _handle_device(*args, **kwargs):
        """Move all array inputs of the function to `aikit.default_device()`.
        ...
        dev = None
        if "device" in kwargs and kwargs["device"] is not None:
            dev = aikit.as_native_dev(kwargs["device"])
        if aikit.soft_device_mode:
            with aikit.DefaultDevice(aikit.default_device(dev)):
                return aikit.handle_soft_device_variable(*args, fn=fn, **kwargs)
        inputs = args + tuple(kwargs.values())
        devices = tuple(aikit.dev(x) for x in inputs if aikit.is_array(x))
        ...
        if len(unique_devices) <= 1:
            ...
            with aikit.DefaultDevice(aikit.default_device(dst_dev)):
                return aikit.handle_soft_device_variable(*args, fn=fn, **kwargs)
...
def handle_out_argument(fn: Callable) -> Callable:
    ...
    def _handle_out_argument(*args, out=None, **kwargs):
        ...
        ret = fn(*args, **kwargs)
        if not aikit.is_array(ret) and not aikit.is_aikit_container(ret):
            return aikit.nested_multi_map(
                lambda x, _: aikit.inplace_update(
                    x[0], aikit.astype(x[1], aikit.dtype(x[0]))
                ),
                [out, ret],
            )
        return aikit.inplace_update(out, aikit.astype(ret, aikit.dtype(out)))
...
def _update_torch_views(x, visited_view=None):
    ...
    if aikit.exists(x._torch_manipulation):
        ...
        if fn == "rot90":
            kwargs = kwargs.copy()
            kwargs["k"] = -kwargs["k"]
        parent_tensor.data[()] = aikit.__dict__[fn](x, *args, **kwargs).data
    if aikit.exists(x._torch_base):
...
def _update_torch_references(x, visited_view=None):
    for ref in x._torch_view_refs:
        view = ref()
        if aikit.exists(view) and view is not visited_view:
            parent_tensor, fn_args_kwargs = view._torch_manipulation
            fn, args, kwargs = fn_args_kwargs
            kwargs["copy"] = True
            view.data[()] = aikit.__dict__[fn](parent_tensor, *args, **kwargs).data
...
def handle_nestable(fn: Callable) -> Callable:
    ...
    def _handle_nestable(*args, **kwargs):
        ...
        if hasattr(aikit.Container, f"_static_{fn_name}"):
            cont_fn = getattr(aikit.Container, f"_static_{fn_name}")
        else:

            def cont_fn(*args, **kwargs):
                return aikit.Container.cont_multi_map_in_function(fn, *args, **kwargs)

        if aikit.nestable_mode and (
            aikit.nested_any(args, aikit.is_aikit_container, check_nests=True)
            or aikit.nested_any(kwargs, aikit.is_aikit_container, check_nests=True)
        ):
...
def handle_ragged(fn: Callable) -> Callable:
    ...
    def _handle_ragged(*args, **kwargs):
        ...
        def nested_fn(*args, **kwargs):
            return aikit.NestedArray.ragged_multi_map_in_function(fn, *args, **kwargs)

        if aikit.nested_any(
            args, aikit.is_aikit_nested_array, check_nests=True
        ) or aikit.nested_any(kwargs, aikit.is_aikit_nested_array, check_nests=True):
...
def temp_asarray_wrapper(fn: Callable) -> Callable:
    ...
    def _temp_asarray_wrapper(*args, **kwargs):
        ...
        def _to_aikit_array(x):
            # if x is a frontend torch Tensor (or any frontend "Tensor" actually) return the wrapped aikit array # noqa: E501
...
def download_cython_wrapper_wrapper(fn: Callable) -> Callable:
    ...
    def _download_cython_wrapper_wrapper(*args, **kwargs):
        ...
        aikit.wrappers.get_wrapper(func_name: str) and then wrap the
        function with the downloaded wrapper.
        """
        aikit.wrappers.download_cython_wrapper(fn.__name__)
        aikit.wrappers.load_one_wrapper(fn.__name__)
        aikit.functional.__dict__[fn.__name__] = getattr(
            aikit.wrappers, fn.__name__ + "_wrapper"
        )(fn)
        return aikit.functional.__dict__[fn.__name__](*args, **kwargs)
        ...
        for linalg_k, linalg_v in to_wrap.__dict__.items():
            if (
                ...
                to_wrap.__dict__[linalg_k] = _wrap_function(
                    linalg_k,
                    linalg_v,
                    aikit.__dict__[linalg_k],
                    compositional=compositional,
                )
        return to_wrap
    if isinstance(to_wrap, FunctionType):
        if aikit.cython_wrappers_mode and aikit.wrappers.wrapper_exists(to_wrap.__name__):
            if to_wrap.__name__ + "_wrapper" in aikit.wrappers.__all__:
                to_wrap = getattr(aikit.wrappers, to_wrap.__name__ + "_wrapper")(to_wrap)
                return to_wrap
            else:
        ...
        for attr in FN_DECORATORS:
            if hasattr(original, attr) and not hasattr(to_wrap, attr):
                ...
                if attr not in skip_wrappers:
                    to_wrap = getattr(aikit, attr)(to_wrap)
...
def casting_modes_ops(fn, ret_dtype_target=None):
    ...
    def method(*args, **kwargs):
        ...
        intersect = set(aikit.function_unsupported_dtypes(fn)).difference(
            set(aikit.invalid_dtypes)
        )
        if not intersect:
            # doesn't have unsupported dtypes specified
            # so check if it's one of the device_and_dtype one
            intersect = set(
                aikit.function_unsupported_devices_and_dtypes(fn).get(
                    aikit.default_device().split(":")[0], {None}
                )
            ).difference(set(aikit.invalid_dtypes))
            if not intersect:
        ...
        if "dtype" in kwargs and kwargs["dtype"] is not None:
            ...
            if dtype:
                kwargs["dtype"] = aikit.as_native_dtype(dtype)
        ...
        def mini_helper(x):
            ...
            if dtype:
                x = aikit.to_native(aikit.astype(x, aikit.as_native_dtype(dtype)))
            return x

        args = aikit.nested_map(mini_helper, args, include_derived=True)
        kwargs = aikit.nested_map(mini_helper, kwargs)

        if not to_cast and ret_dtype_target:
            for arg in ret_dtype_target:
                if arg:
                    to_cast, arg_mod = aikit.promote_types_of_inputs(
                        to_cast,
                        (
                    ...
                    if arg not in kwargs:
                        args[arg_names.index(arg)] = (
                            arg_mod
                            if not aikit.is_array(args[arg_names.index(arg)])
        ...
        return (
            aikit.astype(fn(*args, **kwargs), aikit.to_native(to_cast))
            if to_cast
            else fn(*args, **kwargs)
        )
...
def _dtype_from_version(dic, version):
    ...
    if isinstance(version, str):
        version = aikit.functional.frontends.__dict__["versions"][version]
...
def _leaf_has_nans(x):
    if isinstance(x, aikit.Container):
        return x.has_nans()
    elif aikit.is_array(x):
        return aikit.isnan(x).any()
    elif np.isnan(x):
        return True
    return False
...
def _nest_has_nans(x):
    return aikit.nested_any(x, _leaf_has_nans)
...
def handle_complex_input(fn: Callable) -> Callable:
    ...
    def _handle_complex_input(
        ...
        **kwargs,
        ...
        >>> @handle_complex_input
        >>> def my_func(inp):
        >>>     return aikit.ones_like(inp)

        >>> x = aikit.array([1+1j, 3+4j, 5+12j])
        >>> my_func(x)  # equivalent to setting complex_mode="jax"
        aikit.array([1.+0.j, 1.+0.j, 1.+0.j])

        >>> my_func(x, complex_mode="split")
        aikit.array([1.+1.j, 1.+1.j, 1.+1.j])

        >>> my_func(x, complex_mode="magnitude")
        aikit.array([0.70710681+0.70710675j, 0.60000001+0.79999999j,
        ...
        >>> my_func(x, complex_mode="jax")
        aikit.array([0.+3.j, 0.+3.j, 0.+3.j])
        """
        if not aikit.is_complex_dtype(inp):
        ...
        if complex_mode == "split" or (complex_mode == "jax" and jax_like == "split"):
            real_inp = aikit.real(inp).data
            imag_inp = aikit.imag(inp).data
            if "out" in kwargs and kwargs["out"] is not None:
                out = kwargs.pop("out")
                real_ret = fn(real_inp, *args, out=aikit.real(out), **kwargs)
                imag_ret = fn(imag_inp, *args, out=aikit.imag(out), **kwargs)
            else:
                real_ret = fn(real_inp, *args, **kwargs)
                imag_ret = fn(imag_inp, *args, **kwargs)
            return aikit.add(
                real_ret,
                aikit.multiply(aikit.array(1j, dtype=inp.dtype), imag_ret),
            )

        elif complex_mode == "magnitude" or (
            complex_mode == "jax" and jax_like == "magnitude"
        ):
            mag_inp = aikit.abs(inp).data
            angle_inp = aikit.angle(inp).data
            return aikit.multiply(
                fn(mag_inp, *args, **kwargs), aikit.exp(aikit.multiply(1j, angle_inp))
            )
...
def handle_backend_invalid(fn: Callable) -> Callable:
    ...
    def _handle_backend_invalid(*args, **kwargs):
        ...
        function are instances of aikit.Array or aikit.NativeArray. If so, it
        ...
        """
        array_indices = aikit.nested_argwhere(
            [args, kwargs], lambda x: isinstance(x, aikit.Array)
        )
        array_vals = aikit.multi_index_nest([args, kwargs], array_indices)

        def func(x):
            target_backend = aikit.utils.backend.handler._determine_backend_from_args(x)
            if (
        ...
        aikit.nested_map(func, array_vals, include_derived=True)
...
def globals_getter_func(x=None):
    ...
    # aikit.func_wrapper.globals_getter_func in the module
