...
def _remove_so_log(trace):
    ...
    for idx, st in enumerate(old_stack_trace):
        ...
        if "<module>" in repr(st):
            ...
            and os.path.join("aikit", "compiler") in st.filename
...
def _align_source(st, transpile_frame, module_frame, module_st, traced_lineno):
    from aikit.compiler.utils.VVX import trace_obj
    from aikit.compiler.utils.IIV import Graph
...
def _get_traces(curr_obj, area, local_dict, target_name):
    from aikit.compiler.utils.VVX import trace_obj, get_source_code, CallVistior
...
def _configure_stack_trace(traceback):
    ...
    show_wrappers = aikit.show_func_wrapper_trace_mode

    aikit_path = os.path.join("aikit", "functional", "aikit")
    frontend_path = os.path.join("aikit", "functional", "frontends")
    wrapper_path = os.path.join("aikit", "func_wrapper.py")

    while 1 and tb.tb_next:
        frame = tb.tb_next.tb_frame
        file_path = frame.f_code.co_filename
        if trace_mode == "aikit":
            if _check_if_path_found(aikit_path, file_path):
                tb = tb.tb_next
            else:
                tb.tb_next = tb.tb_next.tb_next
        elif trace_mode == "frontend":
            if _check_if_path_found(frontend_path, file_path) or _check_if_path_found(
                aikit_path, file_path
            ):
                tb = tb.tb_next
            else:
                tb.tb_next = tb.tb_next.tb_next
...
def _combine_messages(*messages, include_backend=True):
    ...
    default = [
        "numpy" if aikit.current_backend_str() == "" else aikit.current_backend_str()
    ]
    delimiter = ": "
...
def _handle_inplace_mode(aikit_pack=None):
    if not aikit_pack:
        aikit_pack = aikit
    current_backend = aikit_pack.current_backend_str()
    if (
        current_backend != ""
        and not _inplace_warning_cache.get(current_backend)
        and not aikit_pack.native_inplace_support
        and aikit_pack.inplace_mode == "lenient"
    ):
        warnings.warn(
...
def _check_inplace_update_support(x, ensure_in_backend):
    current_backend = aikit.current_backend_str()
    is_tf_variable = current_backend == "tensorflow" and not aikit.is_aikit_array(
        x, exclusive=True
    )
    if (
        ensure_in_backend
        or aikit.is_native_array(x)
        or (aikit.inplace_mode == "strict" and not is_tf_variable)
    ):
        raise aikit.utils.exceptions.InplaceUpdateException(
            f"{current_backend} does not support inplace updates "
            "and aikit cannot support the operation in 'strict' mode\n"
            "To enable inplace update, use aikit.set_inplace_mode('lenient')\n"
        )
