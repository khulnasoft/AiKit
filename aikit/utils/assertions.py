...
def _broadcast_inputs(x1, x2):
    x1_, x2_ = x1, x2
    iterables = (list, tuple, aikit.Shape)
...
def check_less(x1, x2, allow_equal=False, message="", as_array=True):
    comp_fn = lambda x1, x2: (aikit.any(x1 > x2), aikit.any(x1 >= x2))
    if not as_array:
...
def check_equal(x1, x2, inverse=False, message="", as_array=True):
    ...
    comp_fn = lambda x1, x2: aikit.any(eq_fn(x1, x2))
    if not as_array:
...
def check_shape(x1, x2, message=""):
    message = (
        ...
        if message != ""
            ...
            f"{x1} and {x2} must have the same shape ({aikit.shape(x1)} vs"
            f" {aikit.shape(x2)})"
        )
    )
...
def check_same_dtype(x1, x2, message=""):
    if aikit.dtype(x1) != aikit.dtype(x2):
        message = (
            message
            if message != ""
            else (
                f"{x1} and {x2} must have the same dtype ({aikit.dtype(x1)} vs"
                f" {aikit.dtype(x2)})"
            )
        )
        raise aikit.utils.exceptions.AikitException(message)
...
def check_unsorted_segment_valid_params(data, segment_ids, num_segments):
    ...
    if aikit.max(segment_ids) >= num_segments:
        error_message = (
            f"segment_ids[{aikit.argmax(segment_ids)}] = "
            f"{aikit.max(segment_ids)} is out of range [0, {num_segments})"
        )
...
def _check_jax_x64_flag(dtype):
    if (
        aikit.backend == "jax"
        and not aikit.functional.backends.jax.jax.config.jax_enable_x64
    ):
