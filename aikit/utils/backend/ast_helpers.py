...
importlib_module_path = "aikit.utils._importlib"
...
_local_import_template = Template(
    "$name = "
    "aikit.utils.backend.handler._compiled_backends_ids[$aikit_id].utils._importlib.$name"
)
_unmodified_aikit_path = sys.modules["aikit"].__path__[0].rpartition(os.path.sep)[0]
...
class ImportTransformer(ast.NodeTransformer):
    def __init__(self):
        ...
        self.include_aikit_import = False

    def visit_Import(self, node):
        ret, should_impersonate = _parse_import(node)
        if should_impersonate and not self.include_aikit_import:
            self.include_aikit_import = True
        return ret
    ...
    def impersonate_import(self, tree: ast.Module, local_aikit_id=None):
        if not self.include_aikit_import:
            return tree
        ...
        if local_aikit_id is None:
            ...
            insert_import(
                _local_import_template.substitute(
                    name=importlib_abs_import_fn, aikit_id=local_aikit_id
                )
            )
            insert_import(
                _local_import_template.substitute(
                    name=importlib_from_import_fn, aikit_id=local_aikit_id
                )
            )
            insert_import("import aikit")
...
class AikitLoader(Loader):
    ...
    def exec_module(self, module, local_aikit_id=None):
        if self.filename in _compiled_modules_cache:
            ...
            transformer.impersonate_import(ast_tree, local_aikit_id)
