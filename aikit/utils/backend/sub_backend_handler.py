...
for backend in os.listdir(
    ...
    sub_backends_dir = os.path.join(
        aikit.__path__[0].rpartition(os.path.sep)[0],
        _backends_subpackage_path.replace(".", os.path.sep),
        backend,
        "sub_backends",
    )
...
def set_sub_backend(sub_backend_str: str):
    ...
    if aikit.current_backend_str() not in _backend_to_sub_backends_dict:
        logging.warning(
            f"backend {aikit.current_backend_str()} does not have any"
    ...
    if sub_backend_str not in _backend_to_sub_backends_dict[aikit.current_backend_str()]:
        logging.warning(
            f"{aikit.current_backend_str()} does not support"
            f" {sub_backend_str} as a sub_backend"
        )
        return

    if sub_backend_str in aikit.current_sub_backends:
        return

    global original_backend_dict
    if original_backend_dict is None:
        original_backend_dict = aikit.__dict__.copy()
    sub_backend = importlib.import_module(_sub_backend_dict[sub_backend_str])
    set_sub_backend_to_specific_version(sub_backend)
    _set_sub_backend_as_aikit(aikit.__dict__.copy(), aikit, sub_backend)
    aikit.current_sub_backends.append(sub_backend_str)


# this is very similar to _set_backend_as_aikit in handler.py, with a minor change
def _set_sub_backend_as_aikit(
...
def unset_sub_backend(sub_backend_str: str):
    if sub_backend_str not in aikit.current_sub_backends:
        return
    global original_backend_dict

    # The sub-backend is cached so this is fast
    sub_backend = importlib.import_module(_sub_backend_dict[sub_backend_str])
    _unset_sub_backend_from_aikit(
        original_backend_dict, aikit, sub_backend, sub_backend.name
    )
    aikit.current_sub_backends.remove(sub_backend_str)


def _unset_sub_backend_from_aikit(
...
def clear_sub_backends():
    if aikit.current_sub_backends:
        aikit.__dict__.update(original_backend_dict)
        aikit.current_sub_backends.clear()
