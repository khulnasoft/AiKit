...
import aikit.utils.backend.handler
from aikit.utils import check_for_binaries
from aikit._version import __version__ as __version__

_not_imported_backends = list(aikit.utils.backend.handler._backend_dict.keys())
try:
...
def is_local():
    return hasattr(aikit, "_is_local_pkg")
...
class FrameworkStr(str):
    def __new__(cls, fw_str):
        aikit.utils.assertions.check_elem_in_list(
            fw_str, aikit.utils.backend.handler._backend_dict.keys()
        )
...
class Device(str):
    def __new__(cls, dev_str):
        if dev_str != "":
            aikit.utils.assertions.check_elem_in_list(dev_str[0:3], ["gpu", "tpu", "cpu"])
            if dev_str != "cpu":
                # aikit.assertions.check_equal(dev_str[3], ":")
...
class Dtype(str):
    ...
    def __ge__(self, other):
        ...
        return self == aikit.promote_types(self, other)
...
class Shape(Sequence):
    def __init__(self, shape_tup):
        valid_types = (int, list, tuple, aikit.Array, aikit.Shape)
        if len(backend_stack) != 0:
            valid_types += (aikit.NativeShape, aikit.NativeArray)
        else:
            valid_types += (
                current_backend(shape_tup).NativeShape,
                current_backend(shape_tup).NativeArray,
            )
        aikit.utils.assertions.check_isinstance(shape_tup, valid_types)
        if len(backend_stack) == 0:
            ...
            self._shape = aikit.to_native_shape(shape_tup)
        else:
            self._shape = None

    @staticmethod
    def _shape_casting_helper(aikit_shape, other):
        if isinstance(other, tuple) and not isinstance(aikit_shape, tuple):
            return tuple(aikit_shape)
        elif isinstance(other, list) and not isinstance(aikit_shape, list):
            return list(aikit_shape)
        else:
            return aikit_shape
    ...
    def __repr__(self):
        ...
        return (
            f"aikit.Shape({shape_repr})" if self._shape is not None else "aikit.Shape(None)"
        )
...
warn_to_regex = {"all": "!.*", "aikit_only": "^(?!.*aikit).*$", "none": ".*"}
...
from .data_classes.array import Array, add_aikit_array_instance_methods
...
from .data_classes.container import (
    ContainerBase,
    Container,
    add_aikit_container_instance_methods,
)
...
from aikit.utils.backend import (
    current_backend,
    compiled_backends,
    with_backend,
    set_backend,
...
from aikit.utils.inspection import fn_array_spec, add_array_specs
...
add_aikit_array_instance_methods(
    Array,
    [
        activations,
        arr_conversions,
...
# defines aikit.globals attribute
...
def _assert_array_significant_figures_formatting(sig_figs):
    aikit.utils.assertions.check_isinstance(sig_figs, int)
    aikit.utils.assertions.check_greater(sig_figs, 0, as_array=False)
...
aikit.array_significant_figures = (
...
def set_array_significant_figures(sig_figs):
    ...
    aikit.__setattr__("array_significant_figures", sig_figs, True)
...
def _assert_array_decimal_values_formatting(dec_vals):
    aikit.utils.assertions.check_isinstance(dec_vals, int)
    aikit.utils.assertions.check_greater(dec_vals, 0, allow_equal=True, as_array=False)


aikit.array_decimal_values = (
...
def set_array_decimal_values(dec_vals):
    ...
    aikit.__setattr__("array_decimal_values", dec_vals, True)
...
aikit.warning_level = warning_level_stack[-1] if warning_level_stack else "aikit_only"
...
def set_warning_level(warn_level):
    ...
    ----------
    warn_level
        string for the warning level to be set, one of "none", "aikit_only", "all"
    """
    global warning_level_stack
    warning_level_stack.append(warn_level)
    aikit.__setattr__("warning_level", warn_level, True)
...
def unset_warning_level():
    ...
    if warning_level_stack:
        ...
        warn_level = warning_level_stack[-1] if warning_level_stack else "aikit_only"
        aikit.__setattr__("warning_level", warn_level, True)


def warn(warning_message, stacklevel=0):
    warn_level = aikit.warning_level
    warnings.filterwarnings("ignore", module=warn_to_regex[warn_level])
    warnings.warn(warning_message, stacklevel=stacklevel)


# nan policy #
aikit.nan_policy = nan_policy_stack[-1] if nan_policy_stack else "nothing"
...
def set_nan_policy(warn_level):
    ...
    aikit.__setattr__("nan_policy", warn_level, True)
...
aikit.dynamic_backend = dynamic_backend_stack[-1] if dynamic_backend_stack else True
...
def set_dynamic_backend(flag):  # noqa: D209
    ...
    aikit.__setattr__("dynamic_backend", flag, True)
...
aikit.cython_wrappers_mode = cython_wrappers_stack[-1] if cython_wrappers_stack else False
...
def set_cython_wrappers_mode(flag: bool = True) -> None:
    ...
    aikit.__setattr__("cython_wrappers_mode", flag, True)
...
class DynamicBackendContext:
    ...
    def __enter__(self):
        self.original = aikit.dynamic_backend
...
for backend_framework in _not_imported_backends:
    if backend_framework in sys.modules:
        warnings.warn(
            f"{backend_framework} module has been imported while aikit doesn't "
...
INTERNAL_FILENAMES = [
    os.path.join("aikit", "compiler"),
    os.path.join("aikit", "functional"),
    os.path.join("aikit", "data_classes"),
    os.path.join("aikit", "stateful"),
    os.path.join("aikit", "utils"),
    os.path.join("aikit_tests", "test_aikit"),
    os.path.join("aikit", "func_wrapper.py"),
    os.path.join("aikit", "__init__.py"),
]
...
if (
    "aikit" in sys.modules
    and sys.modules["aikit"].utils._importlib.IS_COMPILING_WITH_BACKEND
):
