...
class TRTensor(FactorizedTensor):
    ...
    def n_param(self):
        ...
        total_params = sum(int(aikit.prod(tensor.shape)) for tensor in factors)
        return total_params
        ...
        for index, factor in enumerate(factors):
            ...
            if len(factor.shape) != 3:
                raise ValueError(
                    ...
                    f"However, aikit.ndim(factors[{index}]) = {len(factor.shape)}"
                )

            # Consecutive factors should have matching ranks
            if aikit.shape(factors[index - 1])[2] != current_rank:
                raise ValueError(
                    "Consecutive factors should have matching ranks\n -- e.g."
    ...
    def tr_to_tensor(factors):
        ...
        full_tensor = aikit.reshape(factors[0], (-1, factors[0].shape[2]))

        for factor in factors[1:-1]:
            rank_prev, _, rank_next = factor.shape
            factor = aikit.reshape(factor, (rank_prev, -1))
            full_tensor = aikit.dot(full_tensor, factor)
            full_tensor = aikit.reshape(full_tensor, (-1, rank_next))

        full_tensor = aikit.reshape(
            full_tensor, (factors[-1].shape[2], -1, factors[-1].shape[0])
        )
        full_tensor = aikit.moveaxis(full_tensor, 0, -1)
        full_tensor = aikit.reshape(
            full_tensor, (-1, factors[-1].shape[0] * factors[-1].shape[2])
        )
        factor = aikit.moveaxis(factors[-1], -1, 1)
        factor = aikit.reshape(factor, (-1, full_shape[-1]))
        full_tensor = aikit.dot(full_tensor, factor)
        return aikit.reshape(full_tensor, full_shape)

    @staticmethod
    def tr_to_unfolded(factors, mode):
        return aikit.unfold(TRTensor.tr_to_tensor(factors), mode)
    ...
    def validate_tr_rank(tensor_shape, rank="same", rounding="round"):
        ...
        if isinstance(rank, float):
            ...
            solution = int(
                rounding_fun(aikit.sqrt(n_param_tensor / aikit.sum(tensor_shape)))
            )
