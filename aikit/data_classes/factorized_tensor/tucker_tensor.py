...
class TuckerTensor(FactorizedTensor):
    ...
    def n_param(self):
        ...
        total_params = sum(int(aikit.prod(tensor.shape)) for tensor in [core] + factors)
        return total_params
    ...
    def validate_tucker_tensor(tucker_tensor):
        ...
        for i, factor in enumerate(factors):
            ...
            if current_rank != aikit.shape(core)[i]:
                raise ValueError(
                    "Factor `n` of Tucker decomposition should"
                    " verify:\nfactors[n].shape[1] = core.shape[n].However,"
                    f" factors[{i}].shape[1]={aikit.shape(factor)[1]} but"
                    f" core.shape[{i}]={aikit.shape(core)[i]}."
                )
    ...
    def tucker_normalize(tucker_tensor):
        ...
        normalized_factors = []
        for i, factor in enumerate(factors):
            scales = aikit.sqrt(aikit.sum(aikit.abs(factor) ** 2, axis=0))
            scales_non_zero = aikit.where(
                scales == 0, aikit.ones(aikit.shape(scales), dtype=factor[0].dtype), scales
            )
            core = core * aikit.reshape(
    ...
    def tucker_mode_dot(
        ...
        if contract:
            ...
            f = factors.pop(mode)
            core = aikit.mode_dot(core, aikit.dot(matrix_or_vector, f), mode=mode)
        else:
    ...
    def validate_tucker_rank(
        ...
        if isinstance(rank, float):
            ...
            n_param_tensor = int(aikit.prod(tensor_shape))
            ...
            squared_dims = aikit.sum([s**2 for s in tensor_shape])
    ...
    def tucker_n_param(shape, rank):
        core_params = aikit.prod(rank)
        factors_params = aikit.sum([r * s for (r, s) in zip(rank, shape)])
