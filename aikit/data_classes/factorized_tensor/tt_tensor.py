...
class TTTensor(FactorizedTensor):
    def __init__(self, factors, inplace=False):
        ...
        shape, rank = aikit.TTTensor.validate_tt_tensor(factors)
    ...
    # Public Methods #
    def to_tensor(self):
        return aikit.TTTensor.tt_to_tensor(self)

    def to_unfolding(self, mode):
        return aikit.TTTensor.tt_to_unfolded(self, mode)

    def to_vec(self):
        return aikit.TTTensor.tt_to_vec(self)
    ...
    def n_param(self):
        ...
        return aikit.sum(factor_params)
    ...
    def validate_tt_tensor(tt_tensor):
        ...
        for index, factor in enumerate(factors):
            current_rank, current_shape, next_rank = aikit.shape(factor)

            if len(aikit.shape(factor)) != 3:
                raise ValueError(
                    "TT expresses a tensor as third order factors"
                    f" (tt-cores).\nHowever, len(aikit.shape(factors[{index}])) ="
                    f" {len(aikit.shape(factor))}"
                )

            if index and aikit.shape(factors[index - 1])[2] != current_rank:
                raise ValueError(
                    "Consecutive factors should have matching ranks\n -- e.g."
                    " aikit.shape(factors[0])[2]) == aikit.shape(factors[1])[0])\nHowever,"
                    f" aikit.shape(factor[{index-1}])[2] =="
                    f" {aikit.shape(factors[index - 1])[2]} but"
                    f" aikit.shape(factor[{index}])[0] == {current_rank} "
                )
    ...
    def tt_to_tensor(factors):
        ...
        full_tensor = aikit.reshape(factors[0], (full_shape[0], -1))

        for factor in factors[1:]:
            rank_prev, _, rank_next = factor.shape
            factor = aikit.reshape(factor, (rank_prev, -1))
            full_tensor = aikit.matmul(full_tensor, factor)
            full_tensor = aikit.reshape(full_tensor, (-1, rank_next))

        return aikit.reshape(full_tensor, full_shape)
    ...
    def tt_to_unfolded(factors, mode):
        ...
        """
        return aikit.unfold(aikit.TTTensor.tt_to_tensor(factors), mode)
    ...
    def tt_to_vec(factors):
        ...
        """
        return aikit.reshape(aikit.TTTensor.tt_to_tensor(factors), (-1,))
    ...
    def validate_tt_rank(
        ...
        """  # noqa: E501
        if rounding == "ceil":
            rounding_fn = aikit.ceil
        elif rounding == "floor":
            rounding_fn = aikit.floor
        elif rounding == "round":
            rounding_fn = aikit.round
        else:
            raise ValueError(
        ...
        if isinstance(rank, float) and constant_rank:
            n_param_tensor = aikit.prod(tensor_shape) * rank
            ...
            a = aikit.sum(tensor_shape[1:-1])
            b = aikit.sum(tensor_shape[0] + tensor_shape[-1])
            c = -n_param_tensor
            delta = aikit.sqrt(b**2 - 4 * a * c)
            ...
            c = -aikit.prod(tensor_shape) * rank
            delta = aikit.sqrt(b**2 - 4 * a * c)
            ...
            if isinstance(rank, int):
                ...
                message = (
                    ...
                    f" len(aikit.shape(tensor)) + 1, but len(rank) = {len(rank)} while"
                    f" len(aikit.shape(tensor)) + 1  = {n_dim+1}"
                )
        ...
        if allow_overparametrization:
            ...
            for i, s in enumerate(tensor_shape[:-1]):
                n_row = int(rank[i] * s)
                n_column = aikit.prod(tensor_shape[(i + 1) :])
    ...
    def pad_tt_rank(factor_list, n_padding=1, pad_boundaries=False):
        ...
        for i, factor in enumerate(factor_list):
            ...
            r1, *s, r2 = aikit.shape(factor)
            new_factor = aikit.zeros((r1 + n_padding_left, *s, r2 + n_padding_right))
            new_factors.append(
                aikit.TTTensor.index_update(
                    new_factor,
