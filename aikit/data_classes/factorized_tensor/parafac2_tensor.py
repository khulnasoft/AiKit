...
class Parafac2Tensor(FactorizedTensor):
    def __init__(self, parafac2_tensor):
        ...
        shape, rank = aikit.Parafac2Tensor.validate_parafac2_tensor(parafac2_tensor)
    ...
    # ---------------#
    def to_tensor(self):
        return aikit.Parafac2Tensor.parafac2_to_tensor(self)

    def to_vec(self):
        return aikit.Parafac2Tensor.parafac2_to_vec(self)

    def to_unfolded(self, mode):
        return aikit.Parafac2Tensor.parafac2_to_unfolded(self, mode)
    ...
    def from_CPTensor(cls, cp_tensor, parafac2_tensor_ok=False):
        ...
        Q, R = aikit.qr(B)
        projections = [Q for _ in range(aikit.shape(A)[0])]
        B = R
    ...
    def validate_parafac2_tensor(parafac2_tensor):
        ...
        """
        if isinstance(parafac2_tensor, aikit.Parafac2Tensor):
        ...
        for i, projection in enumerate(projections):
            current_mode_size, current_rank = aikit.shape(projection)
            if current_rank != rank:
                raise ValueError(
                    ...
                    f" projections[{i}].shape[1]={aikit.shape(projection)[1]}"
                )

            inner_product = aikit.dot(aikit.permute_dims(projection, (1, 0)), projection)
            if (
                aikit.max(
                    aikit.abs(inner_product - aikit.eye(rank, dtype=inner_product[0].dtype))
                )
                > 1e-5
            ):
                raise ValueError(
                    ...
                    " T.eye(rank)) = "
                    f"""{aikit.sqrt(aikit.sum(aikit.square(inner_product -
                                  aikit.eye(rank,dtype=inner_product[0].dtype)),
                                    axis=0))}"""
                )
        ...
        for i, factor in enumerate(factors[1:]):
            current_mode_size, current_rank = aikit.shape(factor)
            if current_rank != rank:
                raise ValueError(
        ...
        if weights is not None and aikit.shape(weights)[0] != rank:
            raise ValueError(
                f"Given factors for a rank-{rank} PARAFAC2 tensor but"
                f" len(weights)={aikit.shape(weights)[0]}."
            )
    ...
    def parafac2_normalise(parafac2_tensor):
        ...
        _, rank = aikit.Parafac2Tensor.validate_parafac2_tensor(parafac2_tensor)
        weights, factors, projections = parafac2_tensor
        ...
        for i, factor in enumerate(factors):
            ...
            scales_non_zero = aikit.where(
                scales == 0, aikit.ones(aikit.shape(scales), dtype=factors[0].dtype), scales
            )
    ...
    def apply_parafac2_projections(parafac2_tensor):
        ...
        """
        aikit.Parafac2Tensor.validate_parafac2_tensor(parafac2_tensor)
        weights, factors, projections = parafac2_tensor

        evolving_factor = [
            aikit.dot(projection, factors[1]) for projection in projections
        ]
    ...
    def parafac2_to_slice(parafac2_tensor, slice_idx, validate=True):
        ...
        Ct = aikit.permute_dims(C, (1, 0))

        B_i = aikit.dot(projections[slice_idx], B)
        return aikit.dot(B_i * a, Ct)
    ...
    def parafac2_to_slices(parafac2_tensor, validate=True):
        ...
        I, _ = A.shape
        return [
            aikit.Parafac2Tensor.parafac2_to_slice(decomposition, i, validate=False)
            for i in range(I)
        ]
    ...
    def parafac2_to_tensor(parafac2_tensor):
        ...
        slices = aikit.Parafac2Tensor.parafac2_to_slices(parafac2_tensor)
    ...
    def parafac2_to_unfolded(parafac2_tensor, mode):
        ...
        -------
            Full constructed tensor. Uneven slices are padded with zeros.
        """
        return aikit.unfold(aikit.Parafac2Tensor.parafac2_to_tensor(parafac2_tensor), mode)
    ...
    def parafac2_to_vec(parafac2_tensor):
        ...
        """
        return aikit.reshape(aikit.Parafac2Tensor.parafac2_to_tensor(parafac2_tensor), (-1))
