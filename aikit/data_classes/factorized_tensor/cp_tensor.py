...
class CPTensor(FactorizedTensor):
    def __init__(self, cp_tensor):
        ...
        shape, rank = aikit.CPTensor.validate_cp_tensor(cp_tensor)
        weights, factors = cp_tensor

        if weights is None:
            weights = aikit.ones(rank, dtype=factors[0].dtype)
    ...
    # ---------------#
    def to_tensor(self):
        return aikit.CPTensor.cp_to_tensor(self)

    def to_vec(self):
        return aikit.CPTensor.cp_to_vec(self)

    def to_unfolded(self, mode):
        return aikit.CPTensor.cp_to_unfolded(self, mode)

    def cp_copy(self):
        return CPTensor(
            (
                aikit.copy_array(self.weights),
                [aikit.copy_array(self.factors[i]) for i in range(len(self.factors))],
            )
        )
    ...
    def mode_dot(self, matrix_or_vector, mode, keep_dim=False, copy=True):
        ...
        """
        return aikit.CPTensor.cp_mode_dot(
    ...
    def norm(self):
        ...
        ----------
        cp_tensor
            aikit.CPTensor or (core, factors)
        ...
        """
        return aikit.CPTensor.cp_norm(self)
    ...
    def normalize(self, inplace=True):
        ...
        """
        weights, factors = aikit.CPTensor.cp_normalize(self)
        if inplace:
    ...
    @property
    def n_param(self):
        factors_params = self.rank * aikit.sum(self.shape)
        if self.weights:
    ...
    def validate_cp_tensor(cp_tensor):
        ...
        if ndim == 2:
            rank = int(aikit.shape(factors[0])[1])
        elif ndim == 1:
            rank = 1
        else:
            raise ValueError(
        ...
        for i, factor in enumerate(factors):
            ...
            if current_rank != rank:
                raise ValueError(
                    ...
                    f" factors[{i}].shape[1]={aikit.shape(factor)[1]}."
                )
        ...
        if weights is not None and len(weights) != rank:
            raise ValueError(
                ...
                f" len(weights)={aikit.shape(weights)}."
            )
    ...
    def cp_n_param(tensor_shape, rank, weights=False):
        ...
        """
        factors_params = rank * aikit.sum(tensor_shape)
        if weights:
    ...
    def validate_cp_rank(tensor_shape, rank="same", rounding="round"):
        ...
        """
        if rounding == "ceil":
            rounding_fun = aikit.ceil
        elif rounding == "floor":
            rounding_fun = aikit.floor
        elif rounding == "round":
            rounding_fun = aikit.round
        else:
            raise ValueError(
        ...
        if isinstance(rank, float):
            rank = int(
                rounding_fun(aikit.prod(tensor_shape) * rank / aikit.sum(tensor_shape))
            )
        return rank
    ...
    def cp_normalize(cp_tensor):
        ...
        CPTensor = (normalisation_weights, normalised_factors)
        """
        _, rank = aikit.CPTensor.validate_cp_tensor(cp_tensor)
        weights, factors = cp_tensor
        ...
        for i, factor in enumerate(factors):
            if i == 0:
                ...
                weights = aikit.ones((rank,), dtype=factor.dtype)

            scales = aikit.sqrt(aikit.sum(aikit.square(factor), axis=0))
            scales_non_zero = aikit.where(
                scales == 0, aikit.ones(aikit.shape(scales), dtype=factor.dtype), scales
            )
            weights = weights * scales
            normalized_factors.append(factor / aikit.reshape(scales_non_zero, (1, -1)))
    ...
    def cp_flip_sign(cp_tensor, mode=0, func=None):
        ...
        CPTensor = (normalisation_weights, normalised_factors)
        """
        aikit.CPTensor.validate_cp_tensor(cp_tensor)
        weights, factors = cp_tensor
        ...
        for jj in range(0, len(factors)):
            ...
            column_signs = aikit.sign(func(factors[jj], axis=0))

            # Update both the current and receiving factor
            factors[mode] = factors[mode] * column_signs[aikit.newaxis, :]
            factors[jj] = factors[jj] * column_signs[aikit.newaxis, :]

        # Check the weight signs
        weight_signs = aikit.sign(weights)
        factors[mode] = factors[mode] * weight_signs[aikit.newaxis, :]
        weights = aikit.abs(weights)
    ...
    def cp_lstsq_grad(cp_tensor, tensor, return_loss=False, mask=None):
        ...
        diff = tensor - aikit.CPTensor.cp_to_tensor(cp_tensor)

        if mask is not None:
            diff = diff * mask

        grad_fac = [
            -aikit.CPTensor.unfolding_dot_khatri_rao(diff, cp_tensor, ii)
            for ii in range(len(factors))
        ]

        if return_loss:
            return CPTensor((None, grad_fac)), 0.5 * aikit.sum(diff**2)
    ...
    def cp_to_tensor(cp_tensor, mask=None):
        ...
        """
        shape, _ = aikit.CPTensor.validate_cp_tensor(cp_tensor)
        ...
        if len(shape) == 1:  # just a vector
            return aikit.sum(weights * factors[0], axis=1)

        if weights is None:
            weights = 1

        if mask is None:
            full_tensor = aikit.matmul(
                factors[0] * weights,
                aikit.permute_dims(aikit.khatri_rao(factors, skip_matrix=0), (1, 0)),
            )
        else:
            full_tensor = aikit.sum(
                aikit.khatri_rao([factors[0] * weights] + factors[1:], mask=mask), axis=1
            )

        return aikit.fold(full_tensor, 0, shape)
    ...
    def cp_to_unfolded(cp_tensor, mode):
        ...
        if weights is not None:
            return aikit.dot(
                ...
                aikit.permute_dims(aikit.khatri_rao(factors, skip_matrix=mode), (1, 0)),
            )
        else:
            return aikit.dot(
                factors[mode],
    ...
    def cp_to_vec(cp_tensor):
        ...
        aikit.Array
            vectorised tensor
        """
        return aikit.reshape(aikit.CPTensor.cp_to_tensor(cp_tensor), (-1))
    ...
    def cp_mode_dot(cp_tensor, matrix_or_vector, mode, keep_dim=False, copy=False):
        ...
        if copy:
            factors = [aikit.copy_array(f) for f in factors]
            weights = aikit.copy_array(weights)

        if contract:
            factor = factors.pop(mode)
            factor = aikit.dot(matrix_or_vector, factor)
            mode = max(mode - 1, 0)
            factors[mode] *= factor
        else:
            factors[mode] = aikit.dot(matrix_or_vector, factors[mode])
    ...
    def cp_norm(cp_tensor):
        ...
        khatria-rao(A, B)^T x khatri-rao(A, B) = A^T x A  * B^T x B
        """
        _ = aikit.CPTensor.validate_cp_tensor(cp_tensor)
        weights, factors = cp_tensor
        ...
        for f in factors:
            norm = norm * aikit.dot(aikit.permute_dims(f, (1, 0)), aikit.conj(f))

        if weights is not None:
            # norm = T.dot(T.dot(weights, norm), weights)
            norm = norm * (
                aikit.reshape(weights, (-1, 1)) * aikit.reshape(weights, (1, -1))
            )

        return aikit.sqrt(aikit.sum(norm))

    # uncomment when aikit.congruence_coefficient has been implemented
    ...
    #            tensors_to_permute[i] = aikit.CPTensor.cp_normalize(tensors_to_permute[i]) # noqa
    #    ref_cp_tensor = aikit.CPTensor.cp_normalize(ref_cp_tensor)
    #    n_tensors = len(tensors_to_permute)
    #    n_factors = len(ref_cp_tensor.factors)
    #    permutation = []
    #    for i in range(n_tensors):
    #        _, col = aikit.congruence_coefficient(
    #            ref_cp_tensor.factors, tensors_to_permute[i].factors
    #        )
    #        col = aikit.array(col, dtype=aikit.int64)
    ...
    def unfolding_dot_khatri_rao(x, cp_tensor, mode):
        ...
        mttkrp_parts = []
        weights, factors = cp_tensor
        rank = aikit.shape(factors[0])[1]
        for r in range(rank):
            component = aikit.multi_mode_dot(
                x, [aikit.conj(f[:, r]) for f in factors], skip=mode
            )
            mttkrp_parts.append(component)

        if weights is None:
            return aikit.stack(mttkrp_parts, axis=1)
        else:
            return aikit.stack(mttkrp_parts, axis=1) * aikit.reshape(weights, (1, -1))
