...
class _ArrayWithLossesExperimental(abc.ABC):
    ...
    def hinge_embedding_loss(
        ...
        *,
        ...
        >>> input_tensor = aikit.array([1, 2, 3], dtype=aikit.float64)
        >>> target_tensor = aikit.array([1, -1, -1], dtype=aikit.float64)
        >>> input_tensor.hinge_embedding_loss(target_tensor, margin=2.0)
        aikit.array(0.33333333)
        """
        return aikit.hinge_embedding_loss(
            self._data,
            target,
            margin=margin,
            reduction=reduction,
