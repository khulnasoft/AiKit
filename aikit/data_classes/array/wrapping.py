...
def _wrap_function(function_name: str) -> Callable:
    ...
    >>> from aikit.array.wrapping import _wrap_function
    ...
    def new_function(self, *args, **kwargs):
        ...
        if len(args) >= data_idx[0][0]:
            ...
            aikit.insert_into_nest_at_index(args, data_idx, self._data)
        else:
            kwargs = aikit.copy_nest(kwargs, to_mutable=True)
            data_idx = [data_idx[0][1]] + [
                0 if idx is int else idx for idx in data_idx[1:]
            ]
            aikit.insert_into_nest_at_index(kwargs, data_idx, self._data)
        return function(*args, **kwargs)

    return new_function


def add_aikit_array_instance_methods(
    cls: Type[aikit.Array], modules: List[ModuleType], to_ignore: Iterable = ()
):
    """Loop over all aikit modules such as activations, general, etc. and add the
    module functions to aikit arrays as instance methods using _wrap_function.
    ...
    Examples
    --------
    As shown, `add_aikit_array_instance_methods` adds all the appropriate functions from
    the activations module as instance methods to our toy `ArrayExample` class:

    >>> from aikit.functional.aikit import activations
    >>> class ArrayExample:
    ...     pass
    >>> aikit.add_aikit_array_instance_methods(ArrayExample, [activations])
