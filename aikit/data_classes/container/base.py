...
class ContainerBase(dict, abc.ABC):
    def __init__(
        ...
        **kwargs,
        ...
        container_combine_method
            ...
            Default is aikit.Container.cont_list_join
        queue_timeout
        ...
        aikith
            Handle to aikit module to use for the calculations. Default is ``None``, which
            results in the global aikit.
        default_key_color
        ...
        if aikit.exists(self._queues):
            ...
            self._queue_timeout = aikit.default(queue_timeout, aikit.queue_timeout)
    ...
    def cont_multi_map_in_function(
        ...
        *args,
        ...
        out=None,
        **kwargs,
    ) -> Union[Tuple[aikit.Container, aikit.Container], aikit.Container]:
        inspect_fn = fn
        if isinstance(fn, str):
            inspect_fn = aikit.__dict__[fn]
        # retrieve indices where leaves of args are also nested
        arg_cont_idxs = aikit.nested_argwhere(
            args, aikit.is_aikit_container, to_ignore=aikit.Container
        )
        # retrieve indices where leaves of kwargs are also nested
        kwarg_cont_idxs = aikit.nested_argwhere(
            kwargs, aikit.is_aikit_container, to_ignore=aikit.Container
        )

        # retrieve all the containers in args
        arg_conts = aikit.multi_index_nest(args, arg_cont_idxs)
        num_arg_conts = len(arg_conts)

        # retrieve all the containers in kwargs
        kwarg_conts = aikit.multi_index_nest(kwargs, kwarg_cont_idxs)
        ...
        if with_out:
            ...
            num_out_conts = 1
            out_cont_idxs = []
            if not aikit.is_array(out) and not aikit.is_aikit_container(out):
                out_cont_idxs = aikit.nested_argwhere(
                    out, aikit.is_aikit_container, to_ignore=aikit.Container
                )
                out_conts = aikit.multi_index_nest(out, out_cont_idxs)
        ...
        cont0 = conts[0]
        if isinstance(fn, str):
            fn = cont0.cont_aikit.__dict__[fn]
        # Get the function with the name fn_name, enabling containers to specify
        # their backends irrespective of global aikit's backend
        ...
        def map_fn(vals, _):
            ...
            a = aikit.copy_nest(args)
            a = aikit.set_nest_at_indices(a, arg_cont_idxs, arg_vals)
            kwarg_vals = vals[num_arg_conts:]
            kw = aikit.copy_nest(kwargs)
            kw = aikit.set_nest_at_indices(kw, kwarg_cont_idxs, kwarg_vals)
            if with_out:
        ...
        # as inputs
        ret = aikit.Container.cont_multi_map(
            map_fn,
            conts,
            key_chains,
            to_apply,
        ...
        if aikit.is_aikit_container(ret):
            for values in ret.values():
                if isinstance(values, (tuple, list)):
                    for v in values:
                        if aikit.is_aikit_array(v):
                            return ret.cont_unstack_conts(0)

        if with_out:
            for out_cont_idx, out_cont in zip(out_cont_idxs, out_conts):
                out_cont.cont_inplace_update(aikit.index_nest(ret, out_cont_idx))
            if len(out_conts) == 1:
    ...
    def cont_list_join(containers, config=None):
        ...
        if not aikit.exists(config):
            config = (
                container0.cont_config if isinstance(container0, aikit.Container) else {}
            )

        if isinstance(container0, aikit.Container):
            return_dict = {}
            for key in container0.keys():
                new_list = []
                for container in containers:
                    new_list.append(container[key])
                return_dict[key] = aikit.Container.cont_list_join(new_list, config)
            return aikit.Container(return_dict, **config)
        else:
    ...
    def cont_list_stack(containers, dim, config=None):
        ...
        if isinstance(container0, aikit.Container):
            return_dict = {}
            for key in container0.keys():
                return_dict[key] = aikit.Container.cont_list_stack(
    ...
    def _cont_mean_unify(containers, device, _=None, _1=None):
        return aikit.Container._cont_sum_unify(containers, device) / len(containers)
    ...
    def cont_unify(containers, device, mode, axis=0):
        ...
        """
        return {
            "concat": aikit.Container._cont_concat_unify,
            "sum": aikit.Container._cont_sum_unify,
            "mean": aikit.Container._cont_mean_unify,
    ...
    def cont_combine(*containers, config=None):
        ...
        if not aikit.exists(config):
            ...
            config = (
                ...
                if isinstance(container_rightmost, aikit.Container)
                else {}
            )
        ...
        for key in all_keys:
            ...
            return_dict[key] = aikit.Container.cont_combine(
    ...
    def cont_diff(
        ...
        *containers
        ...
        -------
            Compared containers
        """
        aikit.utils.assertions.check_elem_in_list(mode, ["all", "same_only", "diff_only"])
        ...
        if not isinstance(container0, dict):
            equal_mat = aikit.all_equal(*containers, equality_matrix=True)
            if not detect_value_diffs:
                equal_mat = aikit.ones_like(equal_mat)
            if detect_shape_diffs:
                shape_equal_mat = aikit.all_equal(
                    *[c.shape if aikit.is_array(c) else None for c in containers],
                    equality_matrix=True,
                )
                equal_mat = aikit.logical_and(equal_mat, shape_equal_mat)
            # noinspection PyTypeChecker
            if aikit.min(aikit.astype(equal_mat, "int32")) == 1:
                if mode == "diff_only":
                    return aikit.Container(**config)
                return container0
            elif mode == "same_only":
                return aikit.Container(**config)
            else:
                ...
                for idx in cont_range:
                    if idx not in idxs_added:
                        idxs_to_add = aikit.argwhere(equal_mat[idx])
                        idxs_to_add_list = sorted(
                            aikit.to_numpy(idxs_to_add).reshape(-1).tolist()
                        )
                ...
                return aikit.Container(diff_dict, **config)
    ...
    def cont_structural_diff(
        ...
        *containers
        ...
        -------
            Compared containers
        """
        return aikit.Container.cont_diff(
            *containers,
            mode=mode,
            diff_keys=diff_keys,
            detect_key_diffs=detect_key_diffs,
    ...
    def cont_multi_map(
        ...
        container0 = None
        for container in containers:
            if isinstance(container, aikit.Container):
        ...
        aikit.utils.assertions.check_exists(
            container0,
            message="No containers found in the inputs to aikit.Container.cont_multi_map",
        )
        if not aikit.exists(config):
            config = (
                container0.cont_config if isinstance(container0, aikit.Container) else {}
        ...
        for key in keys:
            values = []
            for cont in containers:
                if isinstance(cont, (aikit.Container, list, tuple)) and key in cont:
                    values.append(cont[key])
                elif not isinstance(cont, (aikit.Container, list, tuple)):
                    values.append(cont)
            value0 = values[0]
            if len(values) >= 1:
                this_key_chain = key if key_chain == "" else (key_chain + "/" + key)
                is_container = [aikit.is_aikit_container(x) for x in values]
                ...
                if not assert_identical and not all(is_container) and any(is_container):
                    ...
                    if isinstance(value0, aikit.Container):
                        ret = aikit.Container.cont_multi_map(
                            func,
                            values,
                            key_chains,
                        ...
                        ret = aikit.nested_multi_map(
                            lambda x, _: func(x, None), values, to_aikit=False
                        )
    ...
    def cont_identical(
        ...
        """
        if partial:
            common_key_chains = aikit.Container.cont_common_key_chains(containers)
        ...
        for key in keys:
            ...
            if not min(type_n is type_0 for type_n in types):
                if isinstance(value_0, aikit.Container) or check_types:
                    return False
            if aikit.is_array(value_0):
                if check_shapes:
            ...
            if isinstance(value_0, aikit.Container):
                ret = aikit.Container.cont_identical(
                    values,
                    check_types,
                    check_shapes,
                    same_arrays,
    ...
    def cont_assert_identical(
        ...
        aikit.utils.assertions.check_true(
            aikit.Container.cont_identical(
                containers,
                check_types,
                check_shapes,
                same_arrays,
            ...
            f"Containers were not identical:\n\n{aikit.Container.cont_diff(*containers)}",
        )
    ...
    def cont_identical_structure(
        ...
        -------
            Boolean
        """
        return aikit.Container.cont_identical(
            containers,
            check_types,
            check_shapes,
            False,
    ...
    def cont_assert_identical_structure(
        ...
        """
        aikit.utils.assertions.check_true(
            aikit.Container.cont_identical_structure(
                containers,
                check_types,
                check_shapes,
                key_chains,
            ...
            f" structure:\n\n{aikit.Container.cont_structural_diff(*containers)}",
        )
    ...
    def cont_identical_configs(containers):
        ...
        containers
            containers to check.
        """
        aikit.utils.assertions.check_greater(len(containers), 1, as_array=False)
    ...
    def cont_load(filepath, format="h5py"):
        if format == "json":
            return aikit.Container.cont_from_disk_as_json(filepath)
        elif format == "pickle":
            return aikit.Container.cont_from_disk_as_pickled(filepath)
        elif format == "h5py":
    ...
    def cont_from_disk_as_hdf5(
        h5_obj_or_filepath, slice_obj=slice(None), alphabetical_keys=True, aikith=None
    ):
        ...
        for key, value in items:
            if isinstance(value, h5py.Group):
                container_dict[key] = aikit.Container.cont_from_disk_as_hdf5(
                    value, slice_obj, aikith
                )
            elif isinstance(value, h5py.Dataset):
                container_dict[key] = aikit.default(aikith, aikit).array(
    ...
    @staticmethod
    def cont_from_disk_as_pickled(pickle_filepath, aikith=None):
    ...
    @staticmethod
    def cont_from_disk_as_json(json_filepath, aikith=None):
        ...
        with open(json_filepath) as json_data_file:
            return aikit.Container(json.load(json_data_file), aikith=aikith)
    ...
    def h5_file_size(h5_obj_or_filepath):
        ...
        for key, value in h5_obj.items():
            if isinstance(value, h5py.Group):
                size_to_add, batch_size = aikit.Container.h5_file_size(value)
                size += size_to_add
            elif isinstance(value, h5py.Dataset):
    ...
    def shuffle_h5_file(h5_obj_or_filepath, seed_value=0):
        ...
        for key, value in h5_obj.items():
            if isinstance(value, h5py.Group):
                aikit.Container.shuffle_h5_file(value, seed_value)
            elif isinstance(value, h5py.Dataset):
    ...
    def cont_reduce(containers, reduction, config=None):
        ...
        if isinstance(container0, aikit.Container):
            return_dict = {}
            for key in container0.keys():
                return_dict[key] = aikit.Container.cont_reduce(
    ...
    def cont_trim_key(key, max_length):
        ...
        """
        key_len = len(key)
        if not aikit.exists(max_length) or key_len <= max_length:
            return key
        idxs = (
            np.round(
                (key_len - 1)
    ...
    def _cont_get_shape(self):
        ...
        sub_shapes = [
            ...
            for k, v in self.cont_map(
                lambda x, kc: (
                    list(x.shape)
                    if self._cont_aikit.is_native_array(x) or isinstance(x, aikit.Array)
    ...
    def _cont_get_dev(self, as_native=False):
        sub_devs = [
            ...
            for k, v in self.cont_map(
                lambda x, kc: (
                    self._cont_aikit.dev(x, as_native=as_native)
                    if self._cont_aikit.is_native_array(x) or isinstance(x, aikit.Array)
                    else None
                )
            ).cont_to_iterator()
    ...
    def _cont_at_key_chains_input_as_seq(self, key_chains, ignore_key_errors=False):
        return_cont = aikit.Container({}, **self._config)
        for kc in key_chains:
            val = self.cont_at_key_chain(kc, ignore_key_errors=ignore_key_errors)
            if ignore_key_errors and not aikit.exists(val):
                continue
        ...
        for k, v in key_chains.items():
            ...
            if isinstance(v, dict):
                ...
                if ignore_key_errors and not aikit.exists(val):
                    continue
                return_dict[k] = val
        return aikit.Container(return_dict, **self._config)
        ...
        for k, v in config.items():
            ...
            if k in self._config_in:
                if k == "types_to_iteratively_nest":
                    v = aikit.default(lambda: tuple(v), (), catch_exceptions=True)
                elif k == "keyword_color_dict":
                    v = aikit.default(v, {})
                elif k == "aikith":
                    att_name = "_local_aikit"
                new_config[k] = v
    ...
    def cont_inplace_update(
        self, dict_in: Union[aikit.Container, dict], **config
    ) -> aikit.Container:
        ...
        **config
        ...
        if dict_in is None:
            return
        dict_types = tuple([dict] + aikit.container_types())
        ...
        for key, value in items:
            if (
                ...
                and (
                    not isinstance(value, aikit.Container)
                    or self._rebuild_child_containers
                )
            ) or isinstance(value, tuple(self._types_to_iteratively_nest)):
                self[key] = aikit.Container(value, **self._config)
            elif key in self and isinstance(self[key], aikit.Container):
    ...
    def cont_slice_via_key(self, slice_key):
        ...
        for key, value in self.items():
            if key == slice_key:
                return value
            elif isinstance(value, aikit.Container):
    ...
    def cont_as_bools(
        ...
        def _ret_bool(x):
            if assert_is_bool:
                aikit.utils.assertions.check_isinstance(x, bool)
                return x
            return bool(x)
    ...
    def cont_num_arrays(self, exclusive=False):
        ...
        return sum(
            self.cont_map(
                lambda x, kc: aikit.is_array(x, exclusive=exclusive)
    ...
    def cont_size_ordered_arrays(self, exclusive=False):
        ...
        """
        array_dict = {
            aikit.Container.cont_flatten_key_chain(kc): v
            for kc, v in self.cont_to_iterator()
            if aikit.is_array(v, exclusive=exclusive)
        }
        return aikit.Container(
            dict(
                sorted(
    ...
    def cont_to_disk_as_hdf5(
        ...
        if type(h5_obj_or_filepath) is str:
            ...
            h5_obj = h5_obj_or_filepath
        for key, value in self.items():
            if isinstance(value, aikit.Container):
                ...
                value_as_np = self._cont_aikit.to_numpy(value)
    ...
    def cont_to_raw(self):
        ...
        for key, value in self.items():
            if isinstance(value, aikit.Container):
                ...
                return_item = [
                    v.cont_to_raw() if isinstance(v, aikit.Container) else v
    ...
    def cont_to_iterator(self, key_chain="", leaf_keys_only=False, include_empty=False):
        ...
        for key, value in self.items():
            ...
            if isinstance(value, aikit.Container) and (not include_empty or value):
    ...
    def cont_from_flat_list(self, flat_list):
        ...
        return aikit.Container(new_dict, **self._config)
    ...
    def cont_find_sub_container(self, sub_cont_to_find, partial=False):
        ...
        def _check_sub_cont(sub_cont, kc):
            ...
            if (
                ...
                and aikit.Container.cont_identical(
    ...
    def cont_find_sub_structure(
        ...
        def _check_sub_cont(sub_cont, kc):
            ...
            if (
                kcs_in_sub_cont
                and min(kcs_in_sub_cont)
                and aikit.Container.cont_identical_structure(
    ...
    def cont_set_at_keys(self, target_dict):
        ...
        for key, val in self.items():
            ...
            elif isinstance(val, aikit.Container):
    ...
    def cont_set_at_key_chain(self, key_chain, val, inplace=False):
        ...
        for key in keys[:-1]:
            if key not in sub_cont:
                sub_cont[key] = aikit.Container(**self._config)
        ...
        for key, value in self.items():
            if isinstance(value, aikit.Container):
                ...
                if len(keys_in_chain) != 1 or key != keys_in_chain[0]:
                    out_dict[key] = value
        return aikit.Container(out_dict, **self._config)
    ...
    def cont_format_key_chains(self, format_fn):
        ...
        """
        return aikit.Container({format_fn(k): v for k, v in self.cont_to_iterator()})

    def cont_sort_by_key(self):
        new_dict = {}
        for k, v in self.items():
            if isinstance(v, aikit.Container):
    ...
    def cont_prune_empty(self, keep_nones=False, base=True):
        ...
        for key, value in self.items():
            ...
            elif self._cont_aikit.exists(value) or keep_nones:
                out_dict[key] = value
        if len(out_dict):
            return aikit.Container(out_dict, **self._config)
        if base:
            return aikit.Container(**self._config)
        return
    ...
    def cont_prune_key_from_key_chains(self, absolute=None, containing=None):
        ...
        out_cont = aikit.Container(**self._config)
        for key, value in self.items():
            if (absolute and key == absolute) or (containing and containing in key):
                if isinstance(value, aikit.Container):
                    out_cont = aikit.Container.cont_combine(out_cont, value)
                else:
                    out_cont = value
            elif isinstance(value, aikit.Container):
    ...
    def cont_restructure_key_chains(
        ...
        """
        new_cont = self.cont_copy() if keep_orig else aikit.Container()
        for old_kc, new_kc in keychain_mapping.items():
            ...
            new_cont = aikit.Container.cont_combine(
                new_cont, aikit.Container({new_kc: self[old_kc]})
            )
        return new_cont
        ...
        for old_kc, new in mapping.items():
            ...
            if isinstance(new, dict):
                ...
                if "pattern" in new:
                    ...
                    if isinstance(val, aikit.Container):
                        val = val.einops_rearrange(pattern, **axes_lengths)
                    else:
                        val = aikit.einops_rearrange(val, pattern, **axes_lengths)
            else:
                new_kc = new
            new_cont = aikit.Container.cont_combine(
                new_cont, aikit.Container({new_kc: val})
            )
        return new_cont
    ...
    def cont_flatten_key_chains(
        ...
        return aikit.Container(
            {
                aikit.Container.cont_flatten_key_chain(
    ...
    def cont_copy(self):
        ...
        """
        return aikit.Container(self.cont_to_dict(), **self._config)
    ...
    def cont_deep_copy(self):
        ...
        return self.cont_map(
            lambda x, kc: (
                aikit.copy_array(x) if aikit.is_array(x) and not isinstance(x, str) else x
            )
        )
    ...
    def cont_map_sub_conts(
        ...
        for key, value in self.items():
            ...
            if isinstance(value, aikit.Container):
                ...
                return_dict[key] = value
        ret = return_dict if inplace else aikit.Container(return_dict, **self._config)
    ...
    def cont_reshape_like(self, target_dict, leading_shape=None, return_cont=None):
        ...
        """
        leading_shape = self._cont_aikit.default(leading_shape, [])
        if return_cont is None:
            return_cont = self.cont_copy()
        for (_, v_shape), (k, v) in zip(target_dict.items(), return_cont.items()):
            if isinstance(v_shape, dict):
                ...
                return_cont[k] = self._cont_aikit.reshape(
                    v, leading_shape + list(v_shape)
                )
        return aikit.Container(return_cont, **self._config)
    ...
    def _cont_slice_keys(self, key_slice):
        ...
        if isinstance(key_slice, str):
            aikit.utils.assertions.check_true(len(key_slice) == 3 and key_slice[1] == ":")
            aikit.utils.assertions.check_true(self._alphabetical_keys)
    ...
    def cont_with_aikit_backend(self, aikit_backend: str, inplace=False):
        """Summary.
        ...
        """
        if inplace:
            self._cont_aikit = aikit_backend
            self._config["aikith"] = aikit_backend
            return self
        else:
            return aikit.Container(self, aikith=aikit_backend)
    ...
    def cont_show_sub_container(self, sub_cont_or_keychain):
        ...
        this_cont[sub_cont_kc] = aikit.Container({"SUB_CONT": None})
    ...
    def __repr__(self, as_repr=True):
        ...
        def _align_array(array_str_in):
            ...
            split_phrase = split_phrase_dict[self._cont_aikit.current_backend_str()]
        ...
        for k, v in self.items():
            if isinstance(v, aikit.Container):
                ...
                if (
                    (self._cont_aikit.is_native_array(v) or isinstance(v, aikit.Array))
                    and len(list(v.shape)) > 0
                    and aikit.exists(self._print_limit)
                    ...
                    and v
                    and (
                        self._cont_aikit.is_native_array(v[0])
                        or isinstance(v[0], aikit.Array)
                    )
                ):
                    if (
                        isinstance(v, tuple)
        ...
        if as_repr:
            json_dumped_str = _align_arrays(
                json.dumps(
                    aikit.Container(new_dict, **self._config)
                    .cont_map(
                        lambda x, kc: (
                            x
                            if _is_jsonable(x)
            ...
            if aikit.backend_stack and aikit.current_backend_str() == "tensorflow":
    ...
    def _get_queue_item(self, query):
        if isinstance(query, int):
            ...
            queue_queries = list(
                range(query.start, query.stop, aikit.default(query.step, 1))
            )
    ...
    def __getitem__(self, query):
        ...
        if isinstance(query, str):
            ...
            return ret
        elif aikit.exists(self._queues):
        ...
        for key, value in self.items():
            if isinstance(value, aikit.Container):
                ...
                if isinstance(value, (list, tuple)):
                    ...
                    return_dict[key] = value[query]
        ret = aikit.Container(return_dict, **self._config)
        return ret
    ...
    def __setitem__(self, query, val):
        ...
        val aikit.Container, array, or other
    ...
    def __contains__(self, key):
        ...
        elif isinstance(key, aikit.Container):
    ...
    def __getstate__(self):
        ...
        state_dict["_local_aikit"] = (
            state_dict["_local_aikit"].current_backend_str()
            if state_dict["_local_aikit"] is not None
            else None
        )
        config_in = copy.copy(state_dict["_config_in"])
        config_in["aikith"] = (
            config_in["aikith"].current_backend_str()
            if config_in["aikith"] is not None
            else None
        )
        state_dict["_config_in"] = config_in
        config = copy.copy(state_dict["_config"])
        config["aikith"] = (
            config["aikith"].current_backend_str()
            if getattr(config, "aikith", None) is not None
            else None
        )
    ...
    def __setstate__(self, state_dict):
        if "_local_aikit" in state_dict and aikit.exists(state_dict["_local_aikit"]):
            if len(state_dict["_local_aikit"]) > 0:
                state_dict["_local_aikit"] = aikit.with_backend(state_dict["_local_aikit"])
            else:
                state_dict["_local_aikit"] = aikit
        if "_config_in" in state_dict:
            config_in = copy.copy(state_dict["_config_in"])
            if "aikith" in config_in and aikit.exists(config_in["aikith"]):
                if len(config_in["aikith"]) > 0:
                    config_in["aikith"] = aikit.with_backend(config_in["aikith"])
                else:
                    config_in["aikith"] = aikit
            state_dict["_config_in"] = config_in
        if "_config" in state_dict:
            config = copy.copy(state_dict["_config"])
            if "aikith" in config and aikit.exists(config["aikith"]):
                if len(config["aikith"]) > 0:
                    config["aikith"] = aikit.with_backend(config["aikith"])
                else:
                    config["aikith"] = aikit
            state_dict["_config"] = config
    ...
    @property
    def _cont_aikit(self):
        return aikit.default(self._local_aikit, aikit)

    @_cont_aikit.setter
    def _cont_aikit(self, local_aikit):
        self._local_aikit = local_aikit
