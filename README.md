...
After installing Aikit, you can start using it straight away, for example:
  ...
  <details>
   ...
   torch_fn = aikit.transpile(jax_fn, source="jax", to="torch", args=(jax_x,))
...
torch_eff_encoder = aikit.transpile(eff_encoder, to="torch", args=(noise,))
...
torch_sm = aikit.transpile(sm, source="tensorflow", to="torch")
...
torch_rax = aikit.transpile(rax, source="jax", to="torch")
...
torch_madmom = aikit.transpile(madmom, source="numpy", to="torch")
...
torch_loss = aikit.transpile(loss, source="tensorflow", to="torch")
...
torch_loss = aikit.transpile(loss, source="jax", to="torch")
...
torch_loss = aikit.transpile(loss, source="numpy", to="torch")
...
mlp_encoder = aikit.transpile(mlp_encoder, to="tensorflow", args=(noise,))
...
tf_kornia = aikit.transpile(kornia, source="torch", to="tensorflow")
...
tf_rax = aikit.transpile(rax, source="jax", to="tensorflow")
...
tf_madmom = aikit.transpile(madmom, source="numpy", to="tensorflow")
...
tf_loss = aikit.transpile(loss, source="torch", to="tensorflow")
...
tf_loss = aikit.transpile(loss, source="jax", to="tensorflow")
...
tf_loss = aikit.transpile(loss, source="numpy", to="tensorflow")
...
# Transpile it into a hk.Module with the corresponding parameters
noise = tf.random.normal(shape=(1, 224, 224, 3))
hk_eff_encoder = aikit.transpile(eff_encoder, to="jax", args=(noise,))
...
jax_sm = aikit.transpile(sm, source="tensorflow", to="jax")
...
jax_madmom = aikit.transpile(madmom, source="numpy", to="jax")
...
jax_loss = aikit.transpile(loss, source="torch", to="jax")
...
jax_loss = aikit.transpile(loss, source="tensorflow", to="jax")
...
jax_loss = aikit.transpile(loss, source="numpy", to="jax")
...
np_kornia = aikit.transpile(kornia, source="torch", to="numpy")
...
np_sm = aikit.transpile(sm, source="tensorflow", to="numpy")
...
np_rax = aikit.transpile(rax, source="jax", to="numpy")
...
np_loss = aikit.transpile(loss, source="torch", to="numpy")
...
np_loss = aikit.transpile(loss, source="tensorflow", to="numpy")
...
np_loss = aikit.transpile(loss, source="jax", to="numpy")
...
class AikitNet(aikit.Module):
    ...
    def _build(self, *args, **kwargs):
        self.extractor = aikit.Sequential(
            aikit.Conv2D(self.input_channels, 6, [5, 5], 1, "SAME", data_format=self.data_format),
            aikit.GELU(),
            aikit.Conv2D(6, 16, [5, 5], 1, "SAME", data_format=self.data_format),
            aikit.GELU(),
            aikit.Conv2D(16, self.output_channels, [5, 5], 1, "SAME", data_format=self.data_format),
            aikit.GELU(),
        )

        self.classifier = aikit.Sequential(
            # Since the padding is "SAME", this would be image_height x image_width x output_channels
            aikit.Linear(self.h_w[0] * self.h_w[1] * self.output_channels, 512),
            aikit.GELU(),
            aikit.Linear(512, self.num_classes),
        )

    def _forward(self, x):
        x = self.extractor(x)
        # flatten all dims except batch dim
        x = aikit.flatten(x, start_dim=1, end_dim=-1)
...
Last but not least, we can also build the training pipeline in pure aikit
⬇️
...
def loss_fn(params):
    ...
    return aikit.cross_entropy(y, probs), probs
```
...
device = "cuda:0" if aikit.gpu_is_available() else "cpu"
...
def train(images, classes, epochs, model, device, num_classes=10, batch_size=32):
    ...
    for epoch in range(epochs):
        ...
        for xbatch, ybatch in train_loop:
            ...
            ybatch_encoded = aikit.one_hot(ybatch, num_classes)

            # update model params
            loss_probs, grads = aikit.execute_with_gradients(
                loss_fn,
            ...
            batch_loss = aikit.to_numpy(loss_probs[0]).mean().item()  # batch mean loss
...
class Regressor(aikit.Module):
    ...
    def _build(self, *args, **kwargs):
        self.linear0 = aikit.Linear(self.input_dim, 128)
        self.linear1 = aikit.Linear(128, self.output_dim)

    def _forward(self, x):
        x = self.linear0(x)
        x = aikit.functional.relu(x)
        x = self.linear1(x)
        return x
```
...
aikit.set_backend('torch')  # set backend to PyTorch (or any other backend!)

model = Regressor(input_dim=1, output_dim=1)
optimizer = aikit.Adam(0.3)

n_training_examples = 2000
noise = aikit.random.random_normal(shape=(n_training_examples, 1), mean=0, std=0.1)
x = aikit.linspace(-6, 3, n_training_examples).reshape((n_training_examples, 1))
y = 0.2 * x ** 2 + 0.5 * x + 0.1 + noise


def loss_fn(v, x, target):
    pred = model(x, v=v)
    return aikit.mean((pred - target) ** 2)
...
for epoch in range(40):
    ...
    loss, grads = aikit.execute_with_gradients(lambda params: loss_fn(*params), (model.v, x, y))

    # update parameters
    model.v = optimizer.step(model.v, grads)

    # print current loss
    print(f'Epoch: {epoch + 1:2d} --- Loss: {aikit.to_numpy(loss).item():.5f}')
