...
from aikit_tests.test_aikit.helpers import handle_method, handle_test, BackendHandler
from aikit_tests.test_aikit.test_functional.test_core.test_elementwise import (
    not_too_close_to_zero,
    pow_helper,
)
from aikit_tests.test_aikit.test_functional.test_core.test_linalg import (
    _get_first_matrix_and_dtype,
    _get_second_matrix_and_dtype,
)

CLASS_TREE = "aikit.array"
...
def test_array__ipow__(
    ...
    assume(
        not (aikit.is_int_dtype(input_dtype[0] and aikit.is_float_dtype(input_dtype[1])))
    )

    # Make sure x2 is non-negative when both is integer
    if aikit.is_int_dtype(input_dtype[1]) and aikit.is_int_dtype(input_dtype[0]):
        x[1] = np.abs(x[1])
...
def test_array_function():
    ...
    class MyArray:
        ...
        def __aikit_array_function__(self, func, types, args, kwargs):
            ...
            if not all(
                issubclass(t, (MyArray, aikit.Array, aikit.NativeArray)) for t in types
            ):
                return NotImplemented
            return HANDLED_FUNCTIONS[func](*args, **kwargs)

    def implements(aikit_function):
        """Register an __aikit_array_function__ implementation for MyArray
        objects."""
    ...
    def _(my_array, aikit_array):
        my_array.data = abs(my_array.data)
        aikit_array = aikit.abs(aikit_array)
        return (my_array, aikit_array)

    x = MyArray(-3)
    y = aikit.array([1, -1])
    xy = _(x, aikit_array=y)
    x1 = xy[0]
    y1 = xy[1]
    assert x1.data == 3
    assert all(y1 == aikit.array([1, 1]))


@handle_test(
    fn_tree="functional.aikit.native_array",  # dummy fn_tree
...
def test_array_property_T(
    ...
    with BackendHandler.update_backend(backend_fw) as aikit_backend:
        data = aikit_backend.native_array(data[0])
        x = aikit_backend.Array(data)
        ret = helpers.flatten_and_to_np(ret=x.T, backend=backend_fw)
        ret_gt = helpers.flatten_and_to_np(
            ret=aikit_backend.matrix_transpose(data), backend=backend_fw
        )
        helpers.value_test(
            ret_np_flat=ret,
...
def test_array_property_device(
    ...
    with BackendHandler.update_backend(backend_fw) as aikit_backend:
        data = aikit_backend.native_array(data[0])
        x = aikit_backend.Array(data)
        aikit_backend.utils.assertions.check_equal(
            x.device, aikit_backend.dev(data), as_array=False
        )
...
def test_array_property_dtype(
    ...
    with BackendHandler.update_backend(backend_fw) as aikit_backend:
        ...
        x = aikit_backend.Array(data)
        aikit_backend.utils.assertions.check_equal(
            x.dtype, aikit_backend.dtype(data), as_array=False
        )
...
def test_array_property_imag(
    ...
    with BackendHandler.update_backend(backend_fw) as aikit_backend:
        ...
        ret_gt = helpers.flatten_and_to_np(ret=aikit_backend.imag(x), backend=backend_fw)
        helpers.value_test(
            ret_np_flat=ret,
            ret_np_from_gt_flat=ret_gt,
            backend=backend_fw,
...
def test_array_property_itemsize(
    ...
    with BackendHandler.update_backend(backend_fw) as aikit_backend:
        ...
        x = aikit_backend.Array(data)
        aikit_backend.utils.assertions.check_equal(
            x.itemsize, aikit_backend.to_numpy(x).itemsize, as_array=False
        )
...
def test_array_property_real(
    ...
    with BackendHandler.update_backend(backend_fw) as aikit_backend:
        ...
        ret_gt = helpers.flatten_and_to_np(ret=aikit_backend.real(x), backend=backend_fw)
        helpers.value_test(
            ret_np_flat=ret,
            ret_np_from_gt_flat=ret_gt,
            backend=backend_fw,
...
def test_array_property_shape(
    ...
    with BackendHandler.update_backend(backend_fw) as aikit_backend:
        ...
        x = aikit_backend.Array(data)
        aikit_backend.utils.assertions.check_equal(
            x.shape, aikit_backend.Shape(input_shape), as_array=False
        )
...
def test_array_property_size(
    ...
    with BackendHandler.update_backend(backend_fw) as aikit_backend:
        ...
        for dim in input_shape:
            size_gt *= dim
        aikit_backend.utils.assertions.check_equal(x.size, size_gt, as_array=False)
...
def test_array_property_strides(dtype_x, backend_fw):
    ...
    with BackendHandler.update_backend(backend_fw) as aikit_backend:
        ...
        x = aikit_backend.Array(data)
        aikit_backend.utils.assertions.check_equal(
            x.strides, aikit_backend.to_numpy(x).strides, as_array=False
        )
...
def test_dunder_wrapping(
    ...
    x = aikit.to_native(aikit.array(data[0]))
    y = aikit.array(data[1])
    assert aikit.is_aikit_array(y)
    assert aikit.is_native_array(x)
    res = eval(f"x {op} y")
    assert aikit.is_aikit_array(res)
