...
def test_pickle_to_and_from_disk(dtype_and_x, on_device, backend_fw):
    ...
    x = aikit.array(x[0], dtype=input_dtype[0], device=on_device)
    ...
    save_filepath = "aikit_array.pickle"
    ...
    # check for equality
    assert np.allclose(aikit.to_numpy(x), aikit.to_numpy(unpickled_arr))
