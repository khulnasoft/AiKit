...
def test_jax_eig(
    *,
    ...
    """Make symmetric positive-definite since aikit does not support complex data
    dtypes currently."""
    x = np.matmul(x.T, x) + np.identity(x.shape[0]) * 1e-3
    ...
    with BackendHandler.update_backend(backend_fw) as aikit_backend:
        ret = [aikit_backend.to_numpy(x).astype(np.float64) for x in ret]
...
def test_jax_eigvals(
    *,
    ...
    with BackendHandler.update_backend(backend_fw) as aikit_backend:
        ...
        ret = np.sort(np.abs(aikit_backend.to_numpy(ret))).astype(np.float64)
