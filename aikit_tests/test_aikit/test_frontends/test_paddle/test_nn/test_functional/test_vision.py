...
def test_paddle_channel_shuffle(
    *,
    ...
    if data_format == "NCHW":
        assume(aikit.shape(x[0])[1] % groups == 0)
    else:
        assume(aikit.shape(x[0])[3] % groups == 0)
    helpers.test_frontend_function(
...
def test_paddle_pixel_shuffle(
    *,
    ...
    input_dtype, x = dtype_and_x
    if data_format == "NCHW":
        assume(aikit.shape(x[0])[1] % (factor**2) == 0)
    else:
        assume(aikit.shape(x[0])[3] % (factor**2) == 0)
    helpers.test_frontend_function(
        input_dtypes=input_dtype,
        frontend=frontend,
        test_flags=test_flags,
...
def test_paddle_pixel_unshuffle(
    *,
    ...
    input_dtype, x = dtype_and_x
    if data_format == "NCHW":
        assume(aikit.shape(x[0])[2] % factor == 0)
        assume(aikit.shape(x[0])[3] % factor == 0)
    else:
        assume(aikit.shape(x[0])[1] % factor == 0)
        assume(aikit.shape(x[0])[2] % factor == 0)
    helpers.test_frontend_function(
        input_dtypes=input_dtype,
        frontend=frontend,
