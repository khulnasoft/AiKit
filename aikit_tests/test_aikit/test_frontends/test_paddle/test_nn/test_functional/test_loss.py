...
def test_paddle_dice_loss(
    ...
    labels = aikit.array(labels, dtype=aikit.int64)
    labels = labels.reshape([3, 3, 1])
    helpers.test_frontend_function(
        input_dtypes=[aikit.int64] + [aikit.float64] + x_dtype,
        backend_to_test=backend_fw,
        frontend=frontend,
        test_flags=test_flags,
        fn_tree=fn_tree,
...
def test_paddle_softmax_with_cross_entropy(
    ...
    ignore_index = 0
    if soft_label:
        labels = labels / aikit.sum(labels).to_native()
    else:
        labels = aikit.argmax(labels, axis=axis).to_native()
        flattened_labels = labels.flatten()
        ignore_index = aikit.randint(0, flattened_labels.size)
