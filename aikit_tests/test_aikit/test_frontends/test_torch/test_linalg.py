...
from aikit_tests.test_aikit.test_frontends.test_torch.test_miscellaneous_ops import (
    dtype_value1_value2_axis,
)
from aikit_tests.test_aikit.helpers.hypothesis_helpers.general_helpers import (
    matrix_is_stable,
)
from aikit_tests.test_aikit.test_functional.test_core.test_linalg import _matrix_rank_helper
...
# eig
# TODO: Test for all valid dtypes once aikit.eig supports complex data types
@handle_frontend_test(
...
def test_torch_eig(
    *,
    ...
    if x.dtype == aikit.float32:
        x = x.astype("float64")
        input_dtype = [aikit.float64]
    ...
    ret = [aikit.to_numpy(x).astype("float64") for x in ret]
...
def test_torch_eigh(
    *,
    ...
    ret = [aikit.to_numpy(x) for x in ret]
...
def test_torch_eigvals(
    *,
    ...
    ret = aikit.to_numpy(ret).astype(str(frontend_ret.dtype))
    ret = np.sort(ret)
