...
import aikit
from aikit.functional.frontends.torch.nn.functional.loss_functions import (
    cosine_similarity,
)
...
def test_torch_cosine_embedding_loss(
    *,
    ...
    if input1.ndim == input2.ndim == 1:
        ...
        ones = aikit.ones(input1.shape[0] - (third * 2))
        minus_ones = aikit.ones(third) * -1
        randoms = aikit.random_uniform(shape=[third])
        tar = aikit.hstack((ones, minus_ones, randoms)).shuffle()
