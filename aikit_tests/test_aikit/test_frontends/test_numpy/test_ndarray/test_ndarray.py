...
# local
import aikit
from aikit.functional.frontends.numpy import ndarray
import aikit_tests.test_aikit.helpers as helpers
from aikit_tests.test_aikit.helpers import (
...
# from aikit.functional.frontends.numpy import ndarray
from aikit_tests.test_aikit.test_frontends.test_numpy.test_mathematical_functions.test_miscellaneous import (  # noqa
    _get_clip_inputs,
)
from aikit_tests.test_aikit.test_frontends.test_numpy.test_mathematical_functions.test_sums_products_differences import (  # noqa
    _get_castable_dtypes_values,
)

from aikit_tests.test_aikit.test_frontends.test_numpy.test_manipulation_routines.test_changing_number_of_dimensions import (  # noqa
    _squeeze_helper,
)

CLASS_TREE = "aikit.functional.frontends.numpy.ndarray"
...
def test_numpy_T(
    ...
    with BackendHandler.update_backend(backend_fw) as aikit_backend:
        x = aikit_backend.functional.frontends.numpy.ndarray(shape, dtype[0])
        x.aikit_array = data[0]
        ret = helpers.flatten_and_to_np(ret=x.T.aikit_array, backend=backend_fw)
        ret_gt = helpers.flatten_and_to_np(
            ret=aikit_backend.permute_dims(
                aikit_backend.native_array(data[0]), list(range(len(shape)))[::-1]
            ),
            backend=backend_fw,
        )
        helpers.value_test(
...
def test_numpy_dtype(dtype_x, backend_fw, frontend):
    ...
    with BackendHandler.update_backend(backend_fw) as aikit_backend:
        ...
        aikit_backend.utils.assertions.check_equal(
            x.dtype, aikit.Dtype(dtype[0]), as_array=False
        )
...
def test_numpy_flat(dtype_x, backend_fw):
    ...
    with BackendHandler.update_backend(backend_fw) as aikit_backend:
        ...
        flat_aikit = x.flat
        flat_aikit = flat_aikit.aikit_array.to_numpy()
        flat_generated = aikit_backend.to_numpy(data[0]).flatten()
        aikit_backend.utils.assertions.check_equal(
            flat_aikit, flat_generated, as_array=True
        )
...
def test_numpy_prod(
    ...
    where = aikit.array(where, dtype="bool")
    helpers.test_frontend_method(
        init_input_dtypes=input_dtypes,
        backend_to_test=backend_fw,
        init_all_as_kwargs_np={
...
def test_numpy_property_ndim(dtype_x, backend_fw):
    ...
    with BackendHandler.update_backend(backend_fw) as aikit_backend:
        x = aikit_backend.functional.frontends.numpy.ndarray(shape, dtype[0])
        x.aikit_array = data[0]
        aikit_backend.utils.assertions.check_equal(x.ndim, data[0].ndim, as_array=False)
...
def test_numpy_shape(
    ...
    with BackendHandler.update_backend(backend_fw) as aikit_backend:
        ...
        x.aikit_array = data[0]
        aikit_backend.utils.assertions.check_equal(
            x.shape, aikit.Shape(shape), as_array=False
        )
...
def test_numpy_size(
    ...
    x.aikit_array = data[0]
    aikit.utils.assertions.check_equal(x.size, data[0].size, as_array=False)
