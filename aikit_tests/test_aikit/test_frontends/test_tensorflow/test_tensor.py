...
from aikit_tests.test_aikit.test_frontends.test_tensorflow.test_raw_ops import (
    _pow_helper_shared_dtype,
)
from aikit.functional.frontends.tensorflow import EagerTensor

CLASS_TREE = "aikit.functional.frontends.tensorflow.EagerTensor"
...
def _check_query(query):
    return not isinstance(query, list) and (
        not (aikit.is_array(query) and aikit.is_bool_dtype(query) ^ bool(query.ndim > 0))
    )
...
def test_tensorflow___array__(
    ...
    with BackendHandler.update_backend(backend_fw) as aikit_backend:
        ...
        function_module = local_importer.import_module(
            "aikit.functional.frontends.tensorflow"
        )
...
def test_tensorflow_device(
    ...
    _, data = dtype_x
    data = aikit.native_array(data[0])
    x = EagerTensor(data)
    aikit.utils.assertions.check_equal(x.device, aikit.dev(data), as_array=False)
...
def test_tensorflow_dtype(
    ...
    aikit.utils.assertions.check_equal(x.dtype, aikit.Dtype(dtype[0]), as_array=False)
    aikit.previous_backend()
...
def test_tensorflow_aikit_array(
    dtype_x,
    backend_fw,
):
    aikit.set_backend(backend_fw)
