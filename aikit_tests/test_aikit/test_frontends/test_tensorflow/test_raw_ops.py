...
from aikit.functional.frontends.tensorflow.nn import _convolution_broadcast_helper
from aikit_tests.test_aikit.test_frontends.test_tensorflow.test_nn import _x_and_filters
import numpy as np
import math
...
def _pow_helper_shared_dtype(draw):
    ...
    x1, x2 = x
    if "int" in dtype2:
        x2 = aikit.nested_map(lambda x: abs(x), x2, include_derived={"list": True})

    if aikit.is_int_dtype(dtype2):
        max_val = aikit.iinfo(dtype2).max
    else:
        max_val = aikit.finfo(dtype2).max
...
@handle_frontend_test(
    fn_tree="tensorflow.raw_ops.Xdaikit",
    dtype_and_x=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtypes("float"),
        num_arrays=2,
        shared_dtype=True,
...
def test_tensorflow_Xdaikit(  # NOQA
    *,
    dtype_and_x,
    frontend,
    test_flags,
