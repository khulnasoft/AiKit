...
def _get_type_dict_helper(framework, kind, is_frontend_test):
    ...
    elif aikit.current_backend_str() == framework:
        framework_module = aikit
    else:
        with BackendHandler.update_backend(framework) as aikit_backend:
            framework_module = aikit_backend
...
def array_dtypes(
    ...
    *,
    ...
    if num_arrays == 1:
        ...
        unwanted_types = set(aikit.all_dtypes).difference(set(available_dtypes))
        if array_api_dtypes:
            pairs = aikit.array_api_promotion_table.keys()
        else:
            pairs = aikit.promotion_table.keys()
...
def cast_filter_helper(d, dtype, x, current_backend):
    with BackendHandler.update_backend(current_backend) as aikit_backend:

        def bound_dtype_bits(d):
            return (
                aikit_backend.dtype_bits(d) / 2
                if aikit_backend.is_complex_dtype(d)
                else aikit_backend.dtype_bits(d)
            )

        if aikit_backend.is_int_dtype(d):
            max_val = aikit_backend.iinfo(d).max
            min_val = aikit_backend.iinfo(d).min
        elif aikit_backend.is_float_dtype(d) or aikit_backend.is_complex_dtype(d):
            max_val = aikit_backend.finfo(d).max
            min_val = aikit_backend.finfo(d).min
        else:
            max_val = 1
            min_val = -1
        if x is None:
            if aikit_backend.is_int_dtype(dtype):
                max_x = aikit_backend.iinfo(dtype).max
                min_x = aikit_backend.iinfo(dtype).min
            elif aikit_backend.is_float_dtype(dtype) or aikit_backend.is_complex_dtype(
                dtype
            ):
                max_x = aikit_backend.finfo(dtype).max
                min_x = aikit_backend.finfo(dtype).min
            else:
                max_x = 1
                min_x = -1
        else:
        ...
        return (
            ...
            and (
                aikit_backend.is_complex_dtype(d)
                or not aikit_backend.is_complex_dtype(dtype)
            )
            and (min_x > 0 or not aikit_backend.is_uint_dtype(dtype))
        )
