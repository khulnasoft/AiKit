...
def general_helpers_dtype_info_helper(backend, kind_dtype, dtype):
    with WithBackendContext(backend) as aikit_backend:
        if kind_dtype == "float":
            return (
                aikit_backend.finfo(dtype).max,
                aikit_backend.finfo(dtype).min,
                getattr(aikit_backend.finfo(dtype), "smallest_normal", None),
            )
        elif kind_dtype == "int":
            return (
                aikit_backend.iinfo(dtype).max,
                aikit_backend.iinfo(dtype).min,
                getattr(aikit_backend.iinfo(dtype), "smallest_normal", None),
            )
...
def get_bounds(draw, *, dtype):
    ...
    if "int" in dtype:
        ...
        if aikit.default(low, 0.0) >= aikit.default(high, 1.0):
            return draw(get_bounds(dtype=dtype))
    return [low, high]
