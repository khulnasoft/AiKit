...
import aikit
from aikit_tests.test_aikit.helpers.test_parameter_flags import FunctionTestFlags
import aikit_tests.test_aikit.helpers.test_parameter_flags as pf
import aikit_tests.test_aikit.helpers.globals as t_globals
from aikit.functional.aikit.data_type import _get_function_list, _get_functions_from_string
from aikit_tests.test_aikit.test_frontends import NativeClass
from aikit_tests.test_aikit.helpers.structs import FrontendMethodData
from aikit_tests.test_aikit.helpers.testing_helpers import _create_transpile_report
...
def traced_if_required(backend: str, fn, test_trace=False, args=None, kwargs=None):
    with BackendHandler.update_backend(backend) as aikit_backend:
        if test_trace:
            try:
                if (
                    ...
                    t_globals.CURRENT_TRACED_DATA[
                        t_globals.CURRENT_RUNNING_TEST.fn_name
                    ][backend] = aikit_backend.trace_graph(fn, args=args, kwargs=kwargs)
                elif (
                    t_globals.CURRENT_RUNNING_TEST.fn_name
                    not in t_globals.CURRENT_TRACED_DATA
                ):
...
def _find_instance_in_args(backend: str, args, array_indices, mask):
    ...
    with BackendHandler.update_backend(backend) as aikit_backend:
        instance = aikit_backend.index_nest(args, instance_idx)
        new_args = aikit_backend.copy_nest(args, to_mutable=False)
        aikit_backend.prune_nest_at_index(new_args, instance_idx)
...
def test_function_backend_computation(
    ...
    if test_flags.test_cython_wrapper:
        aikit.set_cython_wrappers_mode(True)
    else:
        aikit.set_cython_wrappers_mode(False)

    with BackendHandler.update_backend(fw) as aikit_backend:
        # Update variable flags to be compatible with float dtype and with_out args
        test_flags.as_variable = [
            v if aikit_backend.is_float_dtype(d) and not test_flags.with_out else False
            for v, d in zip(test_flags.as_variable, input_dtypes)
        ]

    # update instance_method flag to only be considered if the
    # first term is either an aikit.Array or aikit.Container
    ...
    with BackendHandler.update_backend(fw) as aikit_backend:
        ...
        if instance_method:
            ...
            target_fn = aikit_backend.__dict__[fn_name]
        ...
        if test_flags.with_out and not test_flags.test_trace:
            test_ret = (
                ret_from_target[getattr(aikit_backend.__dict__[fn_name], "out_index")]
                if hasattr(aikit_backend.__dict__[fn_name], "out_index")
                else ret_from_target
            )
            out = aikit_backend.nested_map(
                aikit_backend.zeros_like, test_ret, to_mutable=True, include_derived=True
            )
            if instance_method:
                (
                    ret_from_target,
                ...
                (
                    ...
                    fw,
                    aikit_backend.__dict__[fn_name],
                    *args,
                    **kwargs,
                    out=out,
                    precision_mode=test_flags.precision_mode,
            ...
            assert not aikit_backend.nested_any(
                aikit_backend.nested_multi_map(
                    lambda x, _: x[0] is x[1], [test_ret, out]
                ),
                lambda x: not x,
            ), "the array in out argument does not contain same value as the returned"
            if not max(test_flags.container) and aikit_backend.native_inplace_support:
        ...
        if test_flags.with_copy:
            array_fn = aikit_backend.is_array
            if "copy" in list(inspect.signature(target_fn).parameters.keys()):
                kwargs["copy"] = True
            if instance_method:
                first_array = instance
            else:
                first_array = aikit_backend.func_wrapper._get_first_array(
            ...
            first_array = aikit_backend.stop_gradient(first_array).to_numpy()
            ret_ = aikit_backend.stop_gradient(ret_).to_numpy()
            assert not np.may_share_memory(first_array, ret_)

    ret_device = None
    if isinstance(ret_from_target, aikit_backend.Array):  # TODO use str for now
        ret_device = aikit_backend.dev(ret_from_target)
    return (
        ret_from_target,
        ret_np_flat_from_target,
        ret_device,
...
def test_function_ground_truth_computation(
    ...
    with BackendHandler.update_backend(ground_truth_backend) as gt_backend:
        ...
        assert gt_backend.nested_map(
            lambda x: gt_backend.is_aikit_array(x) if gt_backend.is_array(x) else True,
            ret_from_gt,
        ), f"Ground-truth function returned non-aikit arrays: {ret_from_gt}"
        if test_flags.with_out and not test_flags.test_trace:
            test_ret_from_gt = (
...
def test_function(
    *,
    input_dtypes: Union[aikit.Dtype, List[aikit.Dtype]],
    ...
    **all_as_kwargs_np,
    ...
    if (
        ...
        if backend_to_test not in fw_list or not aikit.nested_argwhere(
            all_as_kwargs_np,
            lambda x: (
...
def _assert_frontend_ret(ret, for_fn=True):
    ...
    if is_ret_tuple:
        non_frontend_idxs = aikit.nested_argwhere(
            ret, lambda _x: not _is_frontend_array(_x) if aikit.is_array(_x) else False
        )
        assert not non_frontend_idxs, (
            ...
            f" {aikit.multi_index_nest(ret, non_frontend_idxs)}"
        )
    elif aikit.is_array(ret):
...
def _transpile_if_required_backend(backend: str, fn_name: str, args=None, kwargs=None):
    iterations = 1
    with BackendHandler.update_backend(backend) as aikit_backend:
        args, kwargs = aikit_backend.args_to_aikit(*args, **kwargs)
        backend_fn = aikit.__dict__[fn_name]
...
def test_frontend_function(
    ...
    **all_as_kwargs_np,
    ...
    with BackendHandler.update_backend(backend_to_test) as aikit_backend:
        ...
        create_frontend_array = local_importer.import_module(
            f"aikit.functional.frontends.{frontend}"
        )._frontend_array
        ...
        if test_flags.with_out and "out" in list(
            ...
            if is_ret_tuple:
                ...
                for ret_array, out_array in zip(flatten_ret, flatten_out):
                    if aikit_backend.native_inplace_support and not any(
                        (aikit_backend.isscalar(ret), aikit_backend.isscalar(out))
                    ):
                        assert ret_array.aikit_array.data is out_array.aikit_array.data
                    assert ret_array is out_array
            else:
                if aikit_backend.native_inplace_support and not any(
                    (aikit_backend.isscalar(ret), aikit_backend.isscalar(out))
                ):
                    assert ret.aikit_array.data is out.aikit_array.data
                assert ret is out
            ...
            if test_flags.generate_frontend_arrays:
                first_array = first_array.aikit_array
            ret_ = ret_.aikit_array
            if "bfloat16" in str(ret_.dtype):
                ret_ = aikit_backend.astype(ret_, aikit_backend.float64)
            if "bfloat16" in str(first_array.dtype):
                first_array = aikit_backend.astype(first_array, aikit_backend.float64)
            if not aikit_backend.is_native_array(first_array):
            ...
            if test_flags.generate_frontend_arrays:
                ...
                aikit_backend.is_native_array(first_array)
                and aikit_backend.inplace_arrays_supported()
            ):
                assert first_array is ret_.aikit_array.data
            elif aikit_backend.is_aikit_array(first_array):
                assert first_array.data is ret_.aikit_array.data
        ...
        if not test_values:
            ret = aikit_backend.nested_map(
                _frontend_array_to_aikit, ret, include_derived={"tuple": True}
            )

    # create frontend framework args
    frontend_config = get_frontend_config(frontend)
    args_frontend = aikit.nested_map(
        lambda x: (
    ...
    # change aikit device to native devices
...
def test_gradient_backend_computation(
    ...
    with BackendHandler.update_backend(backend_to_test) as aikit_backend:
        ...
        def _grad_fn(all_args):
            ...
            call_fn = aikit_backend.__dict__[fn] if isinstance(fn, str) else fn[i]
            ...
            return aikit_backend.nested_map(aikit_backend.mean, ret, include_derived=True)

        with aikit_backend.PreciseMode(test_flags.precision_mode):
            _, grads = aikit_backend.execute_with_gradients(
                _grad_fn,
                [args, kwargs, 0],
                xs_grad_idxs=xs_grad_idxs,
                ret_grad_idxs=ret_grad_idxs,
...
def test_method_backend_computation(
    ...
    init_input_dtypes = aikit.default(init_input_dtypes, [])

    # Constructor arguments #
    init_all_as_kwargs_np = aikit.default(init_all_as_kwargs_np, {})
    # split the arguments into their positional and keyword components
    ...
    with BackendHandler.update_backend(backend_to_test) as aikit_backend:
        init_flags.as_variable = [
            v if aikit_backend.is_float_dtype(d) else False
    ...
    # method arguments #
    method_input_dtypes = aikit.default(method_input_dtypes, [])
    ...
    # Run testing
    with BackendHandler.update_backend(backend_to_test) as aikit_backend:
        ins = aikit_backend.__dict__[class_name](*args_constructor, **kwargs_constructor)
        # TODO remove when the handle_method can properly compute unsupported dtypes
        if any(
            dtype
            in aikit_backend.function_unsupported_dtypes(
            ...
            return
        v_np = None
        if isinstance(ins, aikit_backend.Module):
            if init_with_v:
                v = aikit_backend.Container(
                    ins._create_variables(
                        device=on_device, dtype=method_input_dtypes[0]
                    )
                )
                ins = aikit_backend.__dict__[class_name](
                    *args_constructor, **kwargs_constructor, v=v
                )
            v = ins.__getattribute__("v")
            v_np = v.cont_map(
                lambda x, kc: aikit_backend.to_numpy(x) if aikit_backend.is_array(x) else x
            )
            if method_with_v:
        ...
        if isinstance(ret, aikit_backend.Array):
            ret_device = aikit_backend.dev(ret)
        else:
            ret_device = None
...
def test_method_ground_truth_computation(
    ...
    with BackendHandler.update_backend(ground_truth_backend) as gt_backend:
        ...
        assert gt_backend.nested_map(
            lambda x: gt_backend.is_aikit_array(x) if gt_backend.is_array(x) else True,
            ret_from_gt,
        ), f"Ground-truth method returned non-aikit arrays: {ret_from_gt}"
...
def test_method(
    *,
    init_input_dtypes: Optional[List[aikit.Dtype]] = None,
    method_input_dtypes: Optional[List[aikit.Dtype]] = None,
    ...
    init_as_variable_flags
        ...
        should be treated as an aikit.Array.
    ...
    method_container_flags
        dictates whether the corresponding input argument passed to the method should
        be treated as an aikit Container.
    method_all_as_kwargs_np:
    ...
    init_with_v
        if the class being tested is an aikit.Module, then setting this flag as True will
    ...
    #     and not any(aikit.is_complex_dtype(d) for d in method_input_dtypes)
    # ):
    #     if fw in fw_list:
...
def test_frontend_method(
    *,
    init_input_dtypes: Union[aikit.Dtype, List[aikit.Dtype]] = None,
    method_input_dtypes: Union[aikit.Dtype, List[aikit.Dtype]],
    init_flags,
    method_flags,
    init_all_as_kwargs_np: Optional[dict] = None,
    ...
    init_as_variable_flags
        dictates whether the corresponding input argument passed to the constructor
        should be treated as an aikit.Variable.
    init_num_positional_args
        number of input arguments that must be passed as positional arguments to the
        constructor.
    init_native_array_flags
    ...
    with BackendHandler.update_backend(backend_to_test) as aikit_backend:
        if frontend == "jax":
            importlib.import_module("aikit.functional.frontends.jax").config.update(
                "jax_enable_x64", True
            )
        ...
        args_constructor_aikit, kwargs_constructor_aikit = aikit_backend.args_to_aikit(
            *args_constructor, **kwargs_constructor
        )
        args_method_aikit, kwargs_method_aikit = aikit_backend.args_to_aikit(
            *args_method, **kwargs_method
        )

        args_constructor_np = aikit_backend.nested_map(
            lambda x: (
                aikit_backend.to_numpy(x._data) if isinstance(x, aikit_backend.Array) else x
            ),
            args_constructor_aikit,
            shallow=False,
        )
        kwargs_constructor_np = aikit_backend.nested_map(
            lambda x: (
                aikit_backend.to_numpy(x._data) if isinstance(x, aikit_backend.Array) else x
            ),
            kwargs_constructor_aikit,
            shallow=False,
        )
        args_method_np = aikit_backend.nested_map(
            lambda x: (
                aikit_backend.to_numpy(x._data) if isinstance(x, aikit_backend.Array) else x
            ),
            args_method_aikit,
        ...
        kwargs_method_np = aikit_backend.nested_map(
            lambda x: (
                aikit_backend.to_numpy(x._data) if isinstance(x, aikit_backend.Array) else x
            ),
            kwargs_method_aikit,
        ...
        frontend_fw_module = aikit_backend.utils.dynamic_import.import_module(
            frontend_method_data.aikit_init_module
        )
        aikit_frontend_creation_fn = getattr(
            frontend_fw_module, frontend_method_data.init_name
        )

        ins = aikit_frontend_creation_fn(*args_constructor, **kwargs_constructor)
        ...
        if method_flags.inplace:
            ...
            copy_ins = aikit_frontend_creation_fn(*args_constructor, **kwargs_constructor)
    ...
    frontend_config = get_frontend_config(frontend)
    args_constructor_frontend = aikit.nested_map(
        lambda x: frontend_config.native_array(x) if isinstance(x, np.ndarray) else x,
        args_constructor_np,
        shallow=False,
    )
    kwargs_constructor_frontend = aikit.nested_map(
        lambda x: frontend_config.native_array(x) if isinstance(x, np.ndarray) else x,
        kwargs_constructor_np,
        shallow=False,
    )
    args_method_frontend = aikit.nested_map(
        lambda x: (
            frontend_config.native_array(x)
            if isinstance(x, np.ndarray)
            else (
    ...
    kwargs_method_frontend = aikit.nested_map(
        lambda x: frontend_config.native_array(x) if isinstance(x, np.ndarray) else x,
        kwargs_method_np,
        shallow=False,
    )
...
def _get_nested_np_arrays(nest):
    ...
    """
    indices = aikit.nested_argwhere(nest, lambda x: isinstance(x, np.ndarray))

    ret = aikit.multi_index_nest(nest, indices)
...
def create_args_kwargs(
    *,
    ...
    # create args
    with BackendHandler.update_backend(backend) as aikit_backend:
        args = aikit_backend.copy_nest(args_np, to_mutable=False)
        aikit_backend.set_nest_at_indices(
            args,
            args_idxs,
        ...
        # create kwargs
        kwargs = aikit_backend.copy_nest(kwargs_np, to_mutable=False)
        aikit_backend.set_nest_at_indices(
            kwargs,
            kwargs_idxs,
            test_flags.apply_flags(
...
def wrap_frontend_function_args(argument):
    ...
    with BackendHandler.update_backend(t_globals.CURRENT_FRONTEND_STR) as aikit_frontend:
        if aikit_frontend.nested_any(
            argument,
            lambda x: hasattr(x, "__module__")
            and x.__module__.startswith("aikit.functional.frontends"),
        ):
            return aikit_frontend.output_to_native_arrays(
                aikit_frontend.frontend_outputs_to_aikit_arrays(argument)
            )
    if aikit_frontend.nested_any(argument, lambda x: isinstance(x, aikit_frontend.Shape)):
...
def flatten(*, backend: str, ret):
    ...
    if not isinstance(ret, tuple):
        ret = (ret,)
    with BackendHandler.update_backend(backend) as aikit_backend:
        ret_idxs = aikit_backend.nested_argwhere(ret, aikit_backend.is_aikit_array)
        # no aikit array in the returned values, which means it returned scalar
        if len(ret_idxs) == 0:
            ret_idxs = aikit_backend.nested_argwhere(ret, aikit_backend.isscalar)
            ret_flat = aikit_backend.multi_index_nest(ret, ret_idxs)
            ret_flat = [
                aikit_backend.asarray(
                    x, dtype=aikit_backend.Dtype(str(np.asarray(x).dtype))
                )
                for x in ret_flat
            ]
        else:
...
def flatten_frontend(*, ret, backend: str, frontend_array_fn=None):
    ...
    if not isinstance(ret, tuple):
        ret = (ret,)
    with BackendHandler.update_backend(backend) as aikit_backend:
        ret_idxs = aikit_backend.nested_argwhere(ret, _is_frontend_array)
...
def flatten_frontend_fw_to_np(
    ...
    frontend_ret_idxs = aikit.nested_argwhere(frontend_ret, is_native_array_func)
    if len(frontend_ret_idxs) == 0:  # handle scalars
        frontend_ret_idxs = aikit.nested_argwhere(frontend_ret, isscalar_func)
        frontend_ret_flat = aikit.multi_index_nest(frontend_ret, frontend_ret_idxs)
    else:
        frontend_ret_flat = aikit.multi_index_nest(frontend_ret, frontend_ret_idxs)
...
def flatten_and_to_np(*, backend: str, ret):
    ...
    with BackendHandler.update_backend(backend) as aikit_backend:
        ret = [aikit_backend.to_numpy(x) for x in ret_flat]
    return ret
...
def flatten_frontend_to_np(*, backend: str, ret):
    ...
    with BackendHandler.update_backend(backend) as aikit_backend:
        ...
        if len(ret_idxs) == 0:  # handle scalars
            ret_idxs = aikit_backend.nested_argwhere(ret, aikit_backend.isscalar)
            ret_flat = aikit_backend.multi_index_nest(ret, ret_idxs)
            return [aikit_backend.to_numpy(x) for x in ret_flat]
        else:
            ret_flat = aikit_backend.multi_index_nest(ret, ret_idxs)
            return [aikit_backend.to_numpy(x.aikit_array) for x in ret_flat]
...
def get_ret_and_flattened_np_array(
    ...
    with BackendHandler.update_backend(backend_to_test) as aikit_backend:
        with aikit_backend.PreciseMode(precision_mode):
            ret = fn(*args, **kwargs)

        def map_fn(x):
            if _is_frontend_array(x):
                return x.aikit_array
            elif aikit_backend.is_native_array(x) or isinstance(x, np.ndarray):
                return aikit_backend.to_aikit(x)
            return x

        ret = aikit_backend.nested_map(map_fn, ret, include_derived={"tuple": True})
...
def get_frontend_ret(
    ...
    **kwargs,
    ...
    with BackendHandler.update_backend(backend) as aikit_backend:
        if test_trace:
            args, kwargs = aikit_backend.nested_map(
                _frontend_array_to_aikit, (args, kwargs), include_derived={"tuple": True}
            )
        with aikit_backend.PreciseMode(precision_mode):
...
def _get_transpiled_data_if_required(
    ...
    with BackendHandler.update_backend(backend) as aikit_backend:
        if generate_frontend_arrays:
            args_for_test, kwargs_for_test = aikit.nested_map(
                _frontend_array_to_aikit,
                (args_for_test, kwargs_for_test),
                include_derived={"tuple": True},
            )
        else:
            args_for_test, kwargs_for_test = aikit_backend.args_to_aikit(
                *args_for_test, **kwargs_for_test
            )
    ...
    # compile to get aikit nodes
    with BackendHandler.update_backend(backend) as aikit_backend:
        traced_fn_to_aikit = aikit_backend.trace_graph(
            frontend_fn, to="aikit", args=args_for_test, kwargs=kwargs_for_test
        )
    ...
    aikit_nodes = len(traced_fn_to_aikit._functions)
...
def args_to_container(array_args):
    array_args_container = aikit.Container({str(k): v for k, v in enumerate(array_args)})
...
def _is_frontend_array(x):
    return hasattr(x, "aikit_array")


def _frontend_array_to_aikit(x):
    if _is_frontend_array(x):
        return x.aikit_array
    else:
        return x
...
def args_to_frontend(
    ...
    with BackendHandler.update_backend(backend) as aikit_backend:
        frontend_args = aikit_backend.nested_map(
            arrays_to_frontend(backend=backend, frontend_array_fn=frontend_array_fn),
            args,
            include_derived,
            shallow=False,
        )
        frontend_kwargs = aikit_backend.nested_map(
            arrays_to_frontend(backend=backend, frontend_array_fn=frontend_array_fn),
            kwargs,
            include_derived,
            shallow=False,
