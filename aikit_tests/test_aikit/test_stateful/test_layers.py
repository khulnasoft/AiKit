...
from aikit_tests.test_aikit.helpers.assertions import assert_same_type_and_shape
from aikit_tests.test_aikit.test_functional.test_experimental.test_nn import (
...
all_constant_initializers = (aikit.Zeros, aikit.Ones)
all_gaussian_initializers = (aikit.KaimingNormal, aikit.Siren)
all_uniform_initializers = (aikit.GlorotUniform, aikit.FirstLayerSiren, aikit.Siren)
all_initializers = (
    ...
    with aikit.utils.backend.ContextManager(backend_fw):
        ...
        if method_flags.as_variable[0]:
            x = _variable(
                aikit.asarray(
                    aikit.linspace(
                        aikit.zeros(batch_shape), aikit.ones(batch_shape), channels
                    ),
                    dtype=dtype,
                )
            )
        ...
        if with_v:
            ...
            v = Container(
                {
                    "submodules": {
                        "v0": {
                            ...
                            "b": _variable(
                                aikit.zeros([channels], device=on_device, dtype=dtype)
                            ),
                        },
                        "v2": {
                            "w": _variable(
            ...
            v = None
        if seq_v:
            seq = aikit.Sequential(
                aikit.Linear(channels, channels, device=on_device, dtype=dtype),
                aikit.Dropout(0.0, dtype=dtype),
                aikit.Linear(channels, channels, device=on_device, dtype=dtype),
                device=on_device,
        ...
        ret = seq(x)
        # type test
        assert aikit.is_aikit_array(ret)
        # cardinality test
        assert ret.shape == aikit.Shape(batch_shape + [channels])
        # value test
        if not with_v:
            return
        assert np.allclose(
            aikit.to_numpy(seq(x)), np.array(target), rtol=tolerance_dict[dtype]
        )
