...
def get_converter(aikit_backend, converter):
    return getattr(aikit_backend.Module, converter)
...
def test_from_backend_module(bs_ic_oc, from_class_and_args, backend_fw):
    ...
    # using aikit_backend.utils.backend.ContextManager instead of update_backend,
    # because with_backend doesn't work here
    with aikit.utils.backend.ContextManager(backend_fw) as aikit_backend:
        x = aikit_backend.astype(
            aikit_backend.linspace(
                aikit_backend.zeros(batch_shape),
                aikit_backend.ones(batch_shape),
                input_channels,
            ),
            "float32",
        )
        native_module_class = NATIVE_MODULES[aikit_backend.current_backend_str()]
        module_converter = get_converter(
            aikit_backend, FROM_CONVERTERS[aikit_backend.current_backend_str()]
        )

        if from_class_and_args:
            aikit_module = module_converter(
                native_module_class,
                instance_args=[x],
                constructor_kwargs={
            ...
            if aikit_backend.current_backend_str() == "tensorflow":
            ...
            fw_kwargs = {}
            aikit_module = module_converter(native_module, **fw_kwargs)

        def loss_fn(v_=None):
            out = aikit_module(x, v=v_)
            return aikit_backend.mean(out)
        ...
        for i in range(10):
            loss, grads = aikit_backend.execute_with_gradients(loss_fn, aikit_module.v)
            w = aikit_backend.gradient_descent_update(aikit_module.v, grads, 1e-3)
            aikit_backend.inplace_update(aikit_module.v, w)
            assert loss <= loss_tm1
            loss_tm1 = loss

        # type test
        assert aikit_backend.is_array(loss)
        assert isinstance(grads, aikit_backend.Container)
        # cardinality test
        assert loss.shape == ()
        # value test
...
def test_from_jax_module(bs_ic_oc, from_class_and_args, module_type, backend_fw):
    ...
    with aikit.utils.backend.ContextManager(backend_fw) as aikit_backend:
        ...
        module_converter = FROM_CONVERTERS[aikit_backend.current_backend_str()][
            module_type
        ]
        module_converter = get_converter(
            aikit_backend, FROM_CONVERTERS[aikit_backend.current_backend_str()][module_type]
        )
        ...
        if from_class_and_args:
            ...
            if module_type == "haiku":
                ...
                def forward_fn(*a, **kw):
                    ...
                    return model(aikit_backend.to_native(x))
            ...
            if module_type == "haiku":
                ...
                fw_kwargs["params_fx"] = native_module.init(
                    jax.random.PRNGKey(0), aikit_backend.to_native(x)
                )
            aikit_module = module_converter(native_module, **fw_kwargs)
        ...
        for i in range(10):
            loss, grads = aikit_backend.execute_with_gradients(loss_fn, aikit_module.v)
            aikit_module.v = aikit_backend.gradient_descent_update(
                aikit_module.v, grads, 1e-3
            )
            assert loss < loss_tm1
            loss_tm1 = loss
