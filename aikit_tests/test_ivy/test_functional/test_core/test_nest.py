...
def test_all_nested_indices(nest):
    indices = aikit.all_nested_indices(nest)
...
def test_all_nested_indices_w_nest_checks(nest):
    indices = aikit.all_nested_indices(nest, True)
    assert indices[0] == ["a", 0, 0]
...
def test_copy_nest():
    nest = {
        "a": [aikit.array([0]), aikit.array([1])],
        "b": {"c": [aikit.array([[2], [4]]), aikit.array([[6], [8]])]},
    }
    nest_copy = aikit.copy_nest(nest)
    ...
    nest = NAMEDTUPLE(x=aikit.array([1.0]), y=aikit.array([2.0]))
...
def test_duplicate_array_index_chains(nest, x, y):
    ...
    duplicate_index_chains = aikit.duplicate_array_index_chains(nest)
...
def test_index_nest(nest, index):
    ret = aikit.index_nest(nest, index)
    true_ret = nest
    for i in index:
        true_ret = true_ret[i]
    assert ret == true_ret
...
def test_insert_into_nest_at_indices(nest, indices, values):
    aikit.insert_into_nest_at_indices(nest, indices, values)

    def indices_nest(nest, indices):
        ret = tuple(aikit.index_nest(nest, index) for index in indices)
...
def test_insert_into_nest_index(nest, index, value):
    aikit.insert_into_nest_at_index(nest, index, value)

    assert aikit.index_nest(nest, index) == value
...
def test_map_nest_at_index(nest, index, fn, shallow):
    ...
    result = aikit.map_nest_at_index(nest, index, fn, shallow=shallow)
...
def test_map_nest_at_indices(nest, indices, fn, shallow):
    nest_copy = copy.deepcopy(nest)
    result = aikit.map_nest_at_indices(nest, indices, fn, shallow)
...
def test_multi_index_nest(nest, multi_indices):
    rets = aikit.multi_index_nest(nest, multi_indices)
    true_rets = []
...
def test_nested_argwhere(nest):
    indices = aikit.nested_argwhere(nest, lambda x: x < 5)
    assert indices[0] == ["a", 0, 0]
    assert indices[1] == ["a", 1, 0]
    assert indices[2] == ["b", "c", 0, 0, 0]
    assert indices[3] == ["b", "c", 0, 1, 0]
...
def test_nested_map(x, fn, shallow):
    x_copy = copy.deepcopy(x)
    result = aikit.nested_map(fn, x, shallow=shallow)
    map_nested_dicts(x_copy, fn)
...
def test_nested_multi_map(func, nests):
    nests = aikit.nested_map(
        lambda x: aikit.array(x) if isinstance(x, np.ndarray) else x,
        nests,
        include_derived=True,
        shallow=False,
    )
    # without index_chains specification
    nested_multi_map_res = aikit.nested_multi_map(func, nests)

    # modify this to test for other functions
    nests_without_multi_map_res = nests[0] - nests[1]

    assert aikit.all_equal(nested_multi_map_res, nests_without_multi_map_res)


# prune_empty
@pytest.mark.parametrize("nest", [{"a": [{}, {}], "b": {"c": [1], "d": []}}])
def test_prune_empty(nest):
    ret = aikit.prune_empty(aikit.copy_nest(nest))
    assert ret == {"b": {"c": [1]}}
...
def test_prune_nest_at_index(nest, index):
    ...
    try:
        aikit.prune_nest_at_index(nest, index)
...
def test_prune_nest_at_indices(nest, indices):
    nest_copy = copy.deepcopy(nest)
    aikit.prune_nest_at_indices(nest_copy, indices)
    print(nest_copy)
    for idx in indices:
        try:
...
def test_set_nest_at_index(nest, index, value, shallow):
    nest_copy = copy.deepcopy(nest)
    result = aikit.set_nest_at_index(nest, index, value, shallow=shallow)
...
def test_set_nest_at_indices(nest, indices, values, shallow):
    nest_copy = copy.deepcopy(nest)
    result = aikit.set_nest_at_indices(nest, indices, values, shallow=shallow)
