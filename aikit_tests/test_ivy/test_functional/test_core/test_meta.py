...
@handle_test(
    fn_tree="functional.aikit.fomaml_step",
...
def test_fomaml_step_overlapping_vars(
    ...
    with BackendHandler.update_backend(backend_fw) as aikit_backend:
        ...
        variable_fn = aikit_backend.functional.aikit._variable

        # create variables
        if batched:
            variables = aikit_backend.Container(
                {
                    "latent": variable_fn(
                        aikit_backend.repeat(
                            aikit_backend.array([[0.0]], device=on_device),
                            num_tasks,
                            axis=0,
                        )
                    ),
                    "weight": variable_fn(
                        aikit_backend.repeat(
                            aikit_backend.array([[1.0]], device=on_device),
                            num_tasks,
                            axis=0,
                        )
                    ),
            ...
            variables = aikit_backend.Container(
                {
                    "latent": variable_fn(aikit_backend.array([0.0], device=on_device)),
                    "weight": variable_fn(aikit_backend.array([1.0], device=on_device)),
                }
            )

        # batch
        batch = aikit_backend.Container(
            {"x": aikit_backend.arange(1, num_tasks + 1, dtype="float32")}
        )
        ...
        # numpy
        latent_np = aikit_backend.to_numpy(variables.latent[0:1])
        weight_np = aikit_backend.to_numpy(variables.weight[0:1])
        batch_np = batch.cont_map(lambda x, kc: aikit_backend.to_numpy(x))
        ...
        # meta update
        rets = aikit_backend.fomaml_step(
            batch,
            inner_cost_fn,
        ...
        calc_cost = rets[0]
        if stop_gradients:
            assert aikit_backend.equal(
                aikit_backend.functional.aikit._is_variable(calc_cost, exclusive=True),
                False,
            )
        assert np.allclose(aikit_backend.to_scalar(calc_cost), true_cost)
        outer_grads = rets[1]
        assert np.allclose(
            aikit_backend.to_numpy(outer_grads.weight[0]), np.array(true_weight_grad)
        )
        assert np.allclose(
            aikit_backend.to_numpy(outer_grads.latent[0]), np.array(true_latent_grad)
        )
        if return_inner_v:
            inner_v_rets = rets[2]
            assert isinstance(inner_v_rets, aikit_backend.Container)
...
def test_fomaml_step_shared_vars(
    ...
    with BackendHandler.update_backend(backend_fw) as aikit_backend:
        ...
        if batched:
            ...
            variables = aikit_backend.Container(
                {"latent": variable_fn(aikit_backend.array([1.0], device=on_device))}
            )
        ...
        outer_grads = rets[1]
        assert np.allclose(
            aikit_backend.to_numpy(outer_grads.latent[0]), np.array(true_outer_grad)
        )
        if return_inner_v:
            inner_v_rets = rets[2]
            assert isinstance(inner_v_rets, aikit_backend.Container)
...
def test_maml_step_overlapping_vars(
    ...
    with BackendHandler.update_backend(backend_fw) as aikit_backend:
        ...
        # numpy
        latent_np = aikit_backend.to_numpy(variables.latent)
        weight_np = aikit_backend.to_numpy(variables.weight)
        batch_np = batch.cont_map(lambda x, kc: aikit_backend.to_numpy(x))
        ...
        outer_grads = rets[1]
        assert np.allclose(
            aikit_backend.to_numpy(outer_grads.weight), np.array(true_weight_grad)
        )
        assert np.allclose(
            aikit_backend.to_numpy(outer_grads.latent), np.array(true_latent_grad)
        )
        if return_inner_v:
            inner_v_rets = rets[2]
            assert isinstance(inner_v_rets, aikit_backend.Container)
...
def test_maml_step_shared_vars(
    ...
    with BackendHandler.update_backend(backend_fw) as aikit_backend:
        ...
        # numpy
        variables_np = variables.cont_map(lambda x, kc: aikit_backend.to_aikit(x))
        batch_np = batch.cont_map(lambda x, kc: aikit_backend.to_aikit(x))
        ...
        for sub_batch in batch_np.cont_unstack_conts(0, True, num_tasks):
            ...
            if average_across_steps:
                ...
                true_outer_grad = aikit_backend.multiply(
        ...
        outer_grads = rets[1]
        assert np.allclose(
            aikit_backend.to_numpy(outer_grads.latent),
            aikit_backend.to_numpy(true_outer_grad[0]),
            rtol=1e-1,
            atol=1e-1,
        )
        if return_inner_v:
...
def test_maml_step_unique_vars(
    ...
    with BackendHandler.update_backend(backend_fw) as aikit_backend:
        ...
        outer_grads = rets[1]
        assert np.allclose(
            aikit_backend.to_numpy(outer_grads.weight), np.array(true_outer_grad)
        )
        if return_inner_v:
            inner_v_rets = rets[2]
            assert isinstance(inner_v_rets, aikit_backend.Container)
...
def test_reptile_step(
    ...
    with BackendHandler.update_backend(backend_fw) as aikit_backend:
        ...
        # meta update
        rets = aikit_backend.reptile_step(
            batch,
            inner_cost_fn,
            variables,
            inner_grad_steps,
