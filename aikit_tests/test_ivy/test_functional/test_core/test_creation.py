...
from aikit_tests.test_aikit.test_functional.test_core.test_dtype import astype_helper
...
def _asarray_helper(draw):
    ...
    with BackendHandler.update_backend(test_globals.CURRENT_BACKEND) as aikit_backend:
        x_list = aikit_backend.nested_map(lambda x: x.tolist(), x, shallow=False)
        sh = draw(helpers.get_shape(min_num_dims=1))
        sh = aikit_backend.Shape(sh)
    # np_array = x[0]
...
def _fill_value(draw):
    ...
    with BackendHandler.update_backend(test_globals.CURRENT_BACKEND) as aikit_backend:
        ...
        if aikit_backend.is_int_dtype(dtype):
...
# arange
@handle_test(
    fn_tree="functional.aikit.arange",
...
@handle_test(
    fn_tree="functional.aikit.asarray",
...
# copy array
@handle_test(
    fn_tree="functional.aikit.copy_array",
    dtype_and_x=helpers.dtype_and_values(available_dtypes=helpers.get_dtypes("valid")),
    to_aikit_array_bool=st.booleans(),
    test_with_copy=st.just(True),
)
def test_copy_array(
    *,
    ...
    with BackendHandler.update_backend(backend_fw) as aikit_backend:
        ...
        if test_flags.instance_method:
            ret = x.copy_array(to_aikit_array=to_aikit_array_bool)
        else:
            ret = aikit_backend.copy_array(x, to_aikit_array=to_aikit_array_bool)
        # type test
        test_ret = ret
        test_x = x
        if test_flags.container[0]:
            assert aikit_backend.is_aikit_container(ret)
            test_ret = ret["a"]
            test_x = x["a"]
        if to_aikit_array_bool:
            assert aikit_backend.is_aikit_array(test_ret)
        else:
            assert aikit_backend.is_native_array(test_ret)
        # cardinality test
        assert test_ret.shape == test_x.shape
        # value test
        x, ret = aikit_backend.to_aikit(x), aikit_backend.to_aikit(ret)
...
# empty
@handle_test(
    fn_tree="functional.aikit.empty",
...
# empty_like
@handle_test(
    fn_tree="functional.aikit.empty_like",
...
# from_dlpack
@handle_test(
    fn_tree="functional.aikit.from_dlpack",
    dtype_and_x=helpers.dtype_and_values(
...
def test_from_dlpack(*, dtype_and_x, backend_fw):
    ...
    native_array = aikit.native_array(x[0])
    cap = aikit.to_dlpack(native_array)
    array = aikit.from_dlpack(cap)
    assert aikit.is_native_array(array)


@handle_test(
    fn_tree="functional.aikit.frombuffer",
...
# full_like
@handle_test(
    fn_tree="functional.aikit.full_like",
...
# linspace
@handle_test(
    fn_tree="functional.aikit.linspace",
...
# logspace
@handle_test(
    fn_tree="functional.aikit.logspace",
    dtype_and_start_stop_axis=helpers.dtype_values_axis(
        available_dtypes=helpers.get_dtypes("float"),
        num_arrays=2,
        min_value=-1e5,
...
# meshgrid
@handle_test(
    fn_tree="functional.aikit.meshgrid",
...
# native_array
@handle_test(
    fn_tree="functional.aikit.native_array",
...
# one_hot
@handle_test(
    fn_tree="functional.aikit.one_hot",
...
# ones_like
@handle_test(
    fn_tree="functional.aikit.ones_like",
    dtype_and_x=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtypes("numeric"),
    ),
)
...
# to_dlpack
@handle_test(
    fn_tree="functional.aikit.to_dlpack",
    dtype_and_x=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtypes(kind="float", full=False, key="dtype"),
        min_num_dims=1,
        max_num_dims=5,
...
@handle_test(
    fn_tree="functional.aikit.triu_indices",
...
# zeros
@handle_test(
    fn_tree="functional.aikit.zeros",
    shape=helpers.get_shape(
        allow_none=False,
        min_num_dims=1,
        max_num_dims=5,
...
# zeros_like
@handle_test(
    fn_tree="functional.aikit.zeros_like",
    dtype_and_x=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtypes("valid"),
        min_num_dims=1,
        max_num_dims=5,
