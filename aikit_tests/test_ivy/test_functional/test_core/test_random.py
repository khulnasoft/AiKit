...
# multinomial
@handle_test(
    fn_tree="functional.aikit.multinomial",
...
def test_multinomial(*, everything, seed, test_flags, backend_fw, fn_name, on_device):
    ...
    if seed:
        ...
        assert aikit.any(ret_np == ret_np1)
...
# randint
@handle_test(
    fn_tree="functional.aikit.randint",
...
def test_randint(*, dtype_low_high, seed, test_flags, backend_fw, fn_name, on_device):
    ...
    for u, v in zip(ret, ret_gt):
        assert aikit.all(u >= low)
        assert aikit.all(u < high)
        assert aikit.all(v >= low)
        assert aikit.all(v < high)


# random_normal
@handle_test(
    fn_tree="functional.aikit.random_normal",
...
# random_uniform
@handle_test(
    fn_tree="functional.aikit.random_uniform",
...
def test_seed(seed_val, backend_fw):
    ...
    with BackendHandler.update_backend(backend_fw) as aikit_backend:
        aikit_backend.seed(seed_value=seed_val)


# shuffle
@handle_test(
    fn_tree="functional.aikit.shuffle",
...
def test_shuffle(
    *, dtype_and_x, seed, axis, test_flags, backend_fw, fn_name, on_device
    ...
    for u, v in zip(ret, ret_gt):
        assert aikit.all(aikit.sort(u, axis=0) == aikit.sort(v, axis=0))
