...
def _get_primals_and_tangents(x_, dtype, aikit_backend, primals_cont, tangents_cont):
    if primals_cont:
        primals = aikit_backend.Container(
            {
                "l": {
                    "a": aikit_backend.array(x_[0][0], dtype=dtype),
                    "b": aikit_backend.array(x_[0][1], dtype=dtype),
                }
            }
        )
    else:
        primals = aikit_backend.array(x_[0], dtype=dtype)

    if tangents_cont:
        tangents = aikit_backend.Container(
            {
                "l": {
                    "a": aikit_backend.array([t[0] for t in x_[1]], dtype=dtype),
                    "b": aikit_backend.array([t[0] for t in x_[1]], dtype=dtype),
                }
            }
        )
    else:
        if primals_cont:
            tangents = aikit_backend.array([t[0] for t in x_[1]], dtype=dtype)
        else:
            tangents = aikit_backend.array(x_[1], dtype=dtype).T
...
def test_bind_custom_gradient_function(
    ...
    with BackendHandler.update_backend(backend_fw) as aikit_backend:
        ...
        def inter_func_(x):
            return aikit_backend.__dict__[inter_func_str](x)

        x = aikit_backend.array(x_, dtype=dtype)
        inter_func = aikit_backend.bind_custom_gradient_function(
            inter_func_, custom_grad_fn
        )

        def func(x):
            return aikit_backend.mean(aikit_backend.exp(inter_func(x)))

        ret, grad = aikit_backend.execute_with_gradients(func, x)
...
def test_jvp(x_, dtype, func_str, backend_fw, nested_structs):
    ...
    with BackendHandler.update_backend(backend_fw) as aikit_backend:
        ...
        primals = (primals,)
        tangents = (tangents,)
        primals_out, jvp = aikit_backend.jvp(func, primals, tangents)
...
def test_vjp(x_, dtype, func_str, backend_fw, nested_structs):
    ...
    with BackendHandler.update_backend(backend_fw) as aikit_backend:
        ...
        if nested_structs == "none":
            ...
            primals, tangents = _get_primals_and_tangents(
                x_, dtype, aikit_backend, True, False
            )
        ...
        primals = (primals,)
        tangents = (tangents,)
        primals_out, vjp_fn = aikit_backend.vjp(func, *primals)
        vjp = vjp_fn(*tangents)
        flat_primals_np = helpers.flatten_and_to_np(ret=primals_out, backend=backend_fw)
