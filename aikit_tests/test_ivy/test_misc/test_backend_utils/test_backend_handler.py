...
from aikit_tests.test_aikit.helpers.available_frameworks import _available_frameworks
...
def test_choose_random_backend(excluded):
    backend = aikit.choose_random_backend(excluded=excluded)
    if excluded is None:
...
def test_current_backend(backend, array_type):
    ...
    aikit.unset_backend()
    assert aikit.current_backend(array_type) is importlib.import_module(
    ...
    if "torch" in _available_frameworks():
        ...
        aikit.utils.assertions.check_equal(
            aikit.current_backend(array_type),
...
def test_dynamic_backend_all_combos(middle_backend, end_backend):
    # create an aikit array, container and native container
    a = aikit.array([1, 2, 3])
    b = aikit.array([4, 5, 6])
    aikit_cont = aikit.Container({"w": a, "b": b})

    # clear the backend stack after initialization of inputs
    aikit.unset_backend()

    # set dynamic_backend to false for all objects
    aikit_cont.dynamic_backend = False
    a.dynamic_backend = False
    b.dynamic_backend = False

    # set the middle backend
    aikit.set_backend(middle_backend, dynamic=True)
    var_cont = aikit.Container(
        {
            "w": aikit.gradients._variable(aikit.array([10, 20, 30])),
            "b": aikit.gradients._variable(aikit.array([40, 50, 60])),
        }
    )
    # set dynamic_backend to true for all objects
    aikit_cont.dynamic_backend = True
    a.dynamic_backend = True
    b.dynamic_backend = True

    # set the final backend
    aikit.set_backend(end_backend, dynamic=True)

    # add the necessary asserts to check if the data
    # of the objects are in the correct format

    assert isinstance(a.data, aikit.NativeArray)
    assert isinstance(aikit_cont["b"].data, aikit.NativeArray)

    if {"numpy", "jax"}.intersection([middle_backend, end_backend]):
        # these frameworks don't support native variables
        assert isinstance(var_cont["b"].data, aikit.NativeArray)
    else:
        assert aikit.gradients._is_variable(var_cont["b"])


def test_dynamic_backend_context_manager():
    with aikit.dynamic_backend_as(True):
        a = aikit.array([0.0, 1.0])
        b = aikit.array([2.0, 3.0])

    with aikit.dynamic_backend_as(False):
        c = aikit.array([4.0, 5.0])
        d = aikit.array([6.0, 7.0])
...
def test_dynamic_backend_setter():
    ...
    aikit.set_backend("tensorflow", dynamic=True)
    assert type(a.data) == type_a  # noqa: E721

    a.dynamic_backend = True
    assert isinstance(a.data, tf.Tensor)

    aikit.set_backend("torch", dynamic=True)
...
def test_previous_backend(backend):
    if not aikit.backend_stack:
        assert aikit.previous_backend() is None

    aikit.set_backend(backend)
    stack_before_unset = []
    func_address_before_unset = id(aikit.sum)
    stack_before_unset.extend(aikit.backend_stack)

    previous_backend = aikit.previous_backend()
    stack_after_unset = aikit.backend_stack
    # check that the function id has changed as inverse=True.
    aikit.utils.assertions.check_equal(
        func_address_before_unset, id(aikit.sum), inverse=True, as_array=False
    )
    aikit.utils.assertions.check_equal(
        previous_backend,
    ...
    aikit.utils.assertions.check_equal(aikit.current_backend_str(), backend, as_array=False)
...
def test_set_backend(backend, array_type):
    ...
    stack_before = []
    func_address_before = id(aikit.sum)
    stack_before.extend(aikit.backend_stack)

    aikit.set_backend(backend)
    stack_after = aikit.backend_stack
    # check that the function id has changed as inverse=True.
    aikit.utils.assertions.check_equal(
        func_address_before, id(aikit.sum), inverse=True, as_array=False
    )
    # using aikit assertions to ensure the desired backend is set
    aikit.utils.assertions.check_less(len(stack_before), len(stack_after), as_array=False)
    aikit.utils.assertions.check_equal(aikit.current_backend_str(), backend, as_array=False)
    backend = importlib.import_module(_backend_dict[backend])
    aikit.utils.assertions.check_equal(stack_after[-1], backend, as_array=False)
    x = aikit.array([1, 2, 3])
    aikit.utils.assertions.check_equal(
        str(type(aikit.to_native(x))), array_type, as_array=False
    )
...
def test_unset_backend():
    for backend_str in _available_frameworks():
        aikit.set_backend(backend_str)

    aikit.unset_backend()
    aikit.utils.assertions.check_equal(aikit.backend_stack, [], as_array=False)
...
def test_variables():
    ...
    dyn_cont = aikit.Container({"w": a, "b": b})
    stat_cont = aikit.Container({"w": a, "b": b})
    stat_cont.dynamic_backend = False

    aikit.set_backend("torch", dynamic=True)
    assert aikit.current_backend().gradients.is_variable(dyn_cont["w"].data)

    aikit.set_backend("paddle", dynamic=True)
    assert aikit.current_backend().gradients.is_variable(dyn_cont["w"].data)
