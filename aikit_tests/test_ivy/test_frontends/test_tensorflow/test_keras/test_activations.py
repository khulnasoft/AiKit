...
def simple_test_two_function(
    *,
    ...
    atol_: float = 1e-06,
    aikit_submodules: list = [],
    framework_submodules: list = [],
):
    aikit.set_backend(frontend)
    fn_aikit = aikit.functional.frontends.__dict__[frontend]
    for aikit_submodule in aikit_submodules:
        fn_aikit = fn_aikit.__dict__[aikit_submodule]
    fn_aikit = fn_aikit.__dict__[fn_str]
    ...
    x = aikit.array(x).to_native()

    ret_aikit = fn_aikit(fn_name)(x)
    ret = fn_framework(fn_name)(x)

    ret_aikit = aikit.array(ret_aikit, dtype=dtype_data)
    ret = aikit.array(ret, dtype=dtype_data)

    ret_np_flat = helpers.flatten_and_to_np(ret=ret)
    frontend_ret_np_flat = helpers.flatten_and_to_np(ret=ret_aikit)
...
def test_tensorflow_deserialize(
    *,
    ...
    simple_test_two_function(
        ...
        rtol_=1e-01,
        atol_=1e-01,
        aikit_submodules=["keras", "activations"],
