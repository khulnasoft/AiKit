# global
import numpy as np
import aikit.functional.frontends.jax.lax as jlax
import aikit.functional.frontends.jax.numpy as jnp
...
from aikit_tests.test_aikit.test_functional.test_nn.test_layers import (
    _assume_tf_dilation_gt_1,
)
from aikit.functional.frontends.jax.numpy import can_cast
from aikit.functional.frontends.jax.lax.operators import (
    _dimension_numbers,
    _argsort_tuple,
)
...
def _x_and_filters(draw, dim=2, transpose=False, general=False):
    ...
    vals = aikit.permute_dims(vals, axes=_argsort_tuple(dim_nums[0]))
    filters = draw(
        helpers.array_values(
            dtype=dtype[0],
            shape=filter_shape,
    ...
    filters = aikit.permute_dims(filters, axes=_argsort_tuple(dim_nums[1]))
...
def test_jax_conv_general_dilated(
    *,
    ...
    _assume_tf_dilation_gt_1(aikit.current_backend_str(), on_device, dilations[0])
...
def test_jax_conv_transpose(
    *,
    ...
    dtype, x, filters, dilation, dim_num, stride, pad, fc, pref = x_f_d_other
    _assume_tf_dilation_gt_1(aikit.current_backend_str(), on_device, dilation)
    helpers.test_frontend_function(
        input_dtypes=dtype,
        backend_to_test=backend_fw,
        test_flags=test_flags,
