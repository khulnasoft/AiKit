...
# local
import aikit
from aikit.functional.frontends.numpy.ma.MaskedArray import MaskedArray
...
def test_numpy_data(
    ...
    assert aikit.all(x.data == aikit.array(data[0]))
...
def test_numpy_fill_value(
    ...
    assert x.fill_value == aikit.array(fill, dtype=dtype)
...
def test_numpy_mask(args):
    dtype, data = args
    x = MaskedArray(data[0], mask=aikit.array(data[1]), dtype=dtype, shrink=False)
    assert aikit.all(x.mask == aikit.array(data[1]))
