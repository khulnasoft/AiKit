...
.. _`handle_out_argument`: https://github.com/khulnasoft/aikit/blob/dcfec8b85de3c422dc0ca1970d67cb620cae62a4/aikit/func_wrapper.py#L340
...
.. _`presence of this attribute`: https://github.com/khulnasoft/aikit/blob/8ded4a5fc13a278bcbf2d76d1fa58ab41f5797d0/aikit/func_wrapper.py#L341
.. _`by the backend function`: https://github.com/khulnasoft/aikit/blob/8ded4a5fc13a278bcbf2d76d1fa58ab41f5797d0/aikit/func_wrapper.py#L372
.. _`handled by the wrapper`: https://github.com/khulnasoft/aikit/blob/8ded4a5fc13a278bcbf2d76d1fa58ab41f5797d0/aikit/func_wrapper.py#L373
.. _`_wrap_fn`: https://github.com/khulnasoft/aikit/blob/6497b8a3d6b0d8aac735a158cd03c8f98eb288c2/aikit/container/wrapping.py#L69
...
.. _`aikit.reshape`: https://github.com/khulnasoft/aikit/blob/633eb420c5006a0a17c238bfa794cf5b6add8598/aikit/functional/aikit/manipulation.py#L418
.. _`aikit.astype`: https://github.com/khulnasoft/aikit/blob/8482eb3fcadd0721f339a1a55c3f3b9f5c86d8ba/aikit/functional/aikit/data_type.py#L46
.. _`aikit.asarray`: https://github.com/khulnasoft/aikit/blob/8482eb3fcadd0721f339a1a55c3f3b9f5c86d8ba/aikit/functional/aikit/creation.py#L114
...
.. _`example`: https://github.com/khulnasoft/aikit/blob/0ef2888cbabeaa8f61ce8aaea4f1175071f7c396/aikit/functional/aikit/layers.py#L169-L176
...
The function :func:`aikit.inplace_update` enables explicit inplace updates.
:func:`aikit.inplace_update` is a *primary* function, and the backend-specific implementations for each backend are presented below.
We also explain the rationale for why each implementation is the way it is, and the important differences.

This is one particular area of the Aikit code where, technically speaking, the function :func:`aikit.inplace_update` will result in subtly different behaviour for each backend, unless the :code:`ensure_in_backend` flag is set to :code:`True`.

While :class:`aikit.Array` instances will always be inplace updated consistently, in some cases it is simply not possible to also inplace update the :class:`aikit.NativeArray` which :class:`aikit.Array` wraps, due to design choices made by each backend.
...
Therefore, an inplace update is only performed on :class:`aikit.Array` instances provided in the input.
...
.. code-block:: python
    ...
    def inplace_update(
        x: Union[aikit.Array, np.ndarray],
        val: Union[aikit.Array, np.ndarray],
        /,
        *,
        ensure_in_backend: bool = False,
        keep_input_dtype: bool = False,
    ) -> aikit.Array:
        aikit.utils.assertions.check_inplace_sizes_valid(x, val)
        if aikit.is_array(x) and aikit.is_array(val):
            if keep_input_dtype:
                val = aikit.astype(val, x.dtype)
            (x_native, val_native), _ = aikit.args_to_native(x, val)
...
Following this, an inplace update is then also performed on the :class:`aikit.Array` instance, if provided in the input.
...
Irrespective of whether the native array is a :class:`tf.Tensor` or a :class:`tf.Variable`, an inplace update is then also performed on the :class:`aikit.Array` instance, if provided in the input.
...
.. code-block:: python
    ...
    def inplace_update(
        x: Union[aikit.Array, torch.Tensor],
        val: Union[aikit.Array, torch.Tensor],
        /,
        *,
        ensure_in_backend: bool = False,
        keep_input_dtype: bool = False,
...
The function :func:`aikit.inplace_update` is also *nestable*, meaning it can accept :class:`aikit.Container` instances in the input.
If an :class:`aikit.Container` instance is provided for the argument :code:`x`, then along with the arrays at all of the leaves, the container :code:`x` is **also** inplace updated, meaning that a new :class:`aikit.Container` instance is not created for the function return.

out argument
------------

Most functions in Aikit support inplace updates via the inclusion of a keyword-only :code:`out` argument.
...
All Aikit functions which return a single array should support inplace updates via the :code:`out` argument.
The type hint of the :code:`out` argument is :code:`Optional[aikit.Array]`.
However, as discussed above, if the function is *nestable* then :class:`aikit.Container` instances are also supported.
:class:`aikit.Container` is omitted from the type hint in such cases, as explained in the `Function Arguments <function_arguments.rst>`_ section.

When the :code:`out` argument is unspecified, then the return is simply provided in a newly created :class:`aikit.Array` (or :class:`aikit.Container` if *nestable*).
...
In the case of :class:`aikit.Array` return types, the :code:`out` argument is predominantly handled in `handle_out_argument`_.
...
The implementations of :func:`aikit.tan` for each backend are as follows.
...
If :code:`out` is specified in an Aikit function, then an inplace update is always **also** performed on the :class:`aikit.Array` instance itself, which is how :code:`out` is provided to the function originally.
The inplace update of this :class:`aikit.Array` is always `handled by the wrapper`_.

Alternatively, if :code:`out` is an :class:`aikit.Container`, then the inplace update is always handled by `_wrap_fn`_ in the container wrapping module.
...
This is why we need to call :func:`aikit.inplace_update` explicitly here, to ensure the native inplace update is performed, as well as the :class:`aikit.Array` inplace update.

Another case where we need to use :func:`aikit.inplace_update` with a function that has :attr:`support_native_out` is for the example of the :code:`torch` backend implementation of the :func:`aikit.remainder` function
...
from the :code:`dtype` of the result of the function, we need to use :func:`aikit.inplace_update`, while still trying to utilize the native :code:`out` argument whenever
...
#. generally, inplace updates are performed because memory management is an area of concern for the user.
   ...
   We can maximize the memory efficiency of the function, using inplace updates in as many of the inner Aikit functions as possible.
...
We'll use :func:`aikit.cross_entropy` as an example:
...
Firstly, the return of :func:`aikit.sum` is the same shape and type as the return of the entire function, and so we can also write this output to the :code:`out` argument inplace.
We can then subsequently overwrite the contents of :code:`out` again with the return of the :func:`aikit.negative` function.
...
Additionally, with a PyTorch backend, the :func:`aikit.negative` function defers to the :code:`out` argument of :func:`torch.negative` function directly, which is the most efficient inplace update possible, making use of backend-specific optimizations.

If we had instead simply used the wrapper `handle_out_argument`_, then we would not leverage any of these benefits, and instead simply call :func:`aikit.inplace_update` at the very end of the function call.
...
For example, `aikit.multi_head_attention <https://github.com/khulnasoft/aikit/blob/2045db570d7977830681a7498a3c1045fb5bcc79/aikit/functional/aikit/layers.py#L165>`_ includes support for arbitrary functions passed in the input, including :code:`to_out_fn` which, if specified, is applied to the outputs before returning.
For such functions, the inplace update should just be performed using :func:`aikit.inplace_update` at the end of the function, like `so <https://github.com/khulnasoft/aikit/blob/2045db570d7977830681a7498a3c1045fb5bcc79/aikit/functional/aikit/layers.py#L254>`_.
...
Therefore views and their updates must be tracked through Aikit and extra logic has been added to update views in the case an inplace update happens to any array which is meant to be referencing the same memory.
We call views tracked and updated by Aikit functional views as they work with a functional paradigm.
...
Every function with this wrapper should also have a :code:`copy` argument such that Aikit maintains a way to prevent views from being created if necessary.
What that wrapper does is update a few :class:`aikit.Array` attributes which help keep track of views, how they were created, and which arrays should be updated together.
These attributes are then used in the :func:`aikit.inplace_update` to update all the arrays which are meant to be referencing the same memory, at least to NumPy's standard.
Of course, these are normally only used with a JAX and TensorFlow backend since NumPy and PyTorch natively update their views and Aikit does not need to do any extra handling except for a few functions where PyTorch fails to return views when NumPy does.
The functions currently implemented in the Aikit API where PyTorch fails to return views at the time of writing are :func:`aikit.flip`, :func:`aikit.rot90`, :func:`aikit.flipud`, :func:`aikit.fliplr`.
...
Here's a brief description of the additional attributes added to :class:`aikit.Array` and their usage:
...
.. note::
    Parts of an arrays metadata like :code:`stride` are attributed to the low-level memory layout of arrays while views in :code:`aikit` operate at a higher level of abstraction.
    As a result, :func:`aikit.strides` isn't guaranteed to produce an output reflective of the underlying memory layout if the :class:`aikit.Array` passed in is a view (or in other words has a :code:`_base`).
...
This should have hopefully given you a good feel for inplace updates, and how these are handled in Aikit.
