...
.. _`partial_mixed_handler`: https://github.com/khulnasoft/aikit/blob/a07919ebf64181852a3564c4d994bc1c25bd9a6f/aikit/functional/backends/tensorflow/experimental/layers.py#L817
.. _`handle_partial_mixed_function`: https://github.com/khulnasoft/aikit/blob/a07919ebf64181852a3564c4d994bc1c25bd9a6f/aikit/func_wrapper.py#L981

When implementing functions in Aikit, whether they are primary, compositional, or mixed, we are constantly faced with the question: which backend implementation should Aikit most closely follow?

Extending the Standard
----------------------

It might seem as though this question is already answered.
Aikit fully adheres to the `Array API Standard`_, which helpfully limits our design space for the functions, but in its current form this only covers a relatively small number of functions, which together make up less than half of the functions in Aikit.
Even for Aikit functions which adhere to the standard, the standard permits the addition of extra arguments and function features, provided that they do not contradict the requirements of the standard.
Therefore, we are still faced with the same kind of design decisions for all Aikit functions, even those appearing in the `Array API Standard`_.

What is the Superset?
---------------------

We explain through examples how Aikit always goes for the superset of functionality among the backend frameworks.
This means that even if only one framework supports a certain feature, then we still strive to include this feature in the Aikit function.
The Aikit function then entails the *superset* of all backend features.
...
For example, :func:`aikit.linear` is a fully compositional function, but :func:`torch.nn.functional.linear` also exists.
We should therefore make sure the compositional :func:`aikit.linear` function includes all behaviours supported by :func:`torch.nn.functional.linear`.

A Non-Duplicate Superset
------------------------

It would be easy to assume that implementing the superset simply means adding all arguments from all related functions into the Aikit function.
...
We've already explained that we should not duplicate arguments in the Aikit function when striving for the superset.
Does this mean, provided that the proposed argument is not a duplicate, that we should always add this backend-specific argument to the Aikit function?
The answer is **no**.
...
This has nothing to do with the mathematics of the function, and so is *not* included in the superset considerations when implementing Aikit functions.
...
Given the abstraction layer that Aikit operates at, Aikit is fundamentally unable to control under-the-hood specifics such as those mentioned above.
However, this is by design, and the central benefit of Aikit is the ability to abstract many different runtimes and algorithms under the same banner, unified by their shared fundamental mathematics.
...
Therefore, aikit supports :code:`order` for these functions and any remaining logic surrounding order is handled in the NumPy frontend.
...
However, as is discussed below, we always strive to implement Aikit functions such that they support as many data types and devices as possible.
...
The simplest approach would be to implement :func:`softplus` in each Aikit backend as a simple composition.
...
All Aikit code should be traced for efficiency, and in this case all the :code:`if` and :code:`else` statements are removed, and all that remains is the backend functions which were executed.
...
Implementing the superset does not mean adopting the same default values for arguments, it simply means equipping the Aikit function with the capabilities to execute the superset of behaviours.

More Examples
-------------

We now take a look at some examples, and explain our rationale for deciding upon the function signature that we should use in Aikit.
...
With regards to both of these points, Aikit provides the generalized superset implementation among the backend frameworks, with support for the :code:`out` argument and also support for both numeric and boolean arrays in the input.

However, as discussed above, :func:`np.logical_and` also supports the :code:`where` argument, which we opt to **not** support in Aikit.
This is because the behaviour can easily be created as a composition like so :code:`aikit.where(mask, aikit.logical_and(x, y), aikit.zeros_like(mask))`, and we prioritize the simplicity, clarity, and function uniqueness in Aikit's API in this case, which comes at the cost of reduced runtime efficiency for some functions when using a NumPy backend.
...
But when the framework-specific functions do not cover all superset functionality, Aikit also allows for a mixed-compositional hybrid approach.

Consider the example of :func:`interpolate`.
Most frameworks contain some kind of interpolation function, usually limited to 2D and/or 3D, but :func:`aikit.interpolate` should be much more general, including interpolations across a larger number of dimensions.
...
So the ideal superset implementation for :func:`aikit.interpolate` would be supporting the union of all modes supported by different implementations and support a larger number of dimensions in the input.
...
Aikit allows this using the `partial_mixed_handler`_ attribute on the backend-specific implementation. So the :code:`torch` backend implementation of :func:`interpolate` would look like the following,
...
In case you stumble upon an Aikit function that you think has not included all native framework functionalities in the optimal way, you are invited to let us know in the comment section of `this <https://github.com/khulnasoft/aikit/issues/6406>`_ dedicated issue.


**Round Up**

This should have hopefully given you a good feel of what should and should not be included when deciding how to design a new Aikit function.
In many cases, there is not a clear right and wrong answer, and we arrive at the final decision via open discussion.
If you find yourself proposing the addition of a new function in Aikit, then we will most likely have this discussion on your Pull Request!
